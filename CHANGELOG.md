...
- Added private native host-contract derivation from validated function identity, rebuilt resource ABI, cleanup classification, and real value planning. It preserves owned-resource order across interleaved scalar parameters, selects the exact returned owner even for same-type inputs, rejects hostile lifecycle/ABI/ownership mutations, and remains deliberately unwired from public resource preflight.

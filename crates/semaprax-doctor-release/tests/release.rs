...
fn packaging_script_replays_staged_and_independently_unpacked_bytes_no_clobber() {
    ...
    let first = Command::new("/bin/sh")
        .arg(&script)
        .args(&arguments)
        .env("TAR_OPTIONS", "--exclude=*")
        .env("GZIP", "-1")
        .env("GZIP_OPT", "-1")
        .env("COPYFILE_DISABLE", "0")
        .env("LD_LIBRARY_PATH", "/hostile/loader/root-a")
        .env("DYLD_LIBRARY_PATH", "/hostile/loader/root-a")
        .env("LC_ALL", "C.UTF-8")
        .env("PATH", "/hostile/path-a")
    ...
    let replica = Command::new("/bin/sh")
        ...
        .env("TAR_OPTIONS", "--exclude=semaprax-doctor-worker")
        .env("GZIP", "-9")
        .env("GZIP_OPT", "-9")
        .env("COPYFILE_DISABLE", "1")
        .env("LD_LIBRARY_PATH", "/hostile/loader/root-b")
        .env("DYLD_LIBRARY_PATH", "/hostile/loader/root-b")
        .env("LC_ALL", "C")
        .env("PATH", "/hostile/path-b")

...
mod desktop_api {
    ...
    use crate::authority::{Authority, AuthorityError, Credential, ExactLeasePin};
    use crate::callable_semantics::{
        authenticate_dictionary, AuthenticatedSemanticDictionary, ResponseDecodeBuffers,
        ValidatedExecution, ValidatedOutcome,
    };
    use crate::callable_wire::{encode_request, RequestArgument};
    use crate::descriptor::{Descriptor, DescriptorError, ResultShape, ScalarKind};
    ...
    use semaprax_native_loader::{
        open_admitted_callable_exact, open_admitted_exact, ModuleInstanceId,
        NativeCallableModuleLease, NativeModuleLease, OpenError, MAX_DESCRIPTOR_BYTES,
    };
    ...
    impl NativeHost {
        ...
        pub unsafe fn open_admitted_exact(
            ...
            let descriptor =
                Descriptor::parse(expected_descriptor).map_err(AdmissionError::from)?;
            ...
            let registry = HostOwnershipRegistry::try_new()
                .map_err(|_| AdmissionError::InvalidHostContract)?;
    ...
    impl NativeCallableHost {
        ...
        pub unsafe fn open_admitted_callable_exact(
            ...
            let semantics =
                authenticate_dictionary(&descriptor, dictionary, trace_path_certificate)
        ...
        pub fn call_owned_with_values(
            ...
            let CallableResultShape::OwnedInput { owner_ordinal, .. } = self.descriptor.result
            else {
                return Err(rejected(CallRejection::WrongShape, owners));
            };
            if let Err(error) = self.preflight_owners(&owners) {
                ...
                match parameter {
                    CallableParameter::Scalar {
                        ...
                        Some(ScalarValue::I64(value)) => {
                            arguments.push(RequestArgument::I64(*value))
                        }

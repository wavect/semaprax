...
use semaprax::project_revision_store::{
    windows_host, PreparedEntry, StoredEntry, MAX_STORE_ENTRIES, MAX_STORE_ENTRY_JSON_BYTES,
    MAX_STORE_INVENTORY_ENTRIES, MAX_STORE_MANIFEST_BYTES, MAX_STORE_TOTAL_SOURCE_BYTES,
...
impl CreatedEntry {
    fn authenticate(
        ...
        .into_iter()
        .chain(prepared.sources().iter().map(|source| source.source()))
        .collect::<Vec<_>>();
...
pub(super) fn persist(root_path: &Path, prepared: &PreparedEntry) -> Result<(), Vec<Diagnostic>> {
    ...
    if published_names(&initial).len() >= MAX_STORE_ENTRIES {
        return Err(limit("retained_entries", MAX_STORE_ENTRIES));
    }
...
fn root_inventory(root: &sys::Root, allow_stage: bool) -> Result<Inventory, Vec<Diagnostic>> {
    ...
    for name in published_names(&output) {
        ...
        let header = windows_host::inspect(&entry_json, Some(name))?;
        let expected_inventory =
            expected_inventory(header.sources.iter().map(|(path, _)| path.as_str()))?;
        ...
        for (path, length) in &header.sources {
            if before
                .get(&format!("sources/{path}"))
...
fn write_entry(
    ...
    for source in prepared.sources() {
        let (parent_path, name) = source
            .path()
            .rsplit_once('/')
            .map_or((None, source.path()), |(parent, name)| (Some(parent), name));
...
fn authenticate_prepared(
    ...
    let replayed =
        windows_host::replay(stored, prepared.entry_digest(), prepared.project_revision())?;
...
fn read_stored(
    ...
    let source_facts = windows_host::inspect(&entry_json, None)?.sources;

...
Records containing resources are semantically non-copy, but the runtime does not yet have an opaque-resource destructor ABI or shared cleanup IR. Native and Wasm backends therefore reject every record-bearing module explicitly instead of emitting a representation that could leak or double-drop. Executable record support requires prefix-aware partial moves, exactly-once cleanup, C layout lowering, and the Wasm linear-memory/shadow-stack ABI described in RFC 0002.

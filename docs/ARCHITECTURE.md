...
| Owned bounded Vec | `src/vec_ops.rs` owns exact compiler intrinsic identities/signatures and bounds; `src/prelude.rs` owns `core.vec`, preserves frozen five-operation `semaprax.prelude.v2`, and selects additive v3 only for reserve/set/clear use; source/HIR authenticate explicit Copy-scalar instances and same-owner rebind; cleanup and backends consume those identities. The bounded source `for item in values { body }` profile is resolver-owned sugar over one frozen simple immutable `Vec<T>` binding: it snapshots len once and lowers to existing len/get/while HIR, adding no HIR node, stable ID, graph/cleanup schema, prelude, backend operation, package declaration, or public ABI. Gates live in the existing language and owned-data harnesses; [Owned Bounded Vec v1](OWNED-BOUNDED-VEC-V1.md) and [Owned Bounded Vec For Traversal v1](OWNED-BOUNDED-VEC-FOR-TRAVERSAL-V1.md) own the exact profiles. |
| Owned bounded Box | `src/box_ops.rs` owns the exact compiler intrinsic identities, eight-Copy-scalar profile, `semaprax.box.v1` allocation status and 4,096-live-allocation bound; `src/prelude.rs` preserves frozen v1-v3 and selects additive v4 only for compiler-owned Box use. Cleanup, interpreter, native and Wasm modules own target-neutral one-owner settlement and target-specific storage. `src/project/std_mem.rs` authenticates the exact no-export `std.mem` wrapper/example/conformance sources. [Owned Bounded Box v1](OWNED-BOUNDED-BOX-V1.md) owns the bounded contract; authored `record Box<T>` remains inline storage and regions/arenas/ARC/public Box ABI remain separate. |

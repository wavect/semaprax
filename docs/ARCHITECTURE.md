...
Records containing resources are semantically non-copy, but the runtime does not yet have an opaque-resource finalizer/explicit-close ABI or shared cleanup IR. Native and Wasm backends therefore reject every record-bearing module explicitly instead of emitting a representation that could leak or double-drop. Executable record support still requires exactly-once cleanup, C layout lowering, and the Wasm linear-memory/shadow-stack ABI described in RFC 0002. [RFC 0003](RFC-0003-CLEANUP-AND-RESOURCE-ABI.md) specifies the proposed cleanup and resource ABI contract; it is design only and does not change this backend gate.

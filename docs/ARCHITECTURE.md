...
Resources now require one explicitly identified `drop trivial` or `drop import` strategy. Imported strategies resolve through an explicitly identified interface/import contract with ownership, authority, consumption, result-publication, and failure meaning; the v1 source grammar uses the import `@id` as its logical key while HIR keeps those concepts separate. Resources and records containing resources remain semantically non-copy. The compiler has a shared cleanup plan plus target-neutral reference replay/execution, but neither native nor Wasm executes that plan yet: imported calls are not expressions, resource payload/status ABI lowering is absent, and no backend conformance trace exists. Native and Wasm therefore continue to reject every resource-bearing or record-bearing module; record diagnostics retain precedence. RFC 0003 phases 1–2 are implemented; the reference oracle is phase-3 groundwork, not backend cleanup execution.

...
`hir` is the fail-closed boundary between verified human syntax and future semantic consumers. It resolves nominal types, record fields, projections, and calls through persistent declaration IDs, assigns deterministic structural identities to parameters, locals, expressions, and result values, and represents rooted field references as places. Spans and display names remain diagnostic metadata rather than semantic identity.
...
This remains staged groundwork rather than the sole compiler IR: the current verifier still establishes meaning from parsed AST before HIR resolution. Record declarations, constructors, projections, stable field identities, recursive resource/type facts, and by-value recursion rejection now reach validated HIR and Graph v4. The source checker and HIR validator replay prefix-aware partial-place availability; exposing that flow state in the agent graph remains a separate graph-protocol gate. Variant payloads, cleanup, and target-specific aggregate layouts remain RFC 0002 gates.
...
Resources and records containing resources are semantically non-copy, but the runtime does not yet have an opaque-resource finalizer/explicit-close ABI or shared cleanup IR. Native and Wasm backends therefore reject every resource-bearing or record-bearing module explicitly instead of emitting a representation that could leak or double-drop; the record diagnostic takes precedence when both declaration kinds are present. Executable resource support requires lifecycle declarations and exactly-once cleanup. Executable record support additionally requires C layout lowering and the Wasm linear-memory/shadow-stack ABI described in RFC 0002. [RFC 0003](RFC-0003-CLEANUP-AND-RESOURCE-ABI.md) specifies the proposed cleanup and resource ABI contract; it is design only and does not change these backend gates.

## Semantic graph

`semaprax.graph.v4` is serialized exclusively from validated resolved HIR. It contains the canonical human-source revision, module capabilities, entrypoint declaration ID, explicit-versus-automatic declaration identity origin, resource/record/field/function declaration nodes, typed parameter/result/value identities, effects, structural contracts, sorted declaration-ID call dependencies, and the structural body graph. Record fields carry stable IDs, owner IDs, declaration-order indices, and resolved type IDs. Record construction, temporary projection, and place-projection expressions refer to those field identities rather than display names. Expressions expose a stable `type_id` and ownership mode; the top-level `type_facts` index supplies resolved type structure, copy/resource/size/drop facts, and deterministic layout keys without repeating them at every expression.

...
The native and Wasm emitters now consume only validated HIR for semantic lowering; their parsed-AST entry points are compatibility wrappers that resolve first. A centralized HIR validator rejects duplicate/non-canonical identities, invalid declarations and nominal types, lexical-scope violations, inconsistent expression/call types, definite or conditional resource reuse, contract transfers, undeclared or unpermitted effects, effectful contracts, invalid result bindings, and an invalid entrypoint before either backend emits an artifact. Only after those checks pass, `cleanup` independently rebuilds and compares each function's mandatory storage inventory, so a hostile direct-HIR transform cannot remove or retarget a droppable storage candidate before reaching a backend.

This remains staged groundwork rather than the sole compiler IR: the current verifier still establishes meaning from parsed AST before HIR resolution. Explicit trivial/imported resource lifecycles, declaration-only interface/import contracts, record declarations, constructors, projections, stable identities, recursive resource/type facts, and by-value recursion rejection now reach validated HIR and Graph v5. The source checker and HIR validator independently replay lifecycle compatibility, lifecycle-effect authority, and prefix-aware partial-place availability. `CleanupInventory` catalogs owned parameters, bindings, expression temporaries, provisional results, declaration-ordered record shapes, exact field paths, and one flag/lifecycle pair per resource leaf. Its `discovery_index` is only canonical structural order: it proves neither path-sensitive liveness nor initialization/finalization order. Imported functions are not callable yet, and the CFG/status/exit cleanup plan remains a later compiler phase.
...
Resources now require one explicitly identified `drop trivial` or `drop import` strategy. Imported strategies resolve through an explicitly identified interface/import contract with ownership, authority, consumption, result-publication, and failure meaning; the v1 source grammar uses the import `@id` as its logical key while HIR keeps those concepts separate. Resources and records containing resources remain semantically non-copy. The compiler now has a shared cleanup storage inventory but not path-sensitive cleanup control flow, status selection, exit ordering, runtime lowering, or an executable adapter ABI. Native and Wasm therefore reject every resource-bearing or record-bearing module; record diagnostics retain precedence. RFC 0003 phase 1 and provisional inventory evidence are implemented, not cleanup execution.

## Semantic graph

`semaprax.graph.v5` is serialized exclusively from validated resolved HIR. It contains the canonical human-source revision, module capabilities, entrypoint declaration ID, explicit-versus-automatic declaration identity origin, resource/lifecycle/interface/import/record/field/function nodes, typed ownership and result-publication contracts, effects/authority/failure meaning, structural contracts, sorted declaration-ID call dependencies, and the structural body graph. Context closure follows a resource through its lifecycle, full interface contract, import signatures, and nominal types. Record and expression identity behavior remains as specified by Graph v4.

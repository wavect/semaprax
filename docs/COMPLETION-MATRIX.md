...
| WP-05 `doctor` | Authored, unrun | [Explicit bounded offline-profile selection](DOCTOR-PROBE-V1.md), one scoped admission, selector/platform agreement, canonical missing/unavailable failures, and no ambient fallback; injected-host/version checks retained. Lower-level bounded settlement and native64 Linux syscall-denial fixtures remain separate from the CLI | Implement real offline profile provisioning/admission and complete tool/input, filesystem/broker, network and descendant closure on Linux/macOS/Windows; current production acquisition always reports unavailable. Execute physical gates and tool-compatibility cases. Injected tests and the retained Linux denylist do not establish full no-network enforcement; no v0.2 promotion. |

...
| Standard library | Partial; seven `core`-tier packages with local interpreter, native C11, and Core Wasm conformance; bundled `std.*` manifest dependencies, cross-file borrowed-text linking, and both hardened built-in templates have focused local evidence | [Standard Library v1](STANDARD-LIBRARY-V1.md), [catalog](STANDARD-LIBRARY-CATALOG.md) | Every required module exists at its tier with identities, contracts, effects, examples, conformance on every listed target, and generated documentation; ordinary resolved-package builds, the Everyday profile, and the remaining offline templates ship. |

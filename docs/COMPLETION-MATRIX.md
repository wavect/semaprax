...
| Verified source semantics | Partial; Canonical Semantic Workspace Revision v1 locally exercises one immutable nine-node projection of an already admitted Project with separate semantic, source-projection, manifest, and dependency-closure components plus exact fresh replay. Universal Semantic Transaction v1 binds one exact revision and validates one authority-free display rename through the existing complete Project-candidate path. Complete semantic coverage, a persistent service, and a universal operation algebra remain absent. | [Architecture](ARCHITECTURE.md), [Canonical Semantic Workspace Revision v1](CANONICAL-SEMANTIC-WORKSPACE-REVISION-V1.md), [Universal Semantic Transaction v1](UNIVERSAL-SEMANTIC-TRANSACTION-V1.md) | All admitted language features reach validated HIR only after complete type, effect, contract, and ownership checks; cover every mature semantic object family, supply the persistent incremental service, and broaden the transaction algebra without bypassing validation. |

...
| Verified source semantics | Partial; Canonical Semantic Workspace Revision v1 locally exercises one immutable nine-node projection of an already admitted Project with separate semantic, source-projection, manifest, and dependency-closure components plus exact fresh replay, but does not yet provide complete semantic coverage, a persistent service, or mutation | [Architecture](ARCHITECTURE.md), [Canonical Semantic Workspace Revision v1](CANONICAL-SEMANTIC-WORKSPACE-REVISION-V1.md) | All admitted language features reach validated HIR only after complete type, effect, contract, and ownership checks; cover every mature semantic object family, and supply the persistent incremental service and universal transaction separately. |

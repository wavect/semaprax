...
| Edge and server | Partial; the TCP client/Project lanes now have a hosted extension with Rustls-authenticated TLS clients, explicit bind/accept/close-listener operations, loopback evidence, and deterministic fixture-v2 accept queues; npm/browser remains fixture-v1 only | [Bounded Language Network I/O](BOUNDED-LANGUAGE-NETWORK-IO-V1.md), [Bounded Network Services v1](BOUNDED-NETWORK-SERVICES-V1.md) | Native/Wasm service ABI, server TLS, DNS policy, structured async services, typed HTTP/data adapters, observability, deployment, and load/conformance tests are verified. |

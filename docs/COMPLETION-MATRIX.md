...
| --- | --- | --- |
| Persistent Project agent developer loop | Partial | Project Agent Transport v2 retains one authenticated multi-file Project graph/context/test session. Additive opt-in v3 locally proves one preview-digest-bound display rename of one explicit-ID scalar Web export through a server-derived one-operation patch, complete candidate Project validation, overlapping Project/A0 authority, exact post-commit reload, refreshed graph/context/test state, and unchanged stable-ID Web/Node consumption. Additive opt-in [Project Agent Workflow v1](PROJECT-AGENT-WORKFLOW-V1.md) v4 separates derivation, candidate preview, Project-specific typed Impact and fixed Review, A0 apply, and refreshed pathless Web build; Project Manifest v2 additionally admits a bounded pathless npm carrier whose opaque prepared build retains the trusted Project binding, while context-free inspection proves compiler consistency only and grants no path/filesystem/process/npm/network/registry/publication authority. Additive Project Native Rust SDK v1 locally rebuilds the same authenticated Project after daemon shutdown and proves stable-ID behavior across changed revisions. One unified local acceptance runner composes entry/test interpreter and native O0/O2 execution, Core-Wasm, direct and daemon carriers, the change cycle, and optional TypeScript/Rust consumers. General or multi-file change, imported-alias and identity operations, managed-Workspace publication, exact-head hosted promotion, recovery, and exactly-once delivery remain open. |
...
| Web and portable components | Partial | Existing scalar/Copy/resource Core-Wasm and private Component evidence remains bounded, and the Project-v1 Web/Chromium lane retains its hosted evidence and exact bytes. Additive Project Manifest v2 / Useful Text Consumer v1 locally proves non-escaping borrowed UTF-8 exports, disconnected stable-ID export-root linking, a 65,536-byte cumulative external-input ceiling with invocation-root aliasing charged once, linear-time byte-KMP, and range- and UTF-8-validated Wasm/JS with one public scratch page plus two caller-visible reserved fixed-table pages reset before use, deterministic TypeScript, and an exact offline npm package with context-bound publish verification and consistency-only untrusted-envelope inspection. Exact-head hosted promotion is pending; no registry publication, general text processor, arrays/slices/indexing/iteration, aggregate/resource text exports, Components, or multi-engine conformance is claimed. | General source/algebraic Component selection, resource/non-Copy carriers, imports/capabilities, callbacks/async, callable/FFI aggregate signatures, browser/multi-engine conformance, and stable public Component API/ABI remain open |
...
| Meaning in, verified machine code out | Partial | The locally evidenced `u8` scalar retains checked 0..=255 arithmetic, one-byte Native64 layout, Graph, native O0/O2, and Node/Wasm equivalence. The separate Useful Text Consumer v1 `str` is a borrowed UTF-8 pointer/length view, not a byte collection. The first internal [Portable Indexed Byte Data v1](PORTABLE-INDEXED-BYTE-DATA-V1.md) tranche adds target-independent semantic-u64 `usize` plus non-escaping external `Slice<u8>` parameters, immutable aliases, symbolic parameter-root forwarding provenance, cumulative 65,536-byte host admission, `byte_len`, and total guarded `byte_get -> Option<u8>`. Graph v17 serializes all admitted parameter/alias provenance; interpreter, native O0/O2, and an internal Node/Wasm boundary test are locally green. Public Project/npm adapters, arrays, owned `Bytes`, conversions, allocation budgets, and exact-head hosted promotion remain open, so this is not a complete Useful Data claim. | All safe-language guarantees survive every backend; native artifacts and portable components pass conformance suites on every supported target |
...
| WebAssembly core/components | Partial | Existing scalar, bounded Copy/generic/variant, owned-Wasm, and private Component evidence remains bounded. Public Wasm Scalar Exports v1 retains its hosted scalar JS/TS/Chromium evidence. Additive [Useful Text Consumer v1](USEFUL-TEXT-CONSUMER-V1.md) locally proves explicit stable-ID borrowed-`str` export roots, checked pointer/length and UTF-8 admission, an exact 65,536-byte cumulative input bound, linear-time byte-KMP, fixed three-page memory with only the first 64 KiB public scratch, byte-length/empty/prefix/contains semantics, Unicode/embedded-NUL execution, deterministic JS/TS artifacts, and legacy scalar/Web byte preservation. Exact-head hosted promotion is pending. | WASI, general text processing, arrays/slices/indexing/iteration, aggregate/resource/generic text exports, imports, async/capabilities, additional browser engines, general Component mapping and stable public Component API/ABI verified |
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Interface-first packages and target matrices | Partial | Interface Package Report v1 remains the bounded read-only scalar report. [Project Manifest v2](PROJECT-MANIFEST-V2.md) additionally produces one exact local six-file npm package for the Useful Text Consumer profile and a context-bound `semaprax.project-npm-build.v1` carrier binding package/version, Project/workspace/graph revisions, canonical semantic recipe, artifact order/bytes/digests, cumulative bounds, and payload digest. The opaque prepared build retains trusted Project facts for publish verification; context-free inspection proves compiler consistency only. Local evidence performs no-network pack/install and compiler-free consumption with scripts disabled. This is not registry publication and supplies no resolver, lockfile, dependency graph, compatibility engine, signing, provenance, licenses, SBOM, or hosted promotion. | Resolver, lockfile, compatibility, implementations, capabilities, conformance tests, registry publication, provenance, signatures, licenses, SBOM, and reproducibility verified |
...
| JavaScript and TypeScript | Partial | Public Wasm Scalar Exports v1 retains its hosted stable-ID `bigint`/`boolean` facade. Useful Text Consumer v1 locally adds generated JavaScript `string` inputs and exact TypeScript `string`/`bigint`/`boolean` declarations over validated UTF-8 and fixed Wasm scratch, with stable-ID rename preservation and offline installed-package consumption. The generated package is local and unpublished; exact-head hosted promotion is pending. | General strings/typed arrays, promises/errors, callbacks/resources, additional browsers, package/version compatibility, registry publication, and Component transpilation verified |
...
| Web | Partial | Existing scalar HTML/ES/Wasm and Project-v1 hosted evidence remains unchanged. Project Manifest v2 locally adds the config-validator Useful Text Consumer: export-root linking, validated borrowed UTF-8 adapters, deterministic JS/TS, exact npm package/carrier replay, offline installed execution, and stable-ID display rename. It preserves Project-v1 bytes and makes no npm-registry or general browser claim; exact-head hosted promotion is pending. | DOM/CSS application output, accessible UI, SSR/hydration, general text/collection and Wasm resource/Component support, capabilities, additional browser engines, Canvas/WebGPU escape hatch, registry publication, and deployable sample verified |

...
| Project Revision Store v1 follow-on | Implementation and focused evidence subset authored, unrun; Unix-only; unpromoted | [Project Revision Store v1](PROJECT-REVISION-STORE-V1.md), injected held-root authority, immutable content-addressed canonical inputs, bounded structural/content-address replay for unrelated retained entries, complete selected-entry semantic replay, no-clobber stage publication, and focused authored round-trip/limit/drift/substitution/residue cases | Complete the frozen profile and hostile evidence programme, execute it on required Unix hosts, add an equally strong safe Windows authority before any cross-platform claim, and measure retained revision reuse without promoting the store to an ambient cache. |

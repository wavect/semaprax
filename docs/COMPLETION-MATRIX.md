...
| WP-05 `doctor` | Authored, unrun | [Explicit bounded offline-profile selection](DOCTOR-PROBE-V1.md), one scoped admission, selector/platform agreement, canonical missing/unavailable failures, and no ambient fallback; injected-host/version checks retained. [Borrowed sealed-input acquisition](DOCTOR-SEALED-INPUT-V1.md), lower-level bounded settlement and native64 Linux syscall-denial fixtures remain separate from the CLI | Implement real offline profile provisioning/admission and complete tool/input, filesystem/broker, network and descendant closure on Linux/macOS/Windows; current production acquisition always reports unavailable. Execute sealed-input physical fixtures, platform gates and tool-compatibility cases. Input storage checks, injected tests and the retained Linux denylist do not establish full no-network enforcement; no v0.2 promotion. |
...
| Requirement | Status | Evidence owner | Complete when |
| --- | --- | --- | --- |
| Fast development lane | Partial; prepared Project interpreter/source trace and opt-in internal String interpreter authored and unrun | [Interpreter v1](INTERPRETER-V1.md), [Internal String Interpreter v1](INTERPRETER-INTERNAL-STRINGS-V1.md), [Prepared Project Interpreter and Source Trace v1](PROJECT-PREPARED-INTERPRETER-V1.md), [Public Owned Data API v1](PUBLIC-OWNED-DATA-API-V1.md) | Execute and promote the prepared-worker/trace and opt-in String admission/replay/conformance evidence; then incremental refresh, debugging, hot reload, and semantic equivalence must meet the development-performance target. |

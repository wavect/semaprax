...
| Graph-derived documentation | Partial; `semaprax doc` renders one module's Markdown or `semaprax.doc.v1` documentation from the checked program with the graph revision, and the projections harness proves every documented identity of a graph-carried kind is a `graph` node at the same revision on every committed example | [Documentation Projection v1](DOC-PROJECTION-V1.md) | Document whole projects and their `use` closure, generate the bundled agent skills and standard-library catalog from the same graph document, and expose the projection through the editor and workspace session. |

...
| Whole-project and per-function compiler capacity ceilings | Partial; `SPX-G171` (Workspace Semantic Graph builder-bytes budget, `MAX_BUILDER_BYTES` = 18,874,368 bytes, `src/workspace_graph.rs`) and `SPX-H006`'s cleanup-replay path budget (`MAX_REPLAY_PATHS` = 65,536 terminal paths per function, `src/cleanup_plan/replay.rs`) are measured, named with exact constants, and pinned by boundary regression fixtures; the `SPX-H006` diagnostic now states the actual driver (combinatorial multiplication of independently-combined branch outcomes, not raw branch count) and an actionable remedy instead of only the budget name. Neither budget has been raised: no evidence yet establishes a higher value keeps replay and the semantic cache finite, so this remains a genuine, documented size limit on an "ordinary application," not only a discoverability gap. | [Semantic Kernel v1](SEMANTIC-KERNEL-V1.md), [Workspace Semantic Graph](WORKSPACE-SEMANTIC-GRAPH-V1.md), [RFC 0003](RFC-0003-CLEANUP-AND-RESOURCE-ABI.md) | Either budget is raised with fresh evidence that replay/the semantic cache stay finite at the new value and a same-session cross-backend execution fixture near that bound, or the ceiling is accepted as permanent and this row's status changes to reflect that decision instead of "Partial." |

...
| Unified 1.0 command surface | Partial; one `semaprax` binary now admits `doc`, `verify` (schema-selected front over every independent evidence verifier and agent graph bundles) and `agent inspect`, with executable receipts proven byte-identical to the owning routes; `add`, `fetch`, `query`, `package`, and `agent run/resume/replay/reconcile` remain unadmitted and `doctor` and `build --target rust` still need the private host | [Unified CLI v1](UNIFIED-CLI-V1.md), [Documentation Projection v1](DOC-PROJECTION-V1.md), [Guided CLI Help v4](CLI-HELP-V4.md) | Admit the remaining verbs with their own gates, make the private-host split invisible to promoted workflows, and drive the editor surfaces from the same catalog. |

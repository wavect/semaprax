...
| Project Revision Store v1 follow-on | Implementation and expanded evidence authored, unrun; trusted-root Unix-only; unpromoted | [Project Revision Store v1](PROJECT-REVISION-STORE-V1.md), injected current-euid-owned `0700` held-root authority plus explicit same-principal, ancestor, and Darwin-ACL host preconditions, once-per-invocation bounded replay, complete selected-entry semantic replay, no-clobber publication, authority-neutral post-pivot location, and read-only quarantine of one untraversed inert stage identity | First close the separate Project v9 `SPX-W115` Phase-A admission blocker, then execute literal v1-v10 round trips and the full hostile programme on required Unix hosts; freeze and implement an equally strong safe Windows authority before any cross-platform claim, and measure explicit retained revision reuse without promoting the store to an ambient cache. |
...
| Sandboxed builds and dependencies | Partial | [Capability Manifest v1](CAPABILITY-MANIFEST-V1.md), [Offline Package Lock v1](OFFLINE-PACKAGE-LOCK-V1.md), [Offline Resolver v1](OFFLINE-PACKAGE-RESOLVER-V1.md), [Multi-Package Source Capsule v1](OFFLINE-MULTI-PACKAGE-SOURCE-CAPSULE-V1.md), [Effect-Free Wasm Package Build v1](OFFLINE-PURE-WASM-PACKAGE-BUILD-V1.md) | Execute the authored authority-free resolver, source-capsule, and effect-free internal-Wasm build evidence; then verify reproducible acquired inputs and actual least-authority OS sandbox/dependency enforcement. Empty source authority and no external tool execution are not a hermetic sandbox. |

...
locally green and wired into a pending hosted job. The JavaScript and TypeScript row
...
stable-ID rename gates exist. Project CLI publishes Web only; native C lowering
is internal equivalence evidence, while public native output, project run/test,
and exact-head hosted promotion remain held. Post-publication drift is
`SPX-J103`: the complete digest-bound package remains for caller reconciliation
...
edges, aggregates/resources, generics, test discovery, repository authority, or
production packaging. Totals remain 39 Partial/17 Missing.
...
| Web | Partial | Deployable HTML/ES module/Wasm output now includes the bounded Public Wasm Scalar Exports v1 package: multiple stable-ID scalar functions, deterministic JS/TS bindings, a digest manifest, and a calculator consumer with Node execution. Bounded Project Manifest v1 additionally turns one explicit 2–16-source pure-scalar set into the same digest-bound Web package after one in-memory Phase-A pass and stable-ID `use function` provider closure linking. Its native C lowering and test closure are local evidence only; Project CLI publishes Web only, and public native/run/test commands remain held. The legacy accessible scalar entry and narrow owned-resource adapter remain separate lanes. Exact-head hosted Project and real-browser promotion remain pending | Hosted real-browser behavior, DOM/CSS output, accessible application UI, SSR, hydration, general Wasm resource/Component support, browser capabilities, Canvas/WebGPU escape hatch, deployable sample verified |

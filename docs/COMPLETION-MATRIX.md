...
| --- | --- | --- |
| Human and agent semantic projections | Partial | Canonical `.spx`, validated stable-ID HIR, atomic single-file renames, Graph v10 for legacy/Result programs, and feature-minimal Graph v11 for bounded Option propagation are executable, including immutable record update, generic copy-variant construction/match, exact concrete arguments, authenticated ordinary prelude types, and typed propagation meaning; multi-file typed repair/impact remains open |
...
| Web and portable components | Partial | Scalar Core Wasm, public scalar/nested record and bounded generic/prelude/typed-`?` copy-variant Core Wasm, narrow owned-resource Wasm, and private WIT/component evidence exist. The prelude-bound Portable Result Component v3 and exact Source-Result Component v4 runners are hosted green. Private Scalar Algebraic Component v5 locally freezes six direct-copy `Option`/complete `Result`-matrix exports with exact DAG/layout/mapping KATs, hostile closure, upstream validation, and zero-import runner source locks; pinned Rust 1.97.1 typed execution is hosted pending. General `Result`/`Option`/`?` mapping, records/resources/imports/capabilities, callable/FFI aggregate signatures, browser/multi-engine conformance, and public API/ABI remain open |
...
legacy/Result-only output. Phase-3 evidence now includes status/trace types, independent
...
hostile profile closure, and zero-import runner source/KATs locally; pinned
Rust 1.97.1 typed execution is hosted pending. General source `Result`/`Option`/`?`
component mapping, records/resources/imports/capabilities, callable/FFI
...
| --- | --- | --- | --- |
| Agent-native semantic program | Partial | Graph v10 serializes validated legacy/Result HIR plus complete CleanupPlan v2 plans; program-level Graph v11 and per-function CleanupPlan v3 are derived only from authenticated bounded Option propagation. Exact `try_result` and `try_option` carrier/member/source/target identities, normal-result early exits, shared epilogues, mixed-root context schema binding, recursive type facts, bounded context, prelude-bound SHA-256 revision-v2 renames, and fail-closed APIs are locally executable | Versioned multi-file graph API covers callers, targets, tests, packages, generated artifacts, typed repairs, impact, and semantic review |
...
| WebAssembly core/components | Partial | Validated stable-ID HIR lowers to direct Wasm core with browser ES runtime, checked arithmetic, contracts, HTML, `semaprax.web.v3`, public nested scalar record execution, bounded generic/prelude copy variants, hosted bounded typed-`Result` propagation, and locally green bounded typed-`Option` propagation under Node with complete poison, exact concrete layouts, invalid-tag closure, shared postconditions, and restored shadow-stack state. A Node-executed `semaprax.wasm-owned.v1` subset covers one direct trivial resource. Private components v1-v4 retain their exact hosted evidence. Private Scalar Algebraic Component v5 locally authenticates six fixed direct-copy `Option`/complete `Result`-matrix exports, canonical layouts, exact source/core/profile/component DAG, hostile reindexing/cross-version closure, and zero-import isolated-runner sources; typed Wasmtime execution is hosted pending | Browser/WASI modules, general source-language `Result`/`Option`/`?` Component mapping, general/resource-bearing records or variants, imports, async/capabilities, multi-engine conformance, general canonical resource ABI, stable public aggregate ABI, callable/FFI aggregate signatures, sandboxing, cross-realm/worker identity, public component API/ABI, hosted Option-propagation and v5 runtime evidence, and production native-host/Wasm conformance verified |
...
| WIT and WebAssembly Components | Partial | Deterministic private `SPXWIT01` boundary plus standalone v1, checked v2, Portable Result v3, and Source-Result v4 evidence remain frozen; v3/v4 typed runners are hosted green. Private Scalar Algebraic Component v5 adds a separate WIT 0.3 six-export profile for `Option<i64>`, `Option<bool>`, and every direct-copy `Result<T,E>` combination over `i64`/`bool`, all nested inside outer physical status. Exact source/core/profile/component/layout/mapping KATs, canonical-memory reconstruction, same-signature reindexing and mutation/cross-version closure, upstream validation, default-consumer hiding, invalid-value traps, and isolated zero-import runner source locks are locally green; pinned Rust 1.97.1 typed execution is hosted pending | General source selection/export, nested/record/user-variant/resource/non-copy carriers, imports/capabilities, futures/streams, callbacks/reentrancy, browser and multi-engine conformance, package negotiation, callable/FFI aggregate signatures, public API/ABI, and `SPX-B104`/`SPX-W111` admission verified |

...
Manifest v4](PROJECT-MANIFEST-V4.md) locally add one success-published output
operation and a closed compiler-free command consumer. Exact source/HIR
identity and effect authentication, one-write path analysis, loop/cycle
rejection, cleanup-infallibility replay, interpreter/native O0/O2/Wasm
failure-discard, guest-memory import inspection, carrier replay, and binary
`spxgrep` execution are executable evidence. General language I/O, hosted
promotion, safe Windows npm publication, and registry publication remain open;
existing rows stay Partial and totals remain 56 Partial/0 Missing.

[Project Manifest v5](PROJECT-MANIFEST-V5.md) locally adds the fixed
`useful-data-command.v2` native adapter while preserving Project v1-v4 bytes.
The exact four-capability adapter reads one UTF-8 argument and binary stdin
under a cumulative 65,536-byte bound, invokes the authenticated command stable
ID instead of legacy `main`, and preserves the Wasm/Node success-only transcript
and 0/1/2 exit contract. Windows `wmain`/strict UTF conversion/binary stdio and
Unix UTF-8/SIGPIPE fail-stop behavior are bounded local evidence. The additive
`semaprax.project-npm-build.v4` carrier is independently replayed; native
executables have no deterministic carrier. General I/O, atomic physical stdout,
safe Windows npm publication, hosted promotion, registry/release promotion,
and full v0.2 completion remain open. Existing rows and dashboard totals do not
change.

[Bounded Language Command I/O v1](BOUNDED-LANGUAGE-COMMAND-IO-V1.md), Graph
v19, and Project Manifest v6 deepen the existing Partial language, Project,
native, WebAssembly, JavaScript/TypeScript, Web, package, and edge/server rows
with local evidence only. The checked closure admits one CommandArguments and
one Stdin source inside one immutable snapshot, rejects duplicate sources and
multiple reachable stdin reads, and charges argv plus stdin once against
65,536 bytes. Distinct one-write-per-channel stdout/stderr transcripts share a
65,536-byte output bound and publish together only after bool result and
cleanup settlement. Exact HostCommandCall/cleanup replay, native O0/O2,
private-staging Core-Wasm/Node, Graph v19, and independently replayed Project
v6 carrier evidence are local. Earlier graph/manifest/package/carrier bytes
remain preserved; Windows publication remains fail-closed. Hosted, registry,
and release promotion remain open, so all affected rows stay Partial and
...
| Exit criterion | Status | Exact evidence boundary |
| --- | --- | --- |
| Multi-module `semaprax.toml` calculator | Evidenced | Project Manifest v1 authenticates and links the committed calculator sources; its earlier three-host manifest/closure lane is hosted green. |
| Same verified logic on native and browser targets | Exact-head Ubuntu evidenced | The same linked entry/test closures run through the interpreter, native C11 at O0/O2, and Core Wasm/Node. The Project-generated scalar package runs in exact-head Ubuntu Chromium, but Chromium does not directly execute the manifest entry/test closure or an exhaustive identical failure corpus. |
| Several stable-ID functions callable from TypeScript and Rust | Exact-head Ubuntu evidenced | All six `i64`/`bool` calculator exports are present in generated JS/TS and exact Rust manifest rows. Pinned TypeScript statically calls all six and narrows both result variants; JavaScript executes all six plus arithmetic and contract failures; compiler-free Rust executes all six successes plus a contract failure. |
| Calculator application consumes the Project functions | Exact-head Ubuntu Chromium evidenced | One committed interactive shell consumes both the direct `semaprax.web.v4` package and the multi-module `semaprax.web-project.v1` package through the same four arithmetic stable-ID operations. The separate preflight authenticates both schemas and the complete six-export inventory, while the TypeScript consumer statically calls all six; exact-head evidence is [job 97930658621](https://github.com/wavect/semaprax/actions/runs/32887305666/job/97930658621). |
| `semapraxd` inspect/derive/rename/impact/apply/rebuild loop | Exact-head Ubuntu evidenced | Opt-in Transport v4 retains the authenticated Project, derives the bounded rename, produces candidate-bound Impact/Review, applies through A0, reloads, and returns an independently replayed pathless Web build. |
| Stable external API survives the display rename | Exact-head Ubuntu evidenced | Post-rename `app.wasm`, runtime and binding files, parsed scalar ABI, and exact Rust export rows preserve all six stable IDs while Project/workspace/source/subject revisions change. Node executes the renamed carrier and optional Rust executes both revisions; the interactive shell itself is not compared or run after rename. |
| Project tests prove native/Wasm equivalence | Exact-head Ubuntu evidenced | Before and after rename, the four-operation entry closure returns 42 and the test closure's six-success aggregate returns 0 through the interpreter, native O0/O2, and Core Wasm. Separate Node and Rust consumers execute the exported SDK surface, while TypeScript supplies static declaration-consumption evidence rather than runtime behavior. |
| Full promotion CI for every public claim | Pending | Exact-head [run 32887305666](https://github.com/wavect/semaprax/actions/runs/32887305666) proves the dual Chromium fixture and the blocking Ubuntu product lane, but fails blocking macOS Product Acceptance and all baseline Rust lanes. Windows Product/Public-SDK evidence remains diagnostic or masked and therefore cannot promote the claim. |

The v0.2 aggregate exit therefore remains **Partial**; all seven implementation
criteria have concrete evidence, but the mandatory promotion criterion is not.
Criteria 3 and 5–7 are exercised at exact head by the blocking Ubuntu
[Product Acceptance job 97930658781](https://github.com/wavect/semaprax/actions/runs/32887305666/job/97930658781).
Criterion 2 is jointly evidenced by that native/Core-Wasm/Node job and the
exact-head Ubuntu Chromium job linked above; criterion 4 is exercised by the
Chromium job. These single-host successes do not substitute for criterion 8.

| Milestone | Status | Evidence boundary |
| --- | --- | --- |
| Persistent Project agent developer loop | Partial | Project Agent Transport v2 retains one authenticated multi-file Project graph/context/test session. Additive opt-in v3/v4 locally prove the bounded scalar rename/workflow route, while Project Manifest v2 adds the retained-authority Useful Text carrier. Project Manifest v3 preserves one authenticated Useful Data snapshot and its exact v2 npm carrier. Project Manifest v4 adds the exact stdout-only command closure and v3 npm carrier. Project Manifest v5 locally adds exact fixed adapter authority, one UTF-8 argument plus binary stdin, native stable-ID dispatch rather than legacy `main`, native/Wasm/Node transcript and exit parity, and independently replayed `semaprax.project-npm-build.v4`, without changing v1-v4 bytes. Project Manifest v6 locally adds one exact language-command stable ID, Graph v19 meaning, dual success-published transcripts, and an independently replayed semantic carrier while preserving v1-v5 bytes. Context-free carrier inspection proves compiler consistency only and grants no publication authority. General change, managed-Workspace publication, Windows npm publication, exact-head hosted promotion, registry publication, recovery, and exactly-once delivery remain open. |
...
| Native code and interop | Partial | Scalar C11/Clang and bounded private callable/resource evidence exist. Private Native Rust Interoperability v1 A+B has a frozen scalar current-host static-link implementation with direct held compiler authority, a pre-effect 12-use process arena, exact Windows tool-root/linker handling without ambient child `PATH`, bounded preparation/replay, exact-inventory publication, and fail-stop settlement. Its Ubuntu/macOS/Windows, Rust 1.85, Windows runtime/capacity, and Linux sanitizer promotion gate is green at `50b96dccabe3b3dcbcdf38bab380f3eb8699184c` in [run 32402944574](https://github.com/wavect/semaprax/actions/runs/32402944574). Additive Public Native Rust SDK v1 Phase C exposes a narrow API from the still-unpublished builder and emits an exact nine-file, dependency-free local Cargo package with stable-ID methods, a safe scalar export/import facade, deterministic held archiving, an independently replayed outer manifest, and calculator/callback consumers. Its local publication state now performs the final exact inventory replay before consuming cleanup authority, releases all nine leaf handles, closes both child-directory handles, and retains only the authenticated root for the Windows-compatible no-clobber rename; a blocking settled nested-inventory regression complements the explicitly unsettled negative control, and any later failure preserves exact inert residue. Project Native Rust SDK v1 locally extends that same package to one authenticated Project: a target-neutral exact subject binds canonical manifest, Project/workspace/graph, complete source, and exact export-origin facts; the already linked entry HIR reaches the builder without flatten/reparse; distinct Project descriptor/bundle/outer schemas bind target ABI and artifacts; and the complete six-operation Web/Node plus Rust consumers preserve stable-ID behavior and exact export facts across the daemon rename. Its focused CI lane requires held-tool Rust and pinned TypeScript paths on blocking Ubuntu/macOS jobs. Exact-head Ubuntu passed, macOS failed, and Windows remained diagnostic or masked; successful blocking cross-host promotion remains pending. Project Manifest v5 separately adds one fixed native command adapter that invokes an authenticated two-slice/bool command stable ID, accepts one UTF-8 argument plus binary stdin under a shared bound, and maps success-only transcript outcomes to exits 0/1/2; Windows and Unix adapter boundaries are locally evidenced. It is not general native I/O, a stable runner ABI, deterministic executable packaging, or hosted promotion. The Project and direct local gates are implemented, but exact-head blocking Ubuntu/macOS/Windows promotion remains pending; no registry, root CLI, general Project SDK, import, capability, aggregate, or resource claim follows. Compiler sysroot/dynamic-library descendant provenance, independent linker-index semantic reconstruction, and exact linker-descendant image execution under a same-path race are not claimed. Resources, aggregates, borrowing, strings, async, general native execution, and C/Objective-C/Swift/Kotlin ecosystem import remain open. See [NATIVE-RUST-INTEROP-V1](NATIVE-RUST-INTEROP-V1.md) |
| Web and portable components | Partial | Existing scalar/Copy/resource Core-Wasm and private Component evidence remains bounded. Project v2/v3 locally prove borrowed text and public `Slice<u8>` packages. Project v4 adds a fixed third-page guest transcript with authenticated exports and no stdout/WASI import, plus the installed seven-file `spxgrep` command. Project v5 preserves that Wasm/Node route while binding the exact four-capability fixed adapter contract and v4 npm carrier used for native command parity. Project v6 locally adds closed argument/stdin imports, private stdout/stderr staging, owned-token authentication/copy while live, combined output bounds, arena settlement, and semantic-carrier replay without WASI/console/process authority. Unix publication is safely handle-relative; Windows publication remains fail-closed. Exact-head hosted promotion, registry publication, Components, and multi-engine conformance remain open. | General source/algebraic Component selection, resource/non-Copy public returns, general imports/capabilities, callbacks/async, callable/FFI aggregate signatures, browser/multi-engine conformance, safe Windows publication, and stable public Component API/ABI remain open |
...
Project Manifest v4 additively selects the closed
`useful-data-command.v1` profile and preserves v1-v3 bytes. It admits only the
exact `process.stdout.write` entry closure, retains an effect-free test closure,
and emits an exact seven-file `semaprax.project-npm-build.v3` package. Local
evidence covers success-only guest transcript publication, settlement
fail-stop, combined argv/stdin bounds, exact 0/1/2 command exits, independent
carrier replay, and compiler-free binary `spxgrep` execution. General I/O,
hosted promotion, Windows publication, registry publication, and release
promotion remain open; dashboard totals do not change.

Project Manifest v5 additively selects `useful-data-command.v2` and preserves
v1-v4 bytes. Its fixed adapter owns exactly argument read, stderr write, stdin
read, and stdout write, while the SEMAPRAX closure remains stdout-only. Local
evidence covers one UTF-8 argument plus binary stdin under one cumulative
65,536-byte bound, native stable-ID dispatch instead of legacy `main`, native
and Wasm/Node success-only transcript and 0/1/2 exit parity, Windows
`wmain`/strict conversion/binary stdio, Unix UTF-8/SIGPIPE fail-stop behavior,
and independent `semaprax.project-npm-build.v4` replay. This grants no general
I/O, atomic physical stdout, executable determinism, safe Windows npm
publication, hosted promotion, registry/release promotion, or completed row;
dashboard totals do not change.
...
| Meaning in, verified machine code out | Partial | Portable Indexed Byte Data v1 adds target-independent `usize`, fixed arrays, owned immutable `Bytes`, non-escaping `Slice<u8>`, exact capacity/provenance/cleanup, Graph v17, and interpreter/native/Wasm execution. Indexed Byte Loop v2 adds exact compiler-owned dynamic indexing. Bounded Stdout Transcript v1 adds exact stdout authority, Graph v18, one-write path analysis, and success-only interpreter/native/Wasm publication. Project v3/v4 add public Useful Data and the compiler-free `spxgrep` npm adapter. Project v5 locally adds fixed native execution of the authenticated command stable ID with the same bounded transcript/result/exit meaning as Wasm/Node, while preserving v1-v4 bytes. Bounded Language Command I/O v1 and Project v6 locally add exact args/stdin/stderr language meaning, one CommandArguments plus one Stdin source under a shared cumulative input budget, duplicate-source and repeated-read rejection, Graph v19, replay-authenticated cleanup, distinct combined-bounded success-published transcripts, native O0/O2, private-staging Core-Wasm/Node, and semantic-carrier replay while preserving earlier bytes. Native executable bytes are not claimed deterministic. Complete hostile cross-platform evidence, safe Windows publication, exact-head hosted promotion, registry publication, and release promotion remain open. | All safe-language guarantees survive every backend; native artifacts and portable components pass conformance suites on every supported target |
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Interface-first packages and target matrices | Partial | Project v2/v3 retain their exact Useful Text/Useful Data packages and v1/v2 carriers. Project v4 retains its exact seven-file command package and context-bound `semaprax.project-npm-build.v3`. Project v5 locally adds exact `semaprax.useful-data-command.v2` metadata and the independently replayed `semaprax.project-npm-build.v4` carrier. Project v6 locally adds the exact `language-command-io.v1` profile/input/capability/stable-ID shape and an independently replayed semantic carrier while preserving every earlier manifest, package, and carrier byte. Opaque prepared builds retain trusted Project facts; context-free inspection proves consistency only. Unix publication is handle-relative/create-new; Windows publication remains fail-closed. Registry publication, native executable determinism, and hosted promotion are not claimed. | Resolver, lockfile, compatibility, implementations, general capabilities, safe Windows publication, conformance matrices, registry publication, provenance, signatures, licenses, SBOM, and reproducibility verified |
...
| JavaScript and TypeScript | Partial | Public Wasm Scalar Exports v1 retains its hosted stable-ID `bigint`/`boolean` facade, and Useful Text v1 locally adds bounded `string` inputs. Project v3 locally adds exact `Uint8Array` input declarations and a strict runtime facade that rejects shared/resizable/detached/differently typed/coercible inputs, snapshots accepted bytes, enforces cumulative bounds, and authenticates Wasm/export metadata. The committed six-operation calculator now consumes both direct and Project-generated packages through one stable-ID TypeScript surface; the dual-fixture preflight authenticates their distinct schemas and exact common export inventory, and pinned TypeScript 5.8.3 checks both generated declaration consumers. Exact-head Ubuntu product and dual-Chromium evidence passed in run 32887305666, while blocking macOS Product Acceptance failed and Windows remains diagnostic-only. The package is unpublished and exact-head full-matrix promotion is pending. | General strings/typed arrays and owned-byte results, promises/errors, callbacks/resources, additional browsers, package/version compatibility, registry publication, and Component transpilation verified |
...
| Web | Partial | Existing scalar HTML/ES/Wasm and Project-v1 hosted evidence remains unchanged. The committed interactive calculator runs the direct `semaprax.web.v4` and linked multi-module `semaprax.web-project.v1` packages through the same four arithmetic stable-ID operations on isolated loopback roots; exact dual-schema authentication, complete six-export inventory checks, and strict TypeScript checks precede one serial Chromium lane. Project v2 locally adds the config-validator Useful Text package. Project v3 locally adds profile-exact `Slice<u8>` exports, fixed-memory Wasm, strict snapshotting `Uint8Array` JS/TS, exact v2 npm carrier replay, and offline installed multi-module binary-frame execution while preserving v1/v2 bytes. Project v4 adds the fixed Node `spxgrep` adapter; v5 preserves those Wasm/Node semantics and binds its exact adapter authority/metadata through the v4 npm carrier while adding native parity. Project v6 locally adds strict argv/stdin snapshots, private dual-channel Core-Wasm staging including authenticated owned-token copying while live, combined output bounds, arena settlement, and semantic-carrier replay without WASI/console/process imports. The dual calculator fixture is exact-head hosted green on Ubuntu Chromium in job 97930658621 and makes no registry, multi-engine, or general-browser claim; Windows publication stays fail-closed and full promotion is pending. | DOM/CSS application output, accessible UI, SSR/hydration, general collection and Wasm resource/Component support, capabilities, additional browser engines, safe Windows v2 publication, Canvas/WebGPU escape hatch, registry publication, and deployable sample verified |
...
| Edge and server | Partial | Host-native scalar CLI plus locally evidenced fixed native and Node `spxgrep` command adapters over the success-published bounded transcript. Project v5 binds their one-UTF-8-argument/binary-stdin cumulative bound and exact 0/1/2 exits. Bounded Language Command I/O v1 / Project v6 locally add checked args, one owned stdin read, stderr, a cumulative input bound, a distinct combined-output bound, and atomic semantic dual-transcript settlement across interpreter/native/Wasm without granting general process or WASI authority. This is not a server, general I/O runtime, or atomic/durable physical-output claim. | Server runtime, async I/O, HTTP/data adapters, general native/WASI output, observability, deployment, and load/conformance tests verified |

...
separate independently parsed scalar Component Model binary, and private Node
execution of its extracted core module; the result/status mapping is not yet
composed with that component, and there is no engine-native component runtime.
...
| WIT and WebAssembly Components | Partial | Deterministic private `SPXWIT01` WIT/schema/JavaScript bundle with digest KAT, mutation closure, snapshot-only hostile-object normalization, lossless UTF-8/exact status bounds, and Node execution; separate standards-valid scalar Component Model binary with an independent canonical parser and private subset runtime | Compose the WIT result/status mapping with generated checked SEMAPRAX Wasm and execute through a maintained engine-native Component Model runtime; import/export breadth, resources, futures/streams, versions, capabilities, and multi-language composition verified |
...
| macOS | Partial | A private headless `APPL` bundle packages a native executable plus exact callable-v3 provider/descriptor and locally executes two authenticated owned publications with generation reuse and exact replay; hosted execution is CI-configured | AppKit/SwiftUI host, menus/windows/accessibility, lifecycle, signing/notarization metadata, and representative sample verified |
| Windows | Partial | A private portable PE application directory packages the native executable, exact provider/descriptor, and `asInvoker` manifest; its two-call authenticated ownership/replay path is CI-configured. Existing hosted evidence confirms the callable corpus and dependency isolation in [run 31257545008, job 93103151756](https://github.com/wavect/semaprax/actions/runs/31257545008/job/93103151756) | WinUI host, accessibility, lifecycle, installer/MSIX/signing metadata, and representative application sample verified |
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Token-budgeted semantic context | Partial | Versioned deterministic `semaprax.agent-context.v1` adds exact whole-JSON byte and function-node budgets, used/omitted/deferred accounting, closed truncation reasons, query-bound stable-ID progress frontiers with non-dangling emitted call edges, aggregate pagination plus individual-page oversize rejection, strict CLI options, and selectable compact contracts, parameter/result ownership, effects, and reference-closed types; cleanup/lifecycle/import subgraphs are not claimed, and target/diagnostic/test filters are explicitly unavailable rather than fabricated. Graphify remains evidence-gated in ADR 0001 | Exact model-token budgets, real target/diagnostic/test graph edges, cleanup/lifecycle/import facets, relevance guarantees, persistent indexing, and large-repository benchmarks verified |

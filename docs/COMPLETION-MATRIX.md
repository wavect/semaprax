...
| Web | Partial | Deployable HTML/ES module/Wasm output now includes the bounded Public Wasm Scalar Exports v1 package: multiple stable-ID scalar functions, deterministic JS/TS bindings, a digest manifest, and a calculator consumer with Node execution. The legacy accessible scalar entry and narrow owned-resource adapter remain separate lanes. A hosted real-browser calculator interaction is not yet claimed | Hosted real-browser behavior, DOM/CSS output, accessible application UI, SSR, hydration, general Wasm resource/Component support, browser capabilities, Canvas/WebGPU escape hatch, and deployable sample verified |

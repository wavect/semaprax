...
| WP-06 `new` | Local, partial; public scaffold capsule locally exercised | Fifteen CLI cases pass locally on macOS at `177fccf`; [publication contract](NEW-PROJECT-PUBLICATION-V1.md) retains the lower-level and cross-platform requirements. The additive [Public Project Scaffold Capsule v1](PROJECT-SCAFFOLD-V1.md) has focused local authority-free exact-file derivation/replay and stdout-only CLI evidence without changing private publication bytes. | Execute the complete generator, scaffold replay/CLI preservation corpus, Project check/test, and publication gates on the required hosts. |
| WP-07 quickstart | Local, partial; scaffold case locally exercised | All nine previously recorded [quickstart](QUICKSTART.md) tests pass locally on macOS at `177fccf`; the additive scaffold case also passes locally, while source installation and PATH setup are not exercised by that suite | Execute the documented installation, stdout-only scaffold inspection, and publication sequence against each candidate distribution on required hosts. |
...
| WP-11 `Option<Bytes>` / `Result<Bytes, i64>` | Local, partial | Fixed tags, active-payload handling, TypeScript mapping, cleanup evidence, and a locally executed retained-Project reference evaluator covering all six v8 result shapes, plus [actual generated-facade malformed-tag/carrier and inactive-storage observations](OWNED-NPM-INVOCATION-V1.md#authentic-malformed-result-observations) | Execute the identical native/Wasm/npm branches and hostile-tag equivalence gates on required hosts; one local reference lane and test-local arena/decoder checks do not establish facade enforcement. |
...
| WP-13 Project v8 activation | Local, partial | Exact v8 manifest/profile parsing, bounded multi-root linking, semantic-recipe replay, CLI `web`/`npm`/`rust` routing, a locally executed descriptor-bound retained-Project evaluator for the full zero-to-eight-argument v8 surface, and [Windows full-host owned npm publication](WINDOWS-OWNED-NPM-PUBLICATION-V1.md) with standalone rejection | Execute v1–v7 KAT preservation, standalone/full-host routing, Windows held-publication hostility and every v8 target route at one candidate head. |
| WP-14 frame-payload product | Local, partial | [Owned-data evidence](PUBLIC-OWNED-DATA-API-V1.md) records local macOS interpreter/native/Wasm/npm/Rust, strict TypeScript and Chromium execution, calibrated macOS/Linux ASan/UBSan at `177fccf`, and a compiler-free Linux AArch64 Rust 1.85.1 run of four locked/offline consumers from an exact 30-file handoff; display-rename and canonical/supplemental corpora remain shared | Execute the complete identical corpus and external consumers on required hosts; establish required browser breadth and exact-head hosted evidence. |
...
| Project v9 flat owned record follow-on | Local, partial; unpublished; unpromoted | [Public Flat Owned Record API v1](PUBLIC-FLAT-OWNED-RECORD-API-V1.md), [Project Profile Admission v1](PROJECT-PROFILE-ADMISSION-V1.md), exact descriptor, locally executed retained-Project evaluator, Wasm/npm adapter with bounded pre-copy tuple admission, root native provider, safe Rust whole-context settlement guard, and v1-v10 Revision Store evidence | Execute ordinary Project admission, Revision Store replay, hostile input/provider and identical cross-target physical consumers on required hosts, then make an explicit v9 publication and promotion decision before any dependent profile. |
| Project v10 owned UTF-8 follow-on | Authored, unrun; unpublished; unpromoted; blocked on v9 promotion | [Public Owned UTF-8 API v1](PUBLIC-OWNED-UTF8-API-V1.md), distinct descriptor/digest, descriptor-bound retained reference evaluator with compiler-enforced cumulative String materialization accounting and distinct settlement evidence, bounded Wasm/npm input and inline String ownership/call settlement, v10 native String owner ledger and physical allocation fixtures, validating native provider, safe Rust `String` with whole-context settlement guard, and hostile UTF-8 evidence | First promote v9; execute the retained evaluator's exact byte/allocation and preservation gates, then exact replay/carrier, ownership/capacity/hostility, native allocation settlement, valid/invalid UTF-8, host consumers, native sanitizers, and v1-v9 byte preservation subject to the explicit owned-data internal-String correction on every required target before publication or promotion. Ordinary and owned-data native corrections remain unrun; frozen command/callable String limitations remain open. |
...
| --- | --- | --- | --- |
| Agent-native semantic program | Partial | [RFC 0001](RFC-0001.md), [Agent Context v2](AGENT-CONTEXT-V2.md), [Project Agent Transport v5](PROJECT-AGENT-TRANSPORT-V5.md), [Project Revision Store v1](PROJECT-REVISION-STORE-V1.md), [Semantic Workspace Image v1](SEMANTIC-WORKSPACE-IMAGE-V1.md), [Candidate Protocol v2](IMAGE-CANDIDATE-PROTOCOL-V2.md), [Typed Holes v1](PROJECT-CANDIDATE-HOLES-V1.md) (new additions authored, unrun) | Execute image/candidate/hole replay and drift evidence; complete the [graph-operational programme](GRAPH-OPERATIONAL-PROGRAMME.md), persistent and incremental graph lifecycle, general intentions, separate source authority and representative validation. |
...
| Borrowed views and lifetime safety | Partial | [Useful Text Consumer v1](USEFUL-TEXT-CONSUMER-V1.md), [Shared Loan Plan v1](SHARED-LOAN-PLAN-V1.md), [Projected Owned-Byte Field Shared Borrow v1](PROJECTED-OWNED-BYTE-FIELD-BORROW-V1.md), [Projected Owned-Bytes Synchronous Borrowed Call v1](PROJECTED-OWNED-BYTES-BORROWED-CALL-V1.md) | General lifetime inference, mutable and escaping borrows, borrowing beyond the authored direct owned-byte field and synchronous borrowed-call profiles, cross-file use, and public host ABI behavior are verified. |
...
| Requirement | Status | Evidence owner | Complete when |
| --- | --- | --- | --- |
| Fast development lane | Partial; opt-in internal String interpreter locally exercised, prepared Project interpreter/source trace authored and unrun | [Interpreter v1](INTERPRETER-V1.md), [Internal String Interpreter v1](INTERPRETER-INTERNAL-STRINGS-V1.md), [Prepared Project Interpreter and Source Trace v1](PROJECT-PREPARED-INTERPRETER-V1.md), [Public Owned Data API v1](PUBLIC-OWNED-DATA-API-V1.md) | Execute the prepared-worker/trace gates and complete exact-head cross-platform String admission/replay/conformance promotion; then incremental refresh, debugging, hot reload, and semantic equivalence must meet the development-performance target. |
...
| WebAssembly core and components | Partial | [Wasm Scalar Exports](WASM-SCALAR-EXPORTS-V1.md), [Wasm Owned ABI](WASM-OWNED-ABI-V1.md), [Public Owned UTF-8 API v1](PUBLIC-OWNED-UTF8-API-V1.md), [Standalone Wasm String Settlement v1](WASM-INTERNAL-STRINGS-V1.md#local-validation-record) (selected local evidence) | Complete exact-head cross-platform standalone String structural/ownership/hostility gates; stable Components, resources, capabilities, multi-engine conformance, and packaging must still be verified. |
...
| JavaScript and TypeScript | Partial | [Wasm Scalar Exports v1](WASM-SCALAR-EXPORTS-V1.md), [Public Owned Data API v1](PUBLIC-OWNED-DATA-API-V1.md), [Public Owned UTF-8 API v1](PUBLIC-OWNED-UTF8-API-V1.md), [Standalone Wasm String Settlement v1](WASM-INTERNAL-STRINGS-V1.md), [Standalone String Web package](WASM-INTERNAL-STRINGS-WEB-V1.md#local-validation-record) (selected local Node, TypeScript and Chromium evidence) | Complete exact-head required-host and browser/runtime breadth beyond the locally exercised standalone String gates; stable general bindings, owned resources, async/callbacks and packaging must still be verified. |

...
| Agent Transport v6 public-API follow-on | Focused local executable evidence; unpublished and unpromoted | One opt-in read-only profile returns authenticated Project v8-v11 descriptors and replayed npm carriers with closed profile discriminants; focused direct-equivalence, subject-binding, authority-exclusion and zero-write cases passed locally | Run complete current-tree and hosted gates, add generated-client/cross-language conformance, and make explicit v9-v11 package and transport promotion decisions. |
...
| C++ | Partial; local compiled scalar-package evidence | [C++ Shim v1](CXX-SHIM-V1.md), [C++ scalar package v1](CXX-PACKAGE-V1.md) | Cross-platform compiled consumers, owned and borrowed values, exceptions, maintained templates/adapters, compatibility, packaging, and supported-host conformance are verified. |

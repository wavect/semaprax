...
| WP-05 `doctor` | Authored, unrun | Injected-host checks, human/JSON rendering, real multicall/PATH resolution, strict version tokens, [bounded subprocess/fail-stop settlement and native64 Linux inherited syscall-denial fixtures](DOCTOR-PROBE-V1.md), including filter-install rejection and unsupported-ABI admission | Execute physical Linux/macOS/Windows gates and tool-compatibility cases; complete offline discovery/input closure, filesystem/broker isolation and macOS/Windows enforcement. The Linux denylist is partial isolation, not the programme's full no-network guarantee; no v0.2 promotion. |

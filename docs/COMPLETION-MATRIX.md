...
| Editor integration by meaning | Partial; the VS Code adapter has diagnostics on save, go-to-declaration by stable identity, callers from the persistent call index, a module documentation view, and identity/effect/contract code lenses over the compiler's `query --json` and `doc` routes, plus the saved-source session's typed holes, compiler-admitted repairs, and candidate review; safe rename, ownership and cleanup views, agent lifecycle inspection, and trace/evidence views are not admitted | [VS Code adapter](VSCODE-SAVED-SOURCE-ADAPTER-V1.md), [Unified CLI v1](UNIFIED-CLI-V1.md) | Drive the remaining editor surfaces from the same catalog and prove them in a real Extension Host run. |

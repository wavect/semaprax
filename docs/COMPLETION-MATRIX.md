...
| Project v11 nested owned-record follow-on | Authored locally; unpublished and unpromoted; blocked on v9/v10 decisions | Separate canonical descriptor, retained evaluator, Core-Wasm/npm multi-owner settlement, native provider and safe Rust package are authored with bounded hostile cases; focused checks are not current hosted evidence | Run the complete current-tree gates, close provisioned physical/browser consumers, then make explicit prerequisite and v11 promotion decisions. |

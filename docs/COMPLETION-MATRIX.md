...
v6 carrier evidence are local. The ordinary Project CLI additionally publishes
the selected v6 command as a native process product with binary
match/miss/usage, exact input capacity/plus-one rejection, create-new
destination preservation, and stable-ID display-rename evidence. Earlier
graph/manifest/package/carrier bytes remain preserved; Windows npm publication
...
| Same verified logic on native and browser targets | Prior-head Ubuntu evidenced | The same linked entry/test closures run through the interpreter, native C11 at O0/O2, and Core Wasm/Node. The Project-generated scalar package ran in Ubuntu Chromium at the cited prior head, but Chromium does not directly execute the manifest entry/test closure or an exhaustive identical failure corpus. |
| Several stable-ID functions callable from TypeScript and Rust | Prior-head Ubuntu evidenced | All six `i64`/`bool` calculator exports are present in generated JS/TS and exact Rust manifest rows. Pinned TypeScript statically calls all six and narrows both result variants; JavaScript executes all six plus arithmetic and contract failures; compiler-free Rust executes all six successes plus a contract failure. A strict unpublished workspace binary now exposes the same authenticated Project Rust SDK builder locally; it is not a root or installed/public CLI. |
| Calculator application consumes the Project functions | Prior-head Ubuntu Chromium evidenced | One committed interactive shell consumes the direct `semaprax.web.v4` package plus baseline and display-renamed `semaprax.web-project.v1` packages through the same four arithmetic stable-ID operations in current local evidence. The preflight authenticates exact known-answer subjects and the complete six-export inventory, while the TypeScript consumer statically calls all six; the cited prior-head hosted evidence covers the earlier direct-plus-baseline pair in [job 97930658621](https://github.com/wavect/semaprax/actions/runs/32887305666/job/97930658621). |
...
Criteria 3 and 5–7 were exercised at the cited prior head by the blocking Ubuntu
...
| JavaScript and TypeScript | Partial | Public Wasm Scalar Exports v1 retains its hosted stable-ID `bigint`/`boolean` facade, and Useful Text v1 locally adds bounded `string` inputs. Project v3 locally adds exact `Uint8Array` input declarations and a strict runtime facade that rejects shared/resizable/detached/differently typed/coercible inputs, snapshots accepted bytes, enforces cumulative bounds, and authenticates Wasm/export metadata. The committed six-operation calculator now consumes direct, baseline Project, and display-renamed Project packages through one stable-ID TypeScript surface; the local exact-three preflight authenticates their schemas, exact known-answer Project subjects, and common export inventory. At `d27ba9c`, Ubuntu/macOS Product Acceptance and earlier dual-Chromium evidence passed and diagnostic Windows Product passed; hosted promotion of the third fixture and exact-head success of the blocking configuration remain pending. The package is unpublished and exact-head full-matrix promotion is pending. | General strings/typed arrays and owned-byte results, promises/errors, callbacks/resources, additional browsers, package/version compatibility, registry publication, and Component transpilation verified |

...
| Atomic agent changes | Partial; the locally exercised Universal Semantic Transaction v1 kernel binds one exact Canonical Semantic Workspace Revision and old display name, validates one explicit monomorphic non-main function rename through ProjectCandidate, and exact-replays deterministic authority-free artifacts. Universal Semantic Workflow CLI v1 adds a locally exercised compact `change preview rename-display-name` adapter that derives the old name from the authenticated Project and returns the exact result or evidence, but deliberately performs no commit. It does not publish, admit comments/noncanonical bases, support multiple operations, or establish a universal operation algebra. The separate candidate Git slice authors held executable/cwd launch, exact child ambient state and fail-stop process-group settlement without promoting its unexecuted cross-platform gate. | [Universal Semantic Transaction v1](UNIVERSAL-SEMANTIC-TRANSACTION-V1.md), [Universal Semantic Workflow CLI v1](UNIVERSAL-SEMANTIC-WORKFLOW-CLI-V1.md), [Patch Evidence v1](SEMANTIC-PATCH-EVIDENCE-V1.md), [Workspace Change v1](SEMANTIC-WORKSPACE-CHANGE-V1.md), [Candidate Git Publication v1](PROJECT-CANDIDATE-GIT-PUBLICATION-V1.md) | General supported single- and multi-file semantic changes replay and publish atomically with recovery and provenance; preserve comments and exact unrelated trivia, broaden typed operations and preconditions, execute the Git substitution, descriptor, deadline and descendant-settlement matrix, and close same-principal repository-content hostility. |

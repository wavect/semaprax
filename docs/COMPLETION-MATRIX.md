...
| --- | --- | --- |
| Human and agent semantic projections | Partial | Canonical `.spx`, validated stable-ID HIR, atomic single-file renames, and Graph v8 including immutable record update plus copy-variant construction/match meaning are executable; multi-file typed repair/impact remains open |
| Ownership and cleanup meaning | Partial | Move/partial-place checks plus independently rebuilt and replayed cleanup plans are executable; general lifetimes, aliases, concurrency, FFI, and public physical cleanup remain open |
...
| Copy variants + match v1 bounded execution | Partial | Non-generic nominal unit/direct-`i64`/direct-`bool` cases, explicit construction, exhaustive copy-only match with scalar arms, persistent case/payload IDs, CleanupPlan v1 stable-case branching, Graph v8, deterministic internal Native64/Wasm32 layouts, native C11 O0/O2, and Node/Wasm execution are proven. Tests cover authored construction order, one scrutinee, selected-arm-only evaluation, complete caller-output poison, invalid-tag closure, native/Wasm results, and repeated Wasm stack restoration. Generics, `Option`, `Result`, `?`, resource- or record-bearing variant payloads, non-copy ownership modes, stable public aggregate ABI, callable/component signatures, and public resource admission remain open |
...
| Web and portable components | Partial | Scalar Core Wasm, narrow owned-resource Wasm, and private WIT/component evidence exist; Portable Result Component v3 is hosted green in [job 93309086213](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086213), while general records/resources, source `Result`, browser/multi-engine conformance, and public API remain open |
| Desktop and mobile applications | Partial | Private macOS engine/AppKit ([job 93309086230](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086230)), Swift/iOS XCFramework/app ([job 93309086228](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086228)), and Android JNI/Kotlin app ([job 93309086206](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086206)) gates are green; Windows remains pending after [failed run 31341453652, job 93315816975](https://github.com/wavect/semaprax/actions/runs/31341453652/job/93315816975), and the local `llvm-readobj` fix is unhosted. Public SDKs, UI language, lifecycle breadth, signing/distribution, and device breadth remain open |
...
v8 serializes it. Phase-3 evidence now includes status/trace types, independent
...
cannot widen the public compiler graph or MSRV. The hosted Wasmtime gate is
green in [run 31338834586, job
93309086213](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086213);
source-language `Result`/`Option`, broader component
...
| --- | --- | --- | --- |
| Agent-native semantic program | Partial | Graph v8 serializes validated HIR plus complete target-neutral cleanup plans with lifecycle/interface/import/record/field/variant/case/payload identities, ownership/authority/failure contracts, immutable record updates, copy-variant construction/match/pattern meaning, recursive type facts, bounded lifecycle/call/type context, SHA-256 revision-bound renames, and fail-closed APIs | Versioned multi-file graph API covers callers, targets, tests, packages, generated artifacts, typed repairs, impact, and semantic review |
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Records and algebraic variants | Partial | Canonical record declarations/construction/projection/immutable update plus non-generic nominal unit/direct-`i64`/direct-`bool` variants, explicit construction, and exhaustive copy-only matching; stable member/case IDs; deterministic diagnostics; recursive facts/cycle rejection; record partial-place ownership; checked deterministic Native64/Wasm32 internal layouts; independently replayed record cleanup and stable-case match branches; Graph v8; public nested scalar records and copy variants execute through native C11 O0/O2 and Node/Wasm with exact evaluation order, poison, invalid-tag closure, and Wasm stack restoration; one private shared-plan resource-record harness proves exact cleanup trace and zero liveness | Public resource-bearing record execution/admission, resource- or record-bearing variant payloads, generic variants, non-copy match ownership modes, callable/component aggregate signatures, stable public aggregate ABIs, general native/Wasm aggregate execution, `Option`, `Result`, and `?` verified; ordinary `SPX-B104`/`SPX-W111` gates remain closed |
...
| WebAssembly core/components | Partial | Validated stable-ID HIR lowers to direct Wasm core with browser ES runtime, checked arithmetic, contracts, HTML, `semaprax.web.v3`, public nested scalar record execution, and bounded copy-variant construction/exhaustive matching under Node with complete poison and restored shadow-stack state. A Node-executed `semaprax.wasm-owned.v1` subset covers one direct trivial resource, including same-realm duplicated-host isolation and exact semantic ordinal/reference equality for the shared 14-case corpus. Private component v1 provides an exact scalar fixture; checked v2 composes the unmodified generated scalar core with a frozen checked runtime. Portable Result Component v3 privately lifts its distinct checked two-scalar status/out core as `result<s64, status>`, passes independent and pinned-upstream validation, preserves poison/sticky statuses at the core boundary, and executes typed success/add-overflow/division-by-zero/precondition/postcondition through Wasmtime with zero imports, an empty linker, and no WASI; the hosted gate is green in [run 31338834586, job 93309086213](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086213) | Browser/WASI modules, source-language `Result`/`Option`, general/resource-bearing records or variants, imports, async/capabilities, multi-engine conformance, general canonical resource ABI, stable public aggregate ABI, sandboxing, cross-realm/worker identity, public component API, and production native-host/Wasm conformance verified |

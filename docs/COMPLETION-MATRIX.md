...
| WP-05 `doctor` | Exact-tag hosted regression coverage; production profiles unavailable | [Explicit bounded offline-profile selection](DOCTOR-PROBE-V1.md), sealed-input/bundle parsers, injected-host/version checks, and lower-level settlement tests ran where selected by CI | Implement real offline provisioning/admission and complete tool/input, filesystem/broker, network and descendant closure on Linux/macOS/Windows; structural/parser evidence does not establish full no-network enforcement. |
...
| --- | --- | --- | --- |
| Agent-native semantic program | Partial; same-subject selected local Phase 0 v2 evidence, including managed `ACTIVE` and injected result loss after a real local Git ref update; the bounded `function_signature_review_publish_v1` workflow passed its own exact-subject local evidence gate across generated TypeScript/Python/Rust clients and real local SHA-256 Git publication; later focused work adds typed application failures, per-step response accountability, a generated-client contract digest, and one installed zero-authority TypeScript workflow package with its own local Unix review/publication gate | [RFC 0001](RFC-0001.md), [Agent Context v2](AGENT-CONTEXT-V2.md), [Project Agent Transport v5](PROJECT-AGENT-TRANSPORT-V5.md), [Project Revision Store v1](PROJECT-REVISION-STORE-V1.md), [Semantic Workspace Image v1](SEMANTIC-WORKSPACE-IMAGE-V1.md), [Candidate Protocol v2](IMAGE-CANDIDATE-PROTOCOL-V2.md), [Typed Holes v1](PROJECT-CANDIDATE-HOLES-V1.md), [Phase 0 Evidence v2](GRAPH-OPERATIONAL-PHASE0-EXECUTION-EVIDENCE-V2.md), [Product Workflow v1](IMAGE-SUPPORTED-PRODUCT-WORKFLOW-V1.md), [Application Error Data v1](IMAGE-AGENT-APPLICATION-ERROR-DATA-V1.md), [Response Accountability v1](IMAGE-SUPPORTED-WORKFLOW-RESPONSE-ACCOUNTABILITY-V1.md), [Packaged TypeScript Workflow SDK v1](IMAGE-PACKAGED-TYPESCRIPT-WORKFLOW-SDK-V1.md), [Phase 1 Product Workflow Evidence v1](GRAPH-OPERATIONAL-PHASE1-PRODUCT-WORKFLOW-EXECUTION-EVIDENCE-V1.md) | Add cancellation/session invalidation and real MCP/editor task control, broaden beyond the named scalar workflow, then finish the [graph-operational programme](GRAPH-OPERATIONAL-PROGRAMME.md), persistent and incremental graph lifecycle, general intentions, hosted/target-runtime evidence, separate raw-source authority, and representative validation. |

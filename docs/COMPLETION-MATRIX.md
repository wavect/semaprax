...
| Atomic agent changes | Partial; the locally exercised Universal Semantic Transaction v1 kernel binds one exact Canonical Semantic Workspace Revision and old display name, validates one explicit monomorphic non-main function rename through ProjectCandidate, and exact-replays deterministic authority-free artifacts. It does not publish, admit comments/noncanonical bases, support multiple operations, or establish a universal operation algebra. The separate candidate Git slice authors held executable/cwd launch, exact child ambient state and fail-stop process-group settlement without promoting its unexecuted cross-platform gate. | [Universal Semantic Transaction v1](UNIVERSAL-SEMANTIC-TRANSACTION-V1.md), [Patch Evidence v1](SEMANTIC-PATCH-EVIDENCE-V1.md), [Workspace Change v1](SEMANTIC-WORKSPACE-CHANGE-V1.md), [Candidate Git Publication v1](PROJECT-CANDIDATE-GIT-PUBLICATION-V1.md) | General supported single- and multi-file semantic changes replay and publish atomically with recovery and provenance; preserve comments and exact unrelated trivia, broaden typed operations and preconditions, execute the Git substitution, descriptor, deadline and descendant-settlement matrix, and close same-principal repository-content hostility. |
...
| Unified 1.0 command surface | Partial; one `semaprax` binary now admits `doc`, `verify` (schema-selected front over every independent evidence verifier and agent graph bundles), `agent inspect`, `query` (single-module or authenticated whole-Project declaration search with cross-file call predicates and source paths), and the `package report|lock|resolve` namespace, with executable receipts proven byte-identical to the owning routes; `add` (canonical manifest dependency rows) and `fetch` (cache population from explicit local subjects, no network) are admitted with project-harness gates; `agent run` and `agent replay` execute and re-verify deterministic scripted-transcript runs of the bounded runtime; `agent resume` and `agent reconcile` remain unadmitted and `doctor` now runs in both binaries and only `build --target rust` still needs the private host | [Unified CLI v1](UNIFIED-CLI-V1.md), [Documentation Projection v1](DOC-PROJECTION-V1.md), [Guided CLI Help v4](CLI-HELP-V4.md) | Admit the remaining verbs with their own gates, make the private-host split invisible to promoted workflows, and drive the editor surfaces from the same catalog. |

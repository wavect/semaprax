...
| WP-05 `doctor` | Authored, unrun | Injected-host checks, human/JSON rendering, real multicall/PATH resolution and strict full-version-token regressions | Execute the gates on the supported hosts; bounded subprocess settlement remains open and this does not itself promote v0.2. |
...
| Project v10 owned UTF-8 follow-on | Authored, unrun; unpublished; unpromoted; blocked on v9 promotion | [Public Owned UTF-8 API v1](PUBLIC-OWNED-UTF8-API-V1.md), distinct descriptor/digest, length-delimited Wasm/npm string with bounded pre-copy tuple admission, validating native provider, safe Rust `String` with whole-context settlement guard, and hostile invalid-UTF-8 evidence | First promote v9; then execute exact replay/carrier, input/provider hostility, valid/invalid UTF-8, mixed scalar, host consumer, and v1-v9 byte-preservation gates on every required target before any publication or promotion. |

...
| --- | --- | --- |
| Human and agent semantic projections | Partial | Canonical `.spx`, validated stable-ID HIR, atomic single-file renames, Graph v10 for legacy/Result programs, feature-minimal Graph v11 for bounded Option propagation, and program-derived Graph v12 for any authenticated generic record declaration are executable, including immutable record update, generic Copy record/variant construction, exact structured concrete arguments, authenticated ordinary prelude types, typed propagation meaning, and program-wide mixed-root context binding; multi-file typed repair/impact remains open |
...
| Web and portable components | Partial | Scalar Core Wasm, public scalar/nested and generic Copy records, bounded generic/prelude/typed-`?` copy-variant Core Wasm, narrow owned-resource Wasm, and private WIT/component evidence exist. Portable Result v3, Source-Result v4, and Scalar Algebraic v5 runners are hosted green. Private Nested Record Component v6 fixes one `semaprax:private@0.4.0` nested scalar-record transform; its exact DAG/layout/profile KATs, hostile/upstream/default-consumer gates, security review, and pinned Rust 1.97.1/Wasmtime 47 typed runtime are hosted green in [run 31365363898, job 93383304974](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304974). General algebraic mapping, general/empty/generic/resource component records, imports/capabilities, callable/FFI aggregate signatures, browser/multi-engine conformance, and public API/ABI remain open |
...
program-level Graph v12 precedence while retaining the appropriate per-function
cleanup schema. Phase-3 evidence now includes status/trace types, independent
...
General source `Result`/`Option`/`?`
component mapping, general/empty/generic/resource records, imports/capabilities, callable/FFI
...
| --- | --- | --- | --- |
| Agent-native semantic program | Partial | Graph v10 serializes validated legacy/Result HIR plus complete CleanupPlan v2 plans; program-level Graph v11 and per-function CleanupPlan v3 are derived only from authenticated bounded Option propagation; program-level Graph v12 takes precedence when any authenticated generic record declaration exists. Exact `try_result`/`try_option` carrier identities, generic-record owner/index parameters and structured concrete instances, mixed legacy-root context schema binding, recursive type facts, bounded context, prelude-bound SHA-256 revision-v2 renames, and fail-closed APIs are locally executable while prior v10/v11 snapshots remain unchanged | Versioned multi-file graph API covers callers, targets, tests, packages, generated artifacts, typed repairs, impact, and semantic review |
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Records and algebraic variants | Partial | Canonical record declarations/construction/projection/immutable update plus bounded record and variant templates explicitly instantiated with direct `i64`/`bool` arguments, ordinary compiler-owned `Option<T>`/`Result<T,E>`, exhaustive copy matching, and bounded direct-scalar postfix `?`; stable template member/case IDs; owner/index-ordered multi-parameter record substitution; deterministic diagnostics; exact instance/cache/digest/symbol confusion rejection; recursive facts/cycle rejection; record partial-place ownership; checked deterministic Native64/Wasm32 internal layouts; independently replayed cleanup; Graph v10/v11 plus generic-record-precedence v12; public nested scalar records, generic Copy records, and bounded generic/prelude/typed-`?` values execute through native C11 O0/O2 and Node/Wasm with exact evaluation order, poison, invalid-tag closure, and Wasm stack restoration, with generic records hosted green in [run 31365363898, Ubuntu job 93383304995](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304995); one private shared-plan resource-record harness proves exact cleanup trace and zero liveness | Public resource-bearing record execution/admission, nested or resource generic arguments/fields, resource- or record-bearing variant payloads, generic functions/inference, record patterns or non-copy matching/propagation, residual conversion, `?` in contracts, callable/component aggregate signatures, stable public aggregate ABIs, and general native/Wasm aggregate execution verified; ordinary `SPX-B104`/`SPX-W111` gates remain closed |
...
| Optimizing native lane | Partial | Validated stable-ID HIR lowers to sequenced C11/Clang AOT, including public nested scalar records, exact-instance generic Copy records, and bounded copy variants/matches executed at O0/O2 with internal pointer parameters and caller-owned aggregate result storage; generic record C symbols bind the full record ID plus ordered arguments even for same-layout instances. A public build-only callable-v2 API/CLI emits one selected direct-trivial function as a strict host shared library plus deterministic descriptor/dictionary/certificate and hashed manifest; the private host executes the 14-case corpus at O0/O2; Linux provider sanitizers, Windows callable/dependency isolation, and pinned-nightly Rust-host ASan evidence are green in the current Linux/macOS/Windows hosted-CI matrix | Public resource execution/admission, stable public aggregate ABI, general fallback cleanup/quiescence, Android/iOS profiles, LLVM/MLIR lowering, LTO/PGO, cross-compilation, CPU specialization, debug/release parity, and reproducibility verified |
| WebAssembly core/components | Partial | Validated stable-ID HIR lowers to direct Wasm core with browser ES runtime, checked arithmetic, contracts, HTML, `semaprax.web.v3`, public nested scalar and exact-instance generic Copy records, bounded generic/prelude copy variants, and hosted bounded typed-`Result` and typed-`Option` propagation with complete poison, exact concrete layouts, invalid-tag closure, shared postconditions, and restored shadow-stack state. Generic records are hosted green through 4,096-call Node re-entry in [run 31365363898, Ubuntu job 93383304995](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304995). A Node-executed `semaprax.wasm-owned.v1` subset covers one direct trivial resource. Private components v1-v5 retain their exact evidence; private nested-record Component v6 has hosted pinned-Rust-1.97.1 typed execution in [run 31365363898, job 93383304974](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304974) | Browser/WASI modules, general source-language algebraic Component mapping, general/resource-bearing records or variants, imports, async/capabilities, multi-engine conformance, general canonical resource ABI, stable public aggregate ABI, callable/FFI aggregate signatures, sandboxing, cross-realm/worker identity, public component API/ABI, and production native-host/Wasm conformance verified |
...
| WIT and WebAssembly Components | Partial | Deterministic private `SPXWIT01` boundary plus standalone v1, checked v2, Portable Result v3, Source-Result v4, and Scalar Algebraic v5 remain frozen; v3-v5 typed runners are hosted green. Private Nested Record Component v6 fixes package `semaprax:private@0.4.0`, interface `nested-records`, world `semaprax-private-v6`, and one nested scalar-record `transform` export. Its exact source/core/Inner-layout/Outer-layout/profile/component/DAG KATs, canonical reconstruction, mutation/cross-version closure, upstream validation, default-consumer hiding, local core execution, source locks, security review, and pinned Rust 1.97.1/Wasmtime 47 typed execution are hosted green in [run 31365363898, job 93383304974](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304974) | General source selection/export, general/empty/generic/resource records or algebraic carriers, imports/capabilities, futures/streams, callbacks/reentrancy, browser and multi-engine conformance, package/version negotiation, callable/FFI aggregate signatures, public API/ABI, and `SPX-B104`/`SPX-W111` admission verified |

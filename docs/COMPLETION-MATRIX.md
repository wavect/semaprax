...
| Meaning in, verified machine code out | Partial | The locally evidenced `u8` scalar retains checked 0..=255 arithmetic, one-byte Native64 layout, Graph, native O0/O2, and Node/Wasm equivalence. The internal [Portable Indexed Byte Data v1](PORTABLE-INDEXED-BYTE-DATA-V1.md) tranche now adds target-independent semantic-u64 `usize`, fixed `[u8; N]` arrays including storage-free zero arrays, uniquely owned immutable `Bytes`, non-escaping `Slice<u8>` views, `byte_len`, total guarded `byte_get -> Option<u8>`, `bytes_copy`, `bytes_as_slice`, `array_as_slice`, and `str_as_bytes`. Independent source/resolved-HIR capacity checks enforce exact fixed-storage, owned-allocation, and mixed borrowed-root bounds; named-root provenance, canonical `core.bytes.drop` cleanup, Graph v17, interpreter, native O0/O2, and internal Core-Wasm/Node execution are locally green. Public `useful-data.v1` Project/npm adapters, generated `Uint8Array` facades, installed multi-module acceptance, the full hostile boundary matrix, exact-head hosted promotion, and registry publication remain open, so this is not a complete Useful Data claim. | All safe-language guarantees survive every backend; native artifacts and portable components pass conformance suites on every supported target |
...
| WebAssembly core/components | Partial | Existing scalar, bounded Copy/generic/variant, owned-Wasm, and private Component evidence remains bounded. Public Wasm Scalar Exports v1 retains its hosted scalar JS/TS/Chromium evidence. Additive [Useful Text Consumer v1](USEFUL-TEXT-CONSUMER-V1.md) locally proves its bounded borrowed-`str` profile. The internal byte-data tranche additionally proves fixed-memory array/view carriers and an owned-`Bytes` arena whose nonzero 31-bit tokens are monotonic and never reused, with exact token-plus-length validation, guarded cleanup, mixed-root accounting, and Node execution. This is internal Core-Wasm evidence, not the public `Uint8Array` export profile; exact-head hosted promotion is pending. | WASI, public arrays/slices/indexing/iteration adapters, aggregate/resource/generic text exports, imports, async/capabilities, additional browser engines, general Component mapping and stable public Component API/ABI verified |

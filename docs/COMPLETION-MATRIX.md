...
| --- | --- | --- |
| Persistent Project agent developer loop | Partial | Project Agent Transport v2 retains one authenticated multi-file Project graph/context/test session. Additive opt-in v3 locally proves one preview-digest-bound display rename of one explicit-ID scalar Web export through a server-derived one-operation patch, complete candidate Project validation, overlapping Project/A0 authority, exact post-commit reload, refreshed graph/context/test state, and unchanged stable-ID Web/Node consumption. Additive opt-in [Project Agent Workflow v1](PROJECT-AGENT-WORKFLOW-V1.md) v4 separates derivation, candidate preview, Project-specific typed Impact and fixed Review, A0 apply, and refreshed pathless Web-only build; its independently replayed seven-artifact carrier materializes and runs the stable-ID Node calculator without granting the daemon output, filesystem, process, native, Rust, tool, or environment authority. Additive Project Native Rust SDK v1 locally rebuilds the same authenticated Project after daemon shutdown and proves the generated Rust consumer preserves that stable-ID behavior while Project/workspace/subject/source revisions change; it does not claim whole-package byte equality. One unified local acceptance runner now composes entry/test interpreter and native O0/O2 execution, the Core-Wasm test closure, direct and v4 Web consumers, the complete change cycle, and optionally required pinned-TypeScript and baseline/post-rename Project Rust consumers without widening daemon authority. General or multi-file change, imported-alias and identity operations, managed-Workspace publication, hosted promotion, recovery, and exactly-once delivery remain open. |
...
| Web and portable components | Partial | Scalar Core Wasm, bounded generic Copy functions, public scalar/nested and generic Copy records, bounded Copy-record patterns, bounded generic/prelude/typed-`?` copy-variant Core Wasm, narrow owned-resource Wasm, and private WIT/component evidence exist. Public Wasm Scalar Exports v1 additionally selects up to 32 explicit stable-ID monomorphic `i64`/`bool` functions from a completely scalar, effect-free program and emits only stable-ID-derived adapters plus deterministic digest-bound JavaScript/TypeScript package artifacts. Bounded Project Manifest v1 reuses one in-memory Phase-A pass over 2–16 explicit sources, links real stable-ID `use function` provider closures, and publishes the same linked entry HIR as a digest-bound Web package and, additively, as one explicit create-new native executable (Public Project Native Publication v1) through the unchanged shared Clang C11 lane with pre/post-publication held-input rechecks, existing-destination rejection, and stable-ID rename preservation. Public Project Developer Loop v1 executes the exact retained entry or sole manifest test closure in process with bounded fuel and digest-bound deterministic outcomes; it performs no target execution, artifact publication, process spawn, or test discovery. The earlier Project Manifest/Web lane's bounded admission, Node, status, package, rename, pinned-TypeScript, and Chromium evidence is hosted green at `d883ace579bfd86f723cdc6819224fde51f0677d` in [run 32523952912](https://github.com/wavect/semaprax/actions/runs/32523952912), while the developer-loop tranche is locally evidenced only; neither claims aggregate/resource exports, interface/native imports or capabilities, Components, npm publication, or multi-engine conformance. Existing private Component v3-v10 hosted evidence remains unchanged and separately bounded. General source/algebraic Component selection, resource/non-Copy carriers, imports/capabilities, callbacks/async, callable/FFI aggregate signatures, browser/multi-engine conformance, and stable public Component API/ABI remain open |
...
| Meaning in, verified machine code out | Partial | Typed scalar core, effect checks, native status/out contracts and checked arithmetic with exact normalized failure codes, poison-preserving result publication, and a host-native scalar executable. The locally evidenced IEEE-754 tranche adds `f32`/`f64` Copy scalars end-to-end — total float arithmetic without failure statuses, bit-exact finite literals, layouts, graph nodes, native C11 O0/O2, and Node/Wasm equivalence (`tests/floating_point_scalars.rs`, `examples/floats.spx`). The locally evidenced Unicode-scalar tranche adds `char` end-to-end — exact canonical escapes, scalar ordering, 4-byte layouts, graph nodes, native C11 O0/O2, and Node/Wasm equivalence (`tests/character_scalars.rs`, `examples/chars.spx`). The locally evidenced checked-integer tranche adds `i32` end-to-end — explicit suffixed literals (`SPX-P003` for range/suffix errors; unsuffixed literals stay `i64`), width-preserving checked `+`/`-`/`*`/`/` selecting the exact `i64` failure-status codes with `%` still `i64`-only (`SPX-T208`), 4-byte layouts, `"kind":"int32"` graph nodes, native C11 O0/O2 including an overflow status probe, and Node/Wasm equivalence (`tests/i32_scalars.rs`, `examples/integers_i32.spx`) — while generic arguments, the scalar-export profile, the native host corpus, the Native Rust interop boundary, and string/heap types remain explicitly closed; no status change | All safe-language guarantees survive every backend; native artifacts and portable components pass conformance suites on every supported target |
| Meaning in, verified machine code out | Partial | Typed scalar core, effect checks, native status/out contracts and checked arithmetic with exact normalized failure codes, poison-preserving result publication, and a host-native scalar executable. The locally evidenced IEEE-754 tranche adds `f32`/`f64` Copy scalars end-to-end — total float arithmetic without failure statuses, bit-exact finite literals, layouts, graph nodes, native C11 O0/O2, and Node/Wasm equivalence (`tests/floating_point_scalars.rs`, `examples/floats.spx`). The locally evidenced Unicode-scalar tranche adds `char` end-to-end — exact canonical escapes, scalar ordering, 4-byte layouts, graph nodes, native C11 O0/O2, and Node/Wasm equivalence (`tests/character_scalars.rs`, `examples/chars.spx`). The locally evidenced unsigned-byte tranche adds `u8` end-to-end — exact suffixed literals, checked 0..=255 arithmetic with the normalized arithmetic failure statuses, 1-byte Native64 layouts, graph nodes, native C11 O0/O2, and Node/Wasm equivalence (`tests/u8_scalars.rs`, `examples/bytes_u8.spx`) — while generic arguments, the scalar-export profile, the native host corpus, and string/heap types remain explicitly closed; no status change | All safe-language guarantees survive every backend; native artifacts and portable components pass conformance suites on every supported target |
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Fast development lane | Partial | [Reference Interpreter v1](INTERPRETER-V1.md) adds the read-only `semaprax interpret` evaluation lane: one explicit-ID monomorphic effect-free scalar function per invocation is evaluated directly from verified HIR (no backend toolchain) under a closed admission profile, emitting a deterministic digest-authenticated `semaprax.interpret.v1` envelope whose outcome is the returned value or the exact compiler-owned normalized failure status, with fuel accounting (steps used versus budget), argument echo, and source digest; a 28-row corpus proves byte-identical result/status transcripts against native C11 O0/O2 and Node/Wasm for the same inputs, and pinned KATs, fuel-exhaustion fail-closed behavior, the call-depth ceiling, determinism, tamper rejection, drift binding, and CLI exit codes are green locally in `tests/interpreter_v1.rs`; no JIT/AOT/Cranelift, incremental persistence, hot reload, debugger mapping, or target execution is claimed | Cranelift JIT/AOT, incremental affected-node builds, hot reload, and debugger mapping verified |
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Interface-first packages and target matrices | Partial | [Interface Package Report v1](PACKAGE-REPORT-V1.md) adds the read-only `semaprax package-report <file> [--max-bytes N]` projection: one deterministic canonical `semaprax.package-report.v1` envelope per verified module with the sorted admitted export inventory (explicit-ID monomorphic effect-free by-value `i64`/`bool` functions; per export the interface parameter/result types, canonically rendered requires/ensures clauses, declared effects, persistent stable ID, and the exact Native64 prototype line extracted verbatim from the production native projection under its own domain-separated digest), six closed exclusion reasons covering every non-admitted function, a fixed target availability matrix of exactly `native64` and `wasm32` available for this profile, and a closed ten-entry unavailable-capability inventory. Domain-separated SHA-256 digests authenticate the payload, source snapshot, and every export signature; `verify_envelope` independently replays counts, both closed sections, exclusion vocabulary, export ordering, and every signature digest, so forged-but-re-signed mutations of targets, capabilities, counts, or reasons still fail closed (`SPX-P301`-`SPX-P303`). Pinned envelope KATs, determinism, budget fail-closed behavior, per-digest-field tamper rejection, CLI exit codes, and cross-consistency proving the listed exports equal what `semaprax abi-report` admits (byte-equal native prototypes) and what `semaprax openapi` publishes are green locally in `tests/package_report_v1.rs`. No resolver, lockfile, dependency model, package registry, version-compatibility engine, conformance tests, provenance, signatures, licenses, or SBOM are provided, Project Manifest v1 deliberately retains none of them, and hosted promotion is not claimed | Resolver, lockfile, compatibility, implementations, capabilities, conformance tests, provenance, signatures, licenses, SBOM, and reproducibility verified |
...
| JavaScript and TypeScript | Partial | Public Wasm Scalar Exports v1 generates a frozen stable-ID keyed ES-module facade and exact TypeScript declarations for 1–32 selected monomorphic effect-free `i64`/`bool` functions. It binds signed-range `bigint`/canonical `boolean` conversions, exact arithmetic/contract status results, stable raw symbols, digest-authenticated Wasm, a canonical artifact manifest, Node execution, and stable-ID rename preservation. Exact TypeScript 5.8.3 independently compiles the real generated consumer and the Chromium runtime gate is hosted green at `d883ace579bfd86f723cdc6819224fde51f0677d` in [run 32523952912, job 96901973072](https://github.com/wavect/semaprax/actions/runs/32523952912/job/96901973072). The generated package is local and unpublished, and the profile remains scalar-only | Promise/error/typed-array/callback/resource mapping, additional representative browser hosts, package/version compatibility, and Component transpilation verified |

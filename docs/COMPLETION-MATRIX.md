...
[RFC 0003](RFC-0003-CLEANUP-AND-RESOURCE-ABI.md) phases 1–2 are implemented. Every resolved function carries an independently rebuilt target-neutral cleanup CFG with storage/leaf liveness, regions, atomic call commits, sticky failures, guarded finalization, and result publication; Graph v6 serializes it. Phase-3 groundwork includes status/trace types, independent plan replay, a reference executor, native scalar status/out execution, a private [host ownership model](HOST-OWNERSHIP-TRANSACTIONS-V1.md), deterministic authority-free host templates, host-guarded [pointer-free descriptors](NATIVE-ADAPTER-DESCRIPTOR-V1.md) with real shared-library/export tests, private [authenticated capability mechanics](NATIVE-CAPABILITY-TOKENS-V1.md) with an OS-entropy/thread-bound but ledger-disconnected authority, a test-only fake-backed exact-instance lifetime topology, a disconnected unpublished [native loader quarantine](NATIVE-MODULE-LOADER.md) with Linux/macOS fixtures, and exact typed-resource backend-oracle traces. Descriptors are discarded evidence; neither loader nor authority is connected to a production adapter, ledger, owner acquisition, finalizer, or callable export. Public native/Wasm resource hosting, cross-platform hardened adapter retention, Wasm status/out execution, callable imports, and cross-target conformance remain unimplemented and fail closed.

...
| Stable external API survives the display rename | Prior-head Ubuntu evidenced | Post-rename `app.wasm`, runtime and binding files, parsed scalar ABI, and exact Rust export rows preserve all six stable IDs while Project/workspace/source/subject revisions change. Node executes the renamed carrier and optional Rust executes both revisions; the interactive shell itself is not compared or run after rename. |
...
| Full promotion CI for every public claim | Pending | Latest main [run 32890205384](https://github.com/wavect/semaprax/actions/runs/32890205384) at `09fd3e9` retains green Ubuntu Product/Chromium evidence but fails the baseline Rust lanes, blocking macOS Product Acceptance, and Android. Windows Product and full Public-SDK execution remain diagnostic or masked; the local archive/linker/reproducible-COFF hardening in this branch has no hosted evidence yet and therefore cannot promote the claim. |

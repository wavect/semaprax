...
| Project v9 flat owned record follow-on | Exact-tag hosted regression coverage plus local C11 provider-consumer execution; unpublished and unpromoted | Descriptor, retained evaluator, Wasm/npm, native provider, safe Rust settlement, Revision Store, and hostile cases ran where selected by the tagged suite; the descriptor-derived low-level C header compiles, links and executes with the actual provider at O0/O2 | Close remaining separately provisioned and cross-platform physical-consumer gaps and make an explicit v9 publication/promotion decision before dependent profiles. |
...
| Atomic agent changes | Partial; the locally exercised Universal Semantic Transaction v1 kernel binds one exact Canonical Semantic Workspace Revision and old display name, validates one explicit monomorphic non-main function rename through ProjectCandidate, and exact-replays deterministic authority-free artifacts. Universal Semantic Transaction Composition v1 locally exercises a deterministic structural diff, rebases that one operation over compatible unrelated drift, and explicitly orders two distinct-target sibling rename Candidates with exact replay and no writes. Its merge result deliberately is not a multi-operation v1 transaction. Universal Semantic Workflow CLI v1 adds a locally exercised compact `change preview rename-display-name` adapter that derives the old name from the authenticated Project and returns the exact result or evidence, but deliberately performs no commit; additive structural-diff/rebase/merge adapters have separate narrower evidence. This slice does not publish, admit comments/noncanonical bases, preserve arbitrary unrelated trivia, broaden beyond display rename, or establish a universal operation algebra. The separate candidate Git slice authors held executable/cwd launch, exact child ambient state and fail-stop process-group settlement without promoting its unexecuted cross-platform gate. | [Universal Semantic Transaction v1](UNIVERSAL-SEMANTIC-TRANSACTION-V1.md), [Universal Semantic Transaction Composition v1](UNIVERSAL-SEMANTIC-TRANSACTION-COMPOSITION-V1.md), [Universal Semantic Workflow CLI v1](UNIVERSAL-SEMANTIC-WORKFLOW-CLI-V1.md), [Patch Evidence v1](SEMANTIC-PATCH-EVIDENCE-V1.md), [Workspace Change v1](SEMANTIC-WORKSPACE-CHANGE-V1.md), [Candidate Git Publication v1](PROJECT-CANDIDATE-GIT-PUBLICATION-V1.md) | General supported single- and multi-file semantic changes replay and publish atomically with recovery and provenance; preserve comments and exact unrelated trivia, broaden typed operations and preconditions, execute the Git substitution, descriptor, deadline and descendant-settlement matrix, and close same-principal repository-content hostility. |
...
| Structured concurrency | Partial; the proof model now has a bounded Rust scoped-thread runtime with borrowed captures, stable-ID starts/reports, cooperative cancellation, panic normalization, and mandatory join-before-return | [Scoped Task Model v1](SCOPED-TASKS-V1.md), [Structured Tasks Runtime v1](STRUCTURED-TASKS-RUNTIME-V1.md) | Language syntax, `Sendable`/`Shareable` checking, dependency scheduling, deterministic replay, native/Wasm lowering, and target execution are verified. |
...
| Edge and server | Partial; the explicit Rust host now has authenticated TLS clients and servers, bind/accept/settlement, deterministic fixture-v2 accept queues, and a bounded reusable HTTPS GET client with HTTP/1.1/2 negotiation, redirects, pooling, and typed responses; npm/browser remains fixture-v1 only | [Bounded Language Network I/O](BOUNDED-LANGUAGE-NETWORK-IO-V1.md), [Bounded Network Services v1](BOUNDED-NETWORK-SERVICES-V1.md), [HTTPS Client Runtime v1](HTTPS-CLIENT-RUNTIME-V1.md) | Source-level HTTP and server-TLS APIs, native-C11/Wasm/npm/browser service adapters, HTTP/3, DNS policy, structured async services, observability, deployment, and load/conformance tests are verified. |

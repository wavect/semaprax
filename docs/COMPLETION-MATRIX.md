...
| Same verified logic on native and browser targets | Locally evidenced | The linked entry/test closures run through the interpreter, native C11 at O0/O2, and Core Wasm; the new dual browser fixture awaits exact-head hosted execution. |
| Several stable-ID functions callable from TypeScript and Rust | Locally evidenced | All six `i64`/`bool` calculator exports are present in generated JS/TS and exact Rust manifest rows; strict TypeScript and compiler-free Rust consumers exercise success and failure results. |
| Calculator application consumes the Project functions | Locally evidenced | One committed interactive shell authenticates and consumes both the direct `semaprax.web.v4` package and the multi-module `semaprax.web-project.v1` package through the same six stable-ID calls; hosted Chromium evidence for the Project package is pending. |
| `semapraxd` inspect/derive/rename/impact/apply/rebuild loop | Locally evidenced | Opt-in Transport v4 retains the authenticated Project, derives the bounded rename, produces candidate-bound Impact/Review, applies through A0, reloads, and returns an independently replayed pathless Web build. |
| Stable external API survives the display rename | Locally evidenced | Post-rename Web app/bindings plus parsed scalar ABI and exact Rust export rows preserve the six stable IDs while Project/workspace/source/subject revisions change. |
| Project tests prove native/Wasm equivalence | Locally evidenced | The unified runner executes the manifest entry and test closure and compares interpreter, native O0/O2, Core-Wasm, Node, TypeScript, and Rust behavior across the admitted calculator cases. |
| Full promotion CI for every public claim | Pending | The corrected Windows one-way SDK settlement and blocking nested-publication regression plus the dual Project-browser fixture require a new exact-head run. Product/Public-SDK Windows steps remain diagnostic until that evidence is observed and promoted. |

The v0.2 aggregate exit therefore remains **Partial**; seven implementation
criteria are locally evidenced, but the mandatory promotion criterion is not.
...
| Web | Partial | Existing scalar HTML/ES/Wasm and Project-v1 hosted evidence remains unchanged. The committed interactive calculator now locally runs the direct `semaprax.web.v4` and linked multi-module `semaprax.web-project.v1` packages through the same six stable-ID calls on isolated loopback roots; exact dual-schema/export authentication and strict TypeScript checks precede one serial Chromium lane. Project v2 locally adds the config-validator Useful Text package. Project v3 locally adds profile-exact `Slice<u8>` exports, fixed-memory Wasm, strict snapshotting `Uint8Array` JS/TS, exact v2 npm carrier replay, and offline installed binary-frame execution while preserving v1/v2 bytes. Project v4 adds the fixed Node `spxgrep` adapter; v5 preserves those Wasm/Node semantics and binds its exact adapter authority/metadata through the v4 npm carrier while adding native parity. Project v6 locally adds strict argv/stdin snapshots, private dual-channel Core-Wasm staging including authenticated owned-token copying while live, combined output bounds, arena settlement, and semantic-carrier replay without WASI/console/process imports. The dual calculator fixture still awaits exact-head hosted execution and makes no registry, multi-engine, or general-browser claim; Windows publication stays fail-closed and full promotion is pending. | DOM/CSS application output, accessible UI, SSR/hydration, general collection and Wasm resource/Component support, capabilities, additional browser engines, safe Windows v2 publication, Canvas/WebGPU escape hatch, registry publication, and deployable sample verified |

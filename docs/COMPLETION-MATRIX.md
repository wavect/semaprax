...
| Web and portable components | Partial | Scalar Core Wasm, public scalar/nested record and bounded generic/prelude/typed-`?` copy-variant Core Wasm, narrow owned-resource Wasm, and private WIT/component evidence exist. The prelude-bound Portable Result Component v3 KAT/runner is hosted green in [run 31347109201, job 93330959212](https://github.com/wavect/semaprax/actions/runs/31347109201/job/93330959212). Private Source-Result Component v4 locally connects one exact effect-free source `Result`/`?` closure to WIT `result<result<bool, bool>, status>` with deterministic core/component KATs, independent/upstream validation, hostile-profile closure, and a configured typed Wasmtime runner; hosted v4 execution is pending. General `Result`/`Option`/`?` mapping, records/resources/imports/capabilities, callable/FFI aggregate signatures, browser/multi-engine conformance, and public API/ABI remain open |
...
The separate private Source-Result Component v4 locally connects one exact
...
consumer hiding, and CI source locks are green locally. The isolated runner is
configured for typed inner `Ok`/`Err`, outer status, residual short-circuit,
sticky failure, shared postconditions, re-entry, fresh instances, and fuel
failure, but hosted v4 execution is pending. General source `Result`/`Option`/`?`
...
| WebAssembly core/components | Partial | Validated stable-ID HIR lowers to direct Wasm core with browser ES runtime, checked arithmetic, contracts, HTML, `semaprax.web.v3`, public nested scalar record execution, bounded generic/prelude copy variants, and hosted bounded typed-`Result` propagation under Node with complete poison, exact concrete layouts, invalid-tag closure, shared postconditions, and restored shadow-stack state in [run 31353051690](https://github.com/wavect/semaprax/actions/runs/31353051690). A Node-executed `semaprax.wasm-owned.v1` subset covers one direct trivial resource, including same-realm duplicated-host isolation and exact semantic ordinal/reference equality for the shared 14-case corpus. Private component v1 provides an exact scalar fixture; checked v2 composes the unmodified generated scalar core with a frozen checked runtime. Portable Result Component v3 privately lifts its distinct checked two-scalar status/out core as `result<s64, status>`; its prelude-bound KAT/runner is hosted green in [run 31347109201, job 93330959212](https://github.com/wavect/semaprax/actions/runs/31347109201/job/93330959212). Private Source-Result Component v4 locally lifts one exact source `Result`/`?` closure as `result<result<bool, bool>, status>` with exact KATs, independent/upstream validation, generated-core execution, zero imports, and a configured isolated typed runner; hosted v4 execution is pending | Browser/WASI modules, general source-language `Result`/`Option`/`?` Component mapping, general/resource-bearing records or variants, imports, async/capabilities, multi-engine conformance, general canonical resource ABI, stable public aggregate ABI, callable/FFI aggregate signatures, sandboxing, cross-realm/worker identity, public component API, and production native-host/Wasm conformance verified |
...
| WIT and WebAssembly Components | Partial | Deterministic private `SPXWIT01` WIT/schema/JavaScript bundle with digest KAT, mutation closure, snapshot-only hostile-object normalization, lossless UTF-8/exact status bounds, and Node execution; standalone component v1; checked component v2; and private Portable Result Component v3 with exact `result<s64, status>` composition, independent/upstream validation, typed Wasmtime outcomes, zero imports/empty linker/no WASI, and a standalone locked Rust 1.97.1/Wasmtime dependency graph isolated from the compiler MSRV graph. The prelude-bound v3 KAT/runner is hosted green in [run 31347109201, job 93330959212](https://github.com/wavect/semaprax/actions/runs/31347109201/job/93330959212) and again in [run 31353051690, job 93347328905](https://github.com/wavect/semaprax/actions/runs/31353051690/job/93347328905). Private Source-Result Component v4 locally binds one exact validated `Result`/`?` closure to WIT 0.2 `result<result<bool, bool>, status>`, with frozen source/core/profile/component/layout KATs, independent/upstream validation, mutation/cross-version closure, default-consumer hiding, and a configured zero-import typed runner; hosted v4 execution is pending | General source `Result`/`Option`/`?` mapping, general imports/exports, records/resources, futures/streams, capabilities, browser and multi-engine conformance, multi-language composition, callable/FFI aggregate signatures, public API/ABI, and `SPX-B104` admission verified |

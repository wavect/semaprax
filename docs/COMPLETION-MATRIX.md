...
| Editor integration by meaning | Partial; the VS Code adapter has diagnostics on save, go-to-declaration by stable identity, callers from the persistent call index, a module documentation view, identity/effect/contract code lenses, an ownership/contract/effect view over the bounded `context` facets, and agent-definition inspection over the compiler's `query --json`, `doc`, `context`, and `agent inspect` routes, plus the saved-source session's typed holes, compiler-admitted repairs, and candidate review; safe rename, cleanup-plan views, and runtime trace/evidence views are not admitted | [VS Code adapter](VSCODE-SAVED-SOURCE-ADAPTER-V1.md), [Unified CLI v1](UNIFIED-CLI-V1.md) | Drive the remaining editor surfaces from the same catalog and prove them in a real Extension Host run. |

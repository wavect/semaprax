...
are green. Its pinned Rust 1.97.1/Wasmtime 47 typed runtime is configured, but
hosted execution remains pending and is not credited green. V1-v6 bytes remain
...
| WebAssembly core/components | Partial | Validated stable-ID HIR lowers to direct Wasm core with browser ES runtime, checked arithmetic, contracts, HTML, `semaprax.web.v3`, public nested scalar and exact-instance generic Copy records, bounded irrefutable Copy-record patterns, bounded generic/prelude copy variants, and hosted bounded typed-`Result` and typed-`Option` propagation with complete poison, exact concrete layouts, invalid-tag closure, shared postconditions, and restored shadow-stack state. Generic records are hosted green through 4,096-call Node re-entry in [run 31365363898, Ubuntu job 93383304995](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304995); record-pattern Node/Wasm 4,096-entry evidence is locally green and security-clean. A Node-executed `semaprax.wasm-owned.v1` subset covers one direct trivial resource. Private components v1-v6 retain their exact evidence; private Generic Record Component v7 has local authenticated KAT/hostile/source-lock/security evidence, while pinned Rust 1.97.1/Wasmtime 47 hosted execution remains pending and is not credited green | Browser/WASI modules, general source-language algebraic Component mapping, general/resource-bearing records or variants, refutable/non-Copy patterns, imports, async/capabilities, multi-engine conformance, general canonical resource ABI, stable public aggregate ABI, callable/FFI aggregate signatures, sandboxing, cross-realm/worker identity, public component API/ABI, and production native-host/Wasm conformance verified |

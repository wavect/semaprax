...
| --- | --- | --- |
| Human and agent semantic projections | Partial | Canonical `.spx`, validated stable-ID HIR, atomic single-file renames, and Graph v7 including immutable record update are executable; multi-file typed repair/impact remains open |
...
| Desktop and mobile applications | Partial | Private macOS engine/AppKit ([job 93309086230](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086230)), Swift/iOS XCFramework/app ([job 93309086228](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086228)), and Android JNI/Kotlin app ([job 93309086206](https://github.com/wavect/semaprax/actions/runs/31338834586/job/93309086206)) gates are green; Windows remains pending after [failed run 31340893685, job 93314358662](https://github.com/wavect/semaprax/actions/runs/31340893685/job/93314358662), and public SDKs, UI language, lifecycle breadth, signing/distribution, and device breadth remain open |
...
v7 serializes it. Phase-3 evidence now includes status/trace types, independent
...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Records and algebraic variants | Partial | Canonical record declarations/construction/projection/immutable update; stable field IDs; deterministic field diagnostics; recursive facts/cycle rejection; prefix-aware partial-place ownership; checked deterministic Native64/Wasm32 layouts for nested `i64`/`bool`/direct-trivial-resource records with frozen one-byte/alignment-one empty records; independently replayed partial-initialization and update cleanup; validated record HIR; Graph v7; public nested `i64`/`bool` native C11 O0/O2 and Node/Wasm execution with base-first/authored-order status/out semantics; and one private shared-plan resource harness with an exact common cleanup trace and zero liveness | Public resource-bearing record execution/admission, callable/component aggregate signatures, variants, generics, exhaustive matching, stable public aggregate ABIs, general native/Wasm aggregate execution and aggregate-result trace equivalence, `Option`, `Result`, and `?` verified; ordinary `SPX-B104`/`SPX-W111` gates remain closed |

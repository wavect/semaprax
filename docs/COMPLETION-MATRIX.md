...
| Web | Partial | Existing scalar HTML/ES/Wasm and Project-v1 hosted evidence remains unchanged. Project v2 locally adds the config-validator Useful Text package. Project v3 locally adds profile-exact `Slice<u8>` exports, fixed-memory Wasm, strict snapshotting `Uint8Array` JS/TS, exact v2 npm carrier replay, and offline installed binary-frame execution while preserving v1/v2 bytes. Project v4 adds the fixed Node `spxgrep` adapter; v5 preserves those Wasm/Node semantics and binds its exact adapter authority/metadata through the v4 npm carrier while adding native parity. Project v6 locally adds strict argv/stdin snapshots, private dual-channel Core-Wasm staging including authenticated owned-token copying while live, combined output bounds, arena settlement, and semantic-carrier replay without WASI/console/process imports. It makes no registry or general-browser claim; Windows publication stays fail-closed and exact-head hosted promotion is pending. | DOM/CSS application output, accessible UI, SSR/hydration, general collection and Wasm resource/Component support, capabilities, additional browser engines, safe Windows v2 publication, Canvas/WebGPU escape hatch, registry publication, and deployable sample verified |

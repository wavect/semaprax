...
| Meaning in, verified machine code out | Partial | The locally evidenced `u8` scalar retains checked 0..=255 arithmetic, one-byte Native64 layout, Graph, native O0/O2, and Node/Wasm equivalence. The separate Useful Text Consumer v1 `str` is a borrowed UTF-8 pointer/length view, not a byte collection: it cannot escape or be indexed/iterated, and only byte-length/empty/prefix/contains are admitted. V1 projections remain byte-preserved; exact-head hosted promotion and general text/collection semantics remain open. | All safe-language guarantees survive every backend; native artifacts and portable components pass conformance suites on every supported target |

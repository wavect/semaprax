...
| Edge and server | Partial | Host-native scalar CLI plus locally evidenced fixed native and Node `spxgrep` command adapters over the success-published bounded transcript. Project v5 binds their one-UTF-8-argument/binary-stdin cumulative bound and exact 0/1/2 exits. This is not a server, general I/O runtime, or atomic physical-stdout claim. | Server runtime, async I/O, HTTP/data adapters, general native/WASI output, observability, deployment, and load/conformance tests verified |

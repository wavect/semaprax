...
| Requirement | Status | Current evidence | Completion gate |
| --- | --- | --- | --- |
| Interface-first packages and target matrices | Partial | Project v2/v3 retain their exact Useful Text/Useful Data packages and v1/v2 carriers. Project v4 locally adds the exact seven-file command package and context-bound `semaprax.project-npm-build.v3`, with independent replay/tamper rejection and compiler-free `spxgrep` execution. Opaque prepared builds retain trusted Project facts; context-free inspection proves consistency only. Unix publication is handle-relative/create-new; Windows publication remains fail-closed. Registry publication and hosted promotion are not claimed. | Resolver, lockfile, compatibility, implementations, general capabilities, safe Windows publication, conformance matrices, registry publication, provenance, signatures, licenses, SBOM, and reproducibility verified |
...
| Edge and server | Partial | Host-native scalar CLI plus a locally evidenced fixed Node `spxgrep` command adapter over the success-published bounded transcript. This is not a server or general I/O runtime. | Server runtime, async I/O, HTTP/data adapters, general native/WASI output, observability, deployment, and load/conformance tests verified |

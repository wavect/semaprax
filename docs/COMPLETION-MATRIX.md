...
| WP-05 `doctor` | Exact-tag hosted regression coverage; a signed Linux provisioner and role-specific worker policies are authored/unrun; ordinary production profiles remain unavailable | [Explicit bounded offline-profile selection](DOCTOR-PROBE-V1.md), sealed-input/bundle parsers, injected-host/version checks, and lower-level settlement tests ran where selected by CI. [Linux Production Provisioner v1](DOCTOR-PRODUCTION-PROVISIONER-V1.md) now authors exact release-capsule admission, private namespaces, cgroup limits, held static images, and whole-cgroup settlement without claiming execution evidence. | Execute the unpacked signed Linux distribution with real tools and hostile authority/settlement cases, integrate an explicit support handoff, and complete equivalent tool/input, filesystem/broker, network and descendant closure on macOS/Windows; structural/parser/unit evidence does not establish production confinement. |
...
block describing the bounded owned-data API as supported or published.
...
| Requirement | Status | Evidence owner | Complete when |
| --- | --- | --- | --- |
| Records and algebraic variants | Partial | [RFC 0002](RFC-0002-ALGEBRAIC-DATA.md), [Owned Byte Record Algebra](OWNED-BYTE-RECORD-ALGEBRA-V1.md), [Acyclic Nested Owned-Byte Records](NESTED-OWNED-BYTE-RECORDS-V1.md), [Owned Byte Variant Algebra](OWNED-BYTE-VARIANT-ALGEBRA-V1.md) | Execute and promote the bounded nested owned-record corpus, then verify general nested/generic/resource aggregates, matching, layout, cleanup, and public ABIs. |

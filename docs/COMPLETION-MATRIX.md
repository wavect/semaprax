...
| Edge and server | Partial; [Bounded Language Network I/O v1](BOUNDED-LANGUAGE-NETWORK-IO-V1.md) adds six effect-gated TCP client operations, deterministic fixtures, Project v12 native/npm/Web build lanes, and `network-run --fixture`; HTTP/1.1 is composed from them by `std.http` helpers, while npm/browser execution remains fixture-only | [Bounded Language Command I/O](BOUNDED-LANGUAGE-COMMAND-IO-V1.md), [Bounded Language Network I/O](BOUNDED-LANGUAGE-NETWORK-IO-V1.md) | General I/O, TLS, listen sockets, structured async services, typed HTTP/data adapters, observability, deployment, and load/conformance tests are verified. |

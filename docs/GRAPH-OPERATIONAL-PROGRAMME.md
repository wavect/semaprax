...
| Typed expression/declaration constructors | Partial, authored/unrun. Candidate constructors cover bounded scalar/parameter/operator/call expressions, stable-ID record and variant construction with explicit direct-scalar generic arguments, authenticated Option/Result cases, record-field value projection, exhaustive variant value matching with exact-owner typed staging and arm-local payload bindings, and monomorphic function declarations with limited ownership modes. Catalogue/hole discovery exposes bindings, checked templates, field/case owners and compiler provenance; full candidate replay owns admission. Nested/named generic arguments, borrow-preserving projections, record updates, general/ownership-aware patterns, general declarations and expected-type/effect/ownership-guided discovery remain missing. |

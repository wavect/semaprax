...
| Typed expression/declaration constructors | Partial, authored/unrun. Candidate constructors cover bounded scalar/parameter/operator/call expressions, stable-ID monomorphic record and variant construction with checked field inventories, and monomorphic function declarations with limited ownership modes. Catalogue/hole discovery exposes available aggregate bindings; full candidate replay owns admission. Generic aggregate construction, projection/update/match synthesis, general declarations and expected-type/effect/ownership-guided discovery remain missing. |

...
| TypeScript, Python, Rust clients | V5 authors typed TypeScript/Python/Rust request parameters, enum choices, builders and result decoders from runtime-selected catalogues. Additive complete parameter types describe recursive intentions, expressions and recovery objects from compiler-owned schemas; legacy builders and server admission remain unchanged. Typed response helpers expose concrete bundled handles, chunks and other payload shapes through a common model while retaining runtime validators and generic APIs. Heterogeneous compiler-report schemas and encoded chunk interiors remain explicitly opaque. Independently executed cross-language validation, complete typed payloads and client ergonomics remain open. |

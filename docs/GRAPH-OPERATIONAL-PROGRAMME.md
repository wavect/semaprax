...
| Incremental invalidation/rechecking | Partial authored/unrun. The AST-only cache preserves its behavior; a separate semantic opt-in retains compiler-created checked module HIR keyed by exact synthetic AST, imported stubs and manifest context. Changed providers invalidate reverse import dependents. Eligible hits skip source resolution, but HIR validation, cross-file checks, linking, graph and profile admission rerun. Owned-source ImageWorkspace and authenticated v5 refresh share transactional cache adoption; preview/failed work is discarded. Warm cross-process HIR, function-level reuse, target-work reuse and executed equivalence/performance evidence remain missing. |

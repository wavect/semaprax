...
| Targets | Partial, authored/unrun. V5 `image/target-admission` derives actual native-C11/Core-Wasm emission and structural validation facts for complete entry/test closures and checks selected-function membership. The caller-owned [Target Cache v1](PROJECT-TARGET-CACHE-V1.md) retains and independently replays exact scalar-Web and pathless native-C11 carriers without weakening Project admission; the C lane rechecks embedded header envelopes and invokes no emitter on a hit. Whole-closure failure is not blamed on one function. Runtime execution, persistent/cross-process reuse, general declaration-level diagnosis, broader targets and package-profile coverage remain missing. |

...
| TypeScript, Python, Rust clients | V5 authors typed TypeScript/Python/Rust outer request parameters, enum choices, builders and result decoders from runtime-selected catalogues. Additive typed response helpers expose concrete bundled handles, chunks and other payload shapes through a common type model while retaining the existing runtime validators and generic APIs. Nested constructor objects stay server-validated; heterogeneous compiler-report schemas and encoded chunk interiors remain explicitly opaque. Independently executed cross-language validation, complete typed payloads and client ergonomics remain open. |

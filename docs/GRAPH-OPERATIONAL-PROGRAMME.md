...
| --- | --- |
| `rename_declaration` | Partial, authored/unrun. Candidate supports explicit monomorphic non-main functions and source record/variant display renames through the shared authenticated Operations occurrence collector, preserving stable identities and consumer aliases. Generic/owned nominal shapes retain ordinary source/profile admission; unsupported reference pairs fail closed. Catalogue discovery, nominal conflict guards and conservative test relevance are included. Field/case/interface and other declaration renames, broader reference forms, complete merge normalization and executed evidence remain open. |

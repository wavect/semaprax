...
| Typed responses | [Typed Response Clients v1](IMAGE-TYPED-RESPONSE-CLIENTS-V1.md); additive method-specific payload/result types and decoders over already bundled response schemas, authored/unrun |
| Typed requests | [Typed Request Clients v1](IMAGE-TYPED-REQUEST-CLIENTS-V1.md); additive complete parameter types, recursive constructor shapes and builders over selected request schemas, authored/unrun; compiler admission unchanged |
...
| Editor typed holes | [VS Code Adapter v1](VSCODE-SAVED-SOURCE-ADAPTER-V1.md#typed-hole-workflow); three-kind hole planning, compact facets, bound fill scratches and explicit completion, authored/unrun; editor-host execution remains open |
...
| Typed expression/declaration constructors | Partial, authored/unrun. Candidate constructors cover bounded scalar/parameter/operator/call expressions, stable-ID record and variant construction with explicit direct-scalar generic arguments, authenticated Option/Result cases, record-field value projection, direct field places with authenticated nominal roots, subset record updates with base-first evaluation, exhaustive variant value matching with exact-owner typed staging and arm-local payload bindings, monomorphic function declarations with limited ownership modes and stable-ID Copy nominal parameters/returns, and explicit monomorphic record/variant declaration creation with scalar fields. Field places compose with compiler-owned byte calls under the existing direct owned-field borrow profile, without staging a root temporary. Catalogue/hole discovery exposes bindings, checked templates, field/case owners and compiler provenance; full candidate replay, exact added identity inventories and checked signature facts own admission. Nested/named generic arguments, general nested borrowing, general/ownership-aware patterns, generic/owned type creation, general declarations and expected-type/effect/ownership-guided discovery remain missing. |
...
| `add_record_field` | Partial, authored/unrun. Appends one inert i64/bool/i32/u8/usize field to an eligible checked Copy or flat owned-byte record, migrates constructors and exact patterns, preserves old field identities and owning bindings, and revalidates complete Project/layout/loan/cleanup/target admission. Generic/class/variant targets, new owning fields, broader owned shapes and general record evolution remain open. |
...
| Packages | Partial, authored/unrun [Package Semantic Graph](PACKAGE-SEMANTIC-GRAPH-V1.md). Exact source-capsule replay supplies coordinate-qualified interface, import and cross-package call relationships with revision-bound consumer queries. Explicit host attachment exposes a separate immutable package subject, never an inferred Project dependency. General package profiles, installed-consumer discovery, Project/package association and cross-package source migration remain missing. |
| Analysis boundaries | Partial, authored/unrun [Analysis Coverage](SEMANTIC-IMAGE-ANALYSIS-COVERAGE-V1.md): exact retained input and declared interface-import facts alongside explicit deployment, generated provenance, external API behavior, runtime and consumer blind spots. No external-input ingestion, conformance proof or completeness percentage. |
...
| TypeScript, Python, Rust clients | V5 authors typed TypeScript/Python/Rust request parameters, enum choices, builders and result decoders from runtime-selected catalogues. Complete parameter types describe recursive intentions, expressions and recovery objects from compiler-owned schemas; legacy builders and server admission remain unchanged. Typed response helpers now support guarded recursion and a closed repair catalogue, including actual recursive body-change forms. Document-local definitions retain scope, runtime assertions fail closed, and validation work is shared across alternative trials. Other heterogeneous compiler reports and encoded chunk interiors remain explicitly opaque. Independently executed cross-language validation, complete typed payloads and client ergonomics remain open. |
| Optional MCP/editor adapters | Partial, authored/unrun MCP stdio adapter pinned to 2025-11-25 and saved-source VS Code adapter. The server catalogue derives selected tools and self-contained input schemas from v5; calls retain exact revisions, startup-only approval and publication-aware final errors. The editor adds explicit startup, stable-ID change discovery, typed-intention submission and read-only source diffs over closed replayed review reports. Typed holes and separate diagnostic attempts support explicit fills and compiler repair selection; diagnostic inspection authenticates raw report bytes and submits no displayed change bodies. It excludes execution and publication commands and invalidates UI state on source changes. No independent MCP conformance suite or real editor-host integration has run. HTTP transport, richer constructor/hole/repair UX, asynchronous/cancelable scheduling and executed integration evidence remain missing. |
...
| Parallel read-only requests | Partial, authored/unrun. The embedding-host batch API uses at most four scoped workers for sixteen selected immutable image/discovery/candidate/draft/diagnostic reads, restores input order and authenticates held source before/after the entire joined batch. It detaches only request-selected subjects and shares pure sequential report handlers; no registry mutation, refresh, test execution, build or commit authority enters workers. Validation and source-only repair/archive replay keep their ordinary bounds, with no total CPU/RSS guarantee. StdIO remains sequential; general concurrent transport scheduling, cancellation and executed isolation/throughput evidence remain missing. |

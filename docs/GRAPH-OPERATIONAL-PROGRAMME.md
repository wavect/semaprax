...
| Typed expression/declaration constructors | Partial, authored/unrun. Candidate constructors cover bounded scalar/parameter/operator/call expressions, stable-ID record and variant construction with explicit direct-scalar generic arguments, authenticated Option/Result cases, record-field value projection through a hygienic exact-owner typed binding, and monomorphic function declarations with limited ownership modes. Catalogue/hole discovery exposes bindings, checked templates, field owners and compiler provenance; full candidate replay owns admission. Nested/named generic arguments, borrow-preserving projections, update/match synthesis, general declarations and expected-type/effect/ownership-guided discovery remain missing. |

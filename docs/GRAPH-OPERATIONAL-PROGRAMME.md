...
| Manual edits and stale recovery | Partial held-input absorbing invalidation, exact base rejection and library rebase onto a separately admitted revision. V5 reloads through cold or source-exact frontend reuse and preserves historical candidates; explicit startup archives also recover complete candidates after original source removal/edits without making them current. Drafts/attempts clear on refresh. Complete session recovery, checked-HIR/incremental semantic reload and measured recovery benchmarks remain open. |

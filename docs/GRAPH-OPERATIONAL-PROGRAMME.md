...
the ledger therefore distinguishes the two archived focused local executions
from unselected full-profile and hosted evidence. Neither focused result
...
| Typed requests | [Typed Request Clients v1](IMAGE-TYPED-REQUEST-CLIENTS-V1.md); exact local subject evidence executes Python compiler admission while all three languages retain deterministic recursive builders; independent TypeScript/Rust request admission remains open |
...
| MCP integration | [MCP Adapter v1](IMAGE-MCP-ADAPTER-V1.md), [Client/MCP Evidence v1](GRAPH-OPERATIONAL-CLIENT-MCP-EXECUTION-EVIDENCE-V1.md); exact local subject passes the in-process contract and actual `serve-workspace-mcp` stdio process including catalogue paging and notification nonexecution; independent MCP SDK conformance remains open |
...
| TypeScript, Python, Rust clients | V5 authors typed TypeScript/Python/Rust request parameters, enum choices, builders and result decoders from runtime-selected catalogues. Exact-subject local evidence now executes three Python harnesses, an offline compiled Rust recursive-response consumer and a provisioned strict TypeScript 5.8.3/Node consumer with hostile depth/work cases. All-language generation remains deterministic. Rust/TypeScript request admission, the full twelve-step workflow through generated clients, other heterogeneous compiler reports, encoded chunk interiors, complete typed payloads and ergonomics remain open. |
| Optional MCP/editor adapters | Partial. Exact-subject local evidence passes the pinned 2025-11-25 in-process MCP contract and launches the actual `semaprax serve-workspace-mcp` stdio child, drains its catalogue, forwards v5 calls and proves notification nonexecution. The client frames remain authored Rust JSON rather than an independent MCP SDK/conformance client. The saved-source VS Code adapter remains authored/unrun. HTTP, real editor-host integration, richer constructor/hole/repair UX, asynchronous/cancelable scheduling and cross-platform evidence remain missing. |

...
| Incremental invalidation/rechecking | Partial authored/unrun Frontend Cache. Source-exact retained canonical ASTs avoid parser/formatter calls for eligible unchanged modules; changed providers invalidate reverse import dependents. Direct owned-source image refresh uses this cache but still reruns all semantic checking/linking/profile admission. V5 adds explicit cold live-source refresh with expected-old/new bindings; it preserves immutable candidate handles and clears drafts/attempts after success. Checked-HIR reuse remains missing. |
...
| --- | --- |
| `protocol/capabilities`, `protocol/schemas` | Authored/unrun Protocol. Host-selected capabilities and catalogue-driven request/success/error envelopes. Additive `protocol/constructor-schemas` supplies self-contained closed typed-expression/intent/change schemas. V5 adds a runtime-selected schema bundle and typed client generation, while explicitly identifying opaque payload references. Complete response validators and executed client/schema compatibility remain missing. |
| `workspace/open`, `workspace/status`, `workspace/refresh-preview`, `workspace/refresh`, `query/catalog` | Authored/unrun. Host binds the manifest; requests cannot select a new path. V5 preview discovers the currently observed revision without replacing state; refresh cold-loads that same source set with exact expected image/new Project revisions, retains candidate handles and clears incomplete drafts/attempts only after successful bounded response preparation. Failed refresh preserves session state; incremental HIR reuse is not claimed. |
...
| Manual edits and stale recovery | Partial held-input absorbing invalidation, exact base rejection and library rebase onto a separately admitted revision. V5 explicitly reloads the fixed live manifest and preserves historical candidate handles for inspection/rebase; drafts and attempts are cleared on successful refresh. Complete session recovery, warm/incremental reload and recovery benchmarks remain open. |

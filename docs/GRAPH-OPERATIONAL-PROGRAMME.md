...
| Typed expression/declaration constructors | Partial, authored/unrun. Candidate constructors cover bounded scalar/parameter/operator/call expressions, stable-ID record and variant construction with explicit direct-scalar generic arguments, authenticated Option/Result cases, record-field value projection, subset record updates with base-first evaluation, exhaustive variant value matching with exact-owner typed staging and arm-local payload bindings, and monomorphic function declarations with limited ownership modes and stable-ID Copy nominal parameters/returns. Catalogue/hole discovery exposes bindings, checked templates, field/case owners and compiler provenance; full candidate replay and checked signature facts own admission. Nested/named generic arguments, borrow-preserving projections, general/ownership-aware patterns, general declarations and expected-type/effect/ownership-guided discovery remain missing. |
...
| `add_declaration` | Partial, authored/unrun. Typed monomorphic function append in an anchor's module with global identity/namespace/effect checks and exact invariant extension. Existing stable-ID Copy record/variant parameters and returns include direct-scalar local generic instances and authenticated prelude types, gated by rebuilt checked signature facts. General declaration kinds, new type creation, broader ownership modes, recursive construction and placement remain open. |
...
| Candidate branching | Partial in-memory immutable siblings and bounded candidate-only v2 registry. Complete histories now have authored/unrun self-contained source archives and a host-selected immutable content-addressed store. Startup policy can restore explicitly selected candidates; automatic durable branch registries and incomplete-draft recovery remain missing. |

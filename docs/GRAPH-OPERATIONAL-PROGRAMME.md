...
| Candidate branching | Partial in-memory immutable siblings and bounded candidate-only v2 registry. Complete histories have authored/unrun self-contained source archives and a host-selected immutable content-addressed store. Startup policy can restore explicitly selected candidates. Self-contained draft archives now reconstruct pending selectors and valid history after the original checkout changes or disappears, while preserving explicit completion and historical startup fences. Automatic durable branch registries, complete branch lifecycle recovery and executed evidence remain missing. |

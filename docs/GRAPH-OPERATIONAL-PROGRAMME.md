...
| Optional MCP/editor adapters | Partial, authored/unrun MCP stdio adapter pinned to 2025-11-25. The catalogue derives selected tools and self-contained input schemas from v5; calls retain exact revisions, startup-only approval and publication-aware final errors. A shared CLI startup loader preserves policy v1–v6. No independent MCP client or conformance suite has run. HTTP transport, editor adapters, asynchronous/cancelable scheduling and executed integration evidence remain missing. |

...
legacy helper preservation and nested request submission. Python, Rust, and provisioned TypeScript harnesses are authored to compile or resolve their generated public types, emit exact request frames, submit them to ordinary compiler admission, and require hostile unbound places to reject. The implemented expanded corpus has hosted-green release evidence. These selected request paths do not establish a complete SDK, every method, external package ergonomics, every possible host/platform combination, or completion-matrix promotion.

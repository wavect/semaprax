...
| `semaprax` inside a tag archive | The [v0.4.1 prerelease](https://github.com/wavect/semaprax/releases/tag/v0.4.1) archives | The archive's `semaprax` *is* the `semaprax-full` binary, renamed during staging, so archive users write `semaprax doctor`, not `semaprax-full doctor`. |
...
| Host | Archive |
| --- | --- |
| Linux x86-64 | `semaprax-v0.4.1-x86_64-unknown-linux-gnu.tar.gz` |
| Apple Silicon macOS | `semaprax-v0.4.1-aarch64-apple-darwin.tar.gz` |
| Windows x86-64 | `semaprax-v0.4.1-x86_64-pc-windows-msvc.zip` |
...
tar -xzf semaprax-v0.4.1-aarch64-apple-darwin.tar.gz
```
...
Expected shapes, from a local `0.4.1` standalone build:

```text
semaprax 0.4.1 (commit unknown)
```
...
```text
{"schema":"semaprax.version.v1","version":"0.4.1","commit":null,"maturity":"pre-alpha","rust_min":"1.88"}
```

...
| Host | Archive |
| --- | --- |
| Linux x86-64 | `semaprax-v0.4.0-x86_64-unknown-linux-gnu.tar.gz` |
| Apple Silicon macOS | `semaprax-v0.4.0-aarch64-apple-darwin.tar.gz` |
| Windows x86-64 | `semaprax-v0.4.0-x86_64-pc-windows-msvc.zip` |
...
tar -xzf semaprax-v0.4.0-aarch64-apple-darwin.tar.gz
```
...
Expected shapes, from a local `0.4.0` standalone build:

```text
semaprax 0.4.0 (commit unknown)
```
...
```text
{"schema":"semaprax.version.v1","version":"0.4.0","commit":null,"maturity":"pre-alpha","rust_min":"1.88"}
```

...
| `[dependencies]` | one `name = "range"` row per dependency | optional | Dependency requirements. Names are package names, strictly byte-sorted; ranges use only `=x.y.z`, `~x.y.z`, or `^x.y.z` with canonical `u32` components, the grammar of [Offline Semantic Lock v3](OFFLINE-SEMANTIC-PACKAGE-LOCK-V3.md). At most 64 rows. |

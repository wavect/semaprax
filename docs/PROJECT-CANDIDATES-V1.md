...
| `extract_function` | `target`, `expression_id`, `new_id`, `new_name` | Derive immutable Copy captures or one exact whole local `Bytes`/`String` owner and replace an authenticated expression with a new helper call; see [Extraction v1](PROJECT-EXTRACTION-V1.md) and [Owning Capture Extraction v1](PROJECT-OWNING-CAPTURE-EXTRACTION-V1.md). |
...
| `add_record_field` | `target`, `field` | Append a scalar field, or a bounded owning `string`/`Bytes` field to an eligible Copy record, and migrate authenticated constructors; see [Record Field Change v1](PROJECT-RECORD-FIELD-CHANGE-V1.md). |

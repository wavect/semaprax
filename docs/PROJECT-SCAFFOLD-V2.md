...
Version 2 of the authority-free calculator scaffold adds one file, `AGENTS.md`,
to the four files of [version 1](PROJECT-SCAFFOLD-V1.md). A project created by
either `new` route or materialized from the capsule now carries, next to its
source, the commands that check, test, run, format, and build it and the
language rules that differ from other languages. Coding agents read such a
...
```text
derive_project_scaffold(project_name, template)
replay_project_scaffold(project_name, template, canonical_capsule_bytes, digest)
```

The `_v1` suffixes of the v1 API are dropped; the returned `ProjectScaffold`
and its `ProjectScaffoldFile` entries expose the same facts as before. The
standalone `semaprax project-scaffold --name <name>` command prints the v2
capsule bytes to stdout exactly as v1 described, and `semaprax new` in both
executables writes the same five files.
...
- these five files in this order:
  1. `README.md`;
  2. `AGENTS.md`;
  3. `semaprax.toml`;
  4. `src/app.spx`;
...
`semaprax.toml`, `src/app.spx`, and `src/tests.spx` are byte-identical to v1.
`README.md` now shows the directory-operand forms (`semaprax check .`) and
points the reader at `AGENTS.md`. `AGENTS.md` substitutes the project name and
...
five files. Project v1 manifest semantics, source semantics, and every other
command are unchanged.

## Evidence

`tests/project/scaffold.rs` pins the v2 schema, digest domain, ordered
inventory, per-file digests, top-level digest, canonical byte length, and the
same rejection classes v1 required. `tests/project/scaffold_cli.rs`,

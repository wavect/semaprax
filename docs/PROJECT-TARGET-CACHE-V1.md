...
Status: Partial; exact scalar Web and pathless native-C11 target lanes and
focused regression sources are authored. No target execution, benchmark, test
suite, or completion gate was run.
...
carrier. It accepts only a private-constructor
...
`semaprax.project-native-c11-target-work.v1`, canonical manifest bytes, Project
...
On a miss, the ordinary `build_web_inline` or `build_c_inline` path emits the
selected target. On an exact hit, the cache skips only deterministic target
...
entry, and a miss in one lane does not evict the other.
...
execution are absent. npm, Wasm-package, non-scalar, cross-revision,
...
carrier tampering with recovery after restoring the exact entry. They were not
run. Before any performance claim, these lanes need

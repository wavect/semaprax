...
The separate private `semaprax.authenticated-native-moves.v1` profile admits
the same flat owned-`Bytes`, same-instance input/result boundary with a closed
checked movement body: places, fresh record construction, immutable local
bindings/blocks, and literal-boolean `if` expressions. Both branches must be
admitted, and requires/ensures remain literal booleans. The ordinary HIR
ownership verifier and native emitter, not a descriptor-driven permutation
implementation, own field moves, branch selection and cleanup. Generated C11
and C++17 callers bind the exact verified descriptor and sealed moves artifact;
they reuse canonical frame admission and the existing settlement machinery.
The identity-v1 constructor and generated bytes remain unchanged, and the
ordinary flattened prepare operation remains refused by both profiles.

The owning `generated_c_and_cxx_execute_checked_movement_bodies` selector runs
61 physical processes: both branch selections and requires true/false through
C11/C++17 at O0/O2, each with canonical, wrong-path and legacy-flat inputs
(48), plus four checked-call-to-identity-stub omission controls. Eight further
C11/C++17 O0/O2 processes run both movement branches with requires true and
ensures false, twice each, checking raw status 11/ExecutionFailed, consumed
input, absent output, endpoint count, immediate live-allocation baseline,
zero live handles and close-to-zero. One additional identity-stub control
must fail the exact postcondition-status oracle, rather than a payload oracle.
Non-palindromic
distinct leaves distinguish actual field moves from identity and reversal;
existing allocator/handle/close assertions check settlement. Allocating bodies,
including an allocation in the unselected branch, fail at generation. This is
not arbitrary-body or cross-backend parity: ordinary native fresh-`Bytes`
allocation aborts on allocation failure, and its internal 128 KiB value bound
exceeds the physical carrier's 64 KiB leaf bound. General allocation/status

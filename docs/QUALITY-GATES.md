...
A dedicated Linux `callable-host-sanitizers` job is also configured to compile
the generated callable providers with combined ASan/UBSan instrumentation and
load them through the Rust host for the authoritative O0/O2 corpus. Its linker
flags make the sanitizer runtimes available to that executable; they do not
instrument the Rust host code. The job remains pending evidence until a public
CI run is green, and Rust-host sanitizer instrumentation remains a separate
gate.
...
quiescence, green public Windows runtime/dependency isolation, a green public
dynamic-provider sanitizer run, sanitizer instrumentation of the Rust host,

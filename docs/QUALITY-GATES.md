...
31341453652, job 93315816975](https://github.com/wavect/semaprax/actions/runs/31341453652/job/93315816975).
...
| Copy-variant backend | Non-generic nominal unit/direct-`i64`/direct-`bool` construction and exhaustive copy-only match with scalar `i64`/`bool` arms execute through strict native C11 at O0/O2 and real Node/Wasm; constructor payloads evaluate in authored order before zero-initialization/payload writes/tag-last publication; scrutinee evaluates once and only the selected arm runs; internal aggregate pointer parameters and caller-owned results preserve full poison on contract/arithmetic failure; invalid tags fail out of band without semantic-status mapping or publication; repeated same-instance Wasm calls restore the shadow stack; native/Wasm results agree. Generic, resource- or record-bearing variants, non-copy match modes, `Option`, `Result`, `?`, public aggregate ABI, callable/component signatures, and public resource gates remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --lib variant_layout::tests::`, `cargo test --locked -p semaprax --all-features --test executable_variant_backends`, and `cargo test --locked -p semaprax --all-features --lib wasm::aggregate::tests::node_executes_copy_variants_selected_arms_invalid_tags_and_reentry -- --exact` |

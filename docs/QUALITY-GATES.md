...
| Private WIT/component boundary v1/v2/v3/v4/v5/v6 | Preserve every v1-v5 byte/KAT contract. V5 retains its hosted-green six-export direct-copy algebraic matrix. Private Nested Records Component v6 is a separate default-off, capability-free fixed profile over one exact nested scalar record closure; deterministic source/core/profile/component authentication, upstream validation, hostile closure, default-consumer hiding, independent security review, and isolated pinned-Rust-1.97.1 typed execution are hosted green in [run 31365363898, job 93383304974](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304974). General source selection/export, generic/user/resource/non-Copy records, imports/capabilities, callbacks/reentrancy/async, callable/FFI aggregate signatures, browser/multi-engine conformance, package negotiation, public API/ABI, and `SPX-B104`/`SPX-W111` remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --lib wit_component::tests::`, `cargo test --locked -p semaprax --all-features --lib wasm::nested_record_component_v6::tests::`, `cargo test --locked -p semaprax --all-features --lib wit_component::nested_record_v6::tests::`, and `cargo test --locked -p semaprax --test component_runtime_ci_contract` |

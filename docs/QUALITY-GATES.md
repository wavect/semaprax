...
Authority goldens prove deterministic mechanics, and OS smoke proves only that the supported host source was available. The private physical-host fixtures add exact loader-backed retention, ledger replay rejection, and same-thread owner topology on Linux/macOS; they still do not prove mathematical uniqueness, key secrecy after compromise, fork safety, Windows runtime behavior, callable loaded-code safety, or public backend conformance.

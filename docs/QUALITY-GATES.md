...
| Project v8 C++ owned-data package v1 | Exact held-Project v8 descriptor/provider regeneration and package replay; bounded canonical C/C++ artifacts; separate C11 provider and C++17 wrapper compilation, linking and execution at O0/O2; exact/+1 cumulative input and output limits; poison preservation and recovery; wrong-context/stale/duplicate handle rejection; copy/drop/context-close settlement and fail-stop uncertainty. [Project v8 C++ owned-data package](PUBLIC-CXX-OWNED-DATA-PACKAGE-V1.md) owns the local gate; target activation, MSVC/cross-platform ABI, maintained distribution, compatibility and support promotion remain separate. |

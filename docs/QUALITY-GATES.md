...
| Private WIT/component boundary v1 | Default-off feature plus external default-consumer compile-fail; exact `SPXWIT01` framing and SHA-256 KAT; deterministic WIT, canonical mapping JSON, and JavaScript adapter; every-byte/truncation/trailing/magic rejection; own-data snapshot normalization with accessor and changing-proxy regressions; lossless UTF-8 including lone-surrogate rejection, valid-pair acceptance, exact 255-byte ceiling, no NUL, and exact `semaprax.status.v1` u32 bounds; separate standards-valid scalar Component Model binary with frozen digest, exact canonical section/lift/export and embedded-core bytes, independent parser, canonical-LEB enforcement, every-byte/truncation/trailing/same-signature changed-core/hostile-section rejection, immutable runtime input snapshot, exact signed-64 argument bounds, and Node execution of the authenticated core module; explicit no composition between the two artifacts, engine-native component runtime, WIT import, checked-language equivalence, ownership/resource, future/stream, capability, multi-language, public-interface, or `SPX-B104` claim |

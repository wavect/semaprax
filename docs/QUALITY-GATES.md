...
| Native physical ownership host | Separate unpublished crate and unchanged compiler `unsafe_code = "forbid"`/`SPX-B104`; strict canonical descriptor-v1 decoding before load and an independent staged strict descriptor-v2 decoder; compiler-v2 cross-crate acceptance plus every-byte/truncation/trailing mutation rejection; exact loader-instance binding; OS-seeded same-thread authority; credential-to-ledger resource/lifecycle/slot/generation checks; unsafe trusted adoption; noncopying/nonformatting/`!Send`/`!Sync` owners; typed scalar preflight and reusable owners after rejection; scalar/owned success and normalized failure; generation rotation; panic and panicking-payload-destructor containment after commit; one-way draining; exact lease retention through host/authority/owners; equal-descriptor cross-instance rejection; real Linux/macOS fixtures and Windows compile coverage. Descriptor v2 and the callable loader are not yet connected to host ownership execution; no generated callable, Windows runtime, or callable sanitizer conformance is claimed |
...
Authority goldens prove deterministic mechanics, and OS smoke proves only that the supported host source was available. The private physical-host fixtures add exact loader-backed retention, ledger replay rejection, and same-thread owner topology on Linux/macOS. Separately, the shared 14-case corpus proves exact reference/native-generated-C/Wasm semantic trace and outcome equality for the narrow admitted shape; the native side remains a test-only harness rather than the physical ownership host. None of this proves mathematical uniqueness, key secrecy after compromise, fork safety, Windows runtime behavior, a generated callable connected to the host, callable-path sanitizer safety, or public native backend conformance.

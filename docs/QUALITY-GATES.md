...
| Language-native source Agent population | Parse and canonically format the closed `.spx` Agent declaration; lower through the unchanged AgentDefinition v1 compiler; preserve exact AgentDefinition, AgentGraph and Runtime Profile bytes; reject role/kind/identity collisions; retain Project Agent facts in stable-ID order; select one identical populated AgentDefinitions node through canonical workspace, ProgramRoot, typed query and semantic service; no wire change or authority. [Language-native Agent syntax v1](LANGUAGE-NATIVE-AGENT-SYNTAX-V1.md), [Language-native Agent lowering v1](LANGUAGE-NATIVE-AGENT-LOWERING-V1.md), and [Language-native Agent Object v1](LANGUAGE-NATIVE-AGENT-OBJECT-V1.md) own the focused `source_agent_lowering::` gate. |

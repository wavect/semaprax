...
| Private WIT/component boundary v1/v2/v3/v4/v5 | Preserve every v1-v4 requirement and exact byte/KAT contract. Private Scalar Algebraic Component v5 additionally binds a capability-free seven-function source table/order, compiler prelude, six Wasm32 layout-v2 digests, stable-ID/core-index/distinct-WIT-type/export mapping, canonical outer layouts, fieldwise tag-last reconstruction, and exact source/core/profile/component DAG for six `Option`/complete direct-copy `Result` exports nested inside outer status. Require exact-source hostility, every-byte/truncation/trailing/noncanonical LEB closure, valid same-signature function/type reindexing rejection, cross-version confusion rejection, upstream validation, raw noncanonical bool/select and invalid carrier/status traps, default-consumer hiding, and isolated zero-import/empty-linker/no-WASI runner source locks. Typed outcomes must cover every carrier arm and bool value, skipped arithmetic, status precedence, repeated multi-function and fresh-instance calls, plus out-of-band fuel failure; the complete pinned Rust 1.97.1 gate is hosted green in [run 31360176398, job 93367728269](https://github.com/wavect/semaprax/actions/runs/31360176398/job/93367728269). General source selection/export, nested/record/user-variant/resource/non-copy carriers, imports/capabilities, callbacks/reentrancy/async, callable/FFI aggregate signatures, browser/multi-engine conformance, package negotiation, public API/ABI, and `SPX-B104`/`SPX-W111` remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --lib wit_component::tests::`, `cargo test --locked -p semaprax --all-features --lib wasm::scalar_algebra_component_v5::tests::`, `cargo test --locked -p semaprax --all-features --lib wit_component::scalar_algebra_v5::tests::`, and `cargo test --locked -p semaprax --test component_runtime_ci_contract` |

...
| Concrete generic owned-byte records | Exact owner/index substitution in source, HIR, inventory, plan replay and target layouts; direct `Bytes` plus Copy-scalar closure; stable nested/class/variant/resource/non-Copy rejection; borrow then own; post-owner-creation failure settlement; repeated interpreter/native O0/native O2/Wasm execution; one-live-owner Wasm capacity; [Concrete Generic Owned-Byte Records v1](CONCRETE-GENERIC-OWNED-BYTE-RECORDS-V1.md) owns the bounded local gate |

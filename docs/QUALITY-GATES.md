...
| Private WIT/component boundary v1/v2/v3/v4/v5/v6/v7/v8/v9 | Preserve every v1-v8 byte/KAT contract and all existing hosted evidence. Private Record-Pattern Projection Component v8 remains the exact hosted `semaprax:private@0.6.0` monomorphic Graph-v13 profile described above. Private Generic-Function Instance Component v9 is a separate default-off, capability-free exact profile for package `semaprax:private@0.7.0`, interface `generic-function-instances`, world `semaprax-private-v9`, three phantom Copy templates (`preserve<T>`, `invert<T>`, `ordered<T,U>`), and six exact ordered Graph-v14 `FunctionInstanceId` exports. Require one exact monomorphic materializer plus `app.main`; six identical `(bool,s64)->result<bool,status>` WIT signatures; no authored type, record, or layout roots; exact source/Graph/core/plan/profile/raw-component/artifact-DAG KATs; exact instance rather than declaration/wrapper selection; independent/upstream validation; every-byte/truncation/trailing/noncanonical/cross-version hostility; all 15 pair-swap rejections with the exact eight behaviorally observable/seven identity-only partition; status-before-output, full 20-byte poison, input/output bool and tag/status closure; default-consumer hiding; source/runner/CI locks; strict gates; and independent security review. Local v9 core 5/5, component 4/4, CI-lock 4/4, full gates, and security review are green. The isolated pinned-Rust-1.97.1/Wasmtime-47 zero-import/empty-linker/no-WASI typed v9 runner is hosted green in [run 31392541096, job 93467490492](https://github.com/wavect/semaprax/actions/runs/31392541096/job/93467490492). General source selection/export, general/empty/nested/resource/non-Copy record mapping, general generic-function Component mapping, inference/constraints, imports/capabilities, callbacks/reentrancy/async, callable/FFI aggregate signatures, browser/multi-engine conformance, package negotiation, public API/ABI, and `SPX-B104`/`SPX-W111` remain closed. Focused v9 commands: `cargo test --locked -p semaprax --all-features --lib wasm::generic_function_component_v9::tests::`, `cargo test --locked -p semaprax --all-features --lib wit_component::generic_function_v9::tests::`, and `cargo test --locked -p semaprax --test component_runtime_ci_contract` |

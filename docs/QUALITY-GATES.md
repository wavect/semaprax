...
| Graph schema or agent context | Exact schema assertion/snapshot, canonical SHA-256 known answer, resolved-reference integrity, stable-ID collision behavior, owner/index generic-parameter identity, exact concrete argument trees, compiler-prelude schema/digest authentication, canonical-source-plus-prelude revision binding, exact byte/node accounting, ordered truncation reasons and omitted counts, re-expandable frontier replay, exact supported/unavailable filters, deterministic replay, and unknown/duplicate/malformed CLI rejection |
...
| Generic Copy-record backend | Explicit record templates with one or more owner/index-stable parameters, direct scalar/own-parameter fields, and explicit direct `i64`/`bool` arguments must round-trip canonically and substitute exactly through constructor, projection, immutable update, parameters, and results. Require stable `SPX-T220`/`T221`/`T223`, namespace-shadowing closure, owner/index/order/field/type-argument HIR hostiles, full instance-keyed type facts/layout cache/digest/C symbols including same-layout `Phantom<i64>`/`Phantom<bool>`, program-bound Graph v12 precedence and mixed-root context, unchanged v10/v11 snapshots, canonical CleanupPlan v2 replay with no invented resource actions, strict Native C11 O0/O2 construct/update/pass-return/bool/poison/failure-order evidence, deterministic Wasm, and 4,096-call Node re-entry. This gate is hosted green in [run 31365363898, Ubuntu job 93383304995](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304995). Generic functions/inference, nested/resource/non-Copy arguments or fields, public aggregate/callable/FFI ABI, and public resource gates remain closed; the separate bounded record-pattern gate below owns matching evidence. Focused commands: `cargo test --locked -p semaprax --all-features --test generic_records`, `cargo test --locked -p semaprax --all-features --test executable_generic_record_backends`, `cargo test --locked -p semaprax --all-features --lib aggregate_layout::tests::`, and `cargo test --locked -p semaprax --all-features --lib generic_record_c_symbols_bind_the_full_concrete_instance` |
| Irrefutable Copy-record patterns | Exact named-field record patterns require one record scrutinee evaluation, one arm, every stable field exactly once, and scalar `i64`/`bool` output. Admit recursive record patterns, shorthand/renamed scalar or whole Copy-record bindings, ignored fields, and a binding-free top-level wildcard; reject foreign/missing/duplicate fields, shadowing, multiple arms, aggregate results, resources/non-Copy modes, and cross-instance HIR substitutions. Require Graph v13 iff an explicit authenticated record pattern occurs anywhere, with precedence over v12/v11/v10, exact recursive record/type/member/binding serialization, mixed-root context binding, wildcard-only schema neutrality, relabel rejection, and byte-identical lower-schema snapshots. CleanupPlan v2/v3 must stay straight-line with no new slots, transitions, status sources, or variant-case edges. Native C11 O0/O2 and real Node/Wasm must cover nested and generic records, whole-record bindings, ignored fields, both bool paths, exactly one lowered call scrutinee, failure precedence, postcondition/arm poison, deterministic bytes, and 4,096 re-entries. This gate is hosted green in [run 31373317800, Ubuntu job 93406925130](https://github.com/wavect/semaprax/actions/runs/31373317800/job/93406925130). Refutable/literal/guard/or/rest/nested-variant patterns, non-Copy/resource matching, aggregate arms, and public aggregate/callable/FFI admission remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --test record_patterns` and `cargo test --locked -p semaprax --all-features --test executable_record_pattern_backends` |
| Copy/generic-variant backend | Monomorphic unit/direct-scalar variants plus nominal templates explicitly instantiated with direct `i64`/`bool` arguments and ordinary compiler-owned `Option<T>`/`Result<T,E>` execute exhaustive copy-only matches with scalar arms through strict native C11 at O0/O2 and real Node/Wasm. Require owner/index-stable parameters, exact substitution, hostile arity/scope/nested/resource rejection, distinct concrete identity/cache/symbols, internal layout digest-v2 reconstruction/KATs, cleanup-free exact-scrutinee replay, authored constructor order before zero-initialization/payload writes/tag-last publication, one scrutinee, selected-arm-only execution, full caller-output poison, invalid-tag failure outside semantic status/publication, repeated Wasm shadow-stack restoration, and native/Wasm agreement. Generic functions, nested/resource arguments, resource- or record-bearing payloads, non-copy match modes, public aggregate ABI, callable/component signatures, and public resource gates remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --test generic_variants`, `cargo test --locked -p semaprax --all-features --test graph_generics`, `cargo test --locked -p semaprax --all-features --lib variant_layout::tests::`, `cargo test --locked -p semaprax --all-features --test executable_variant_backends`, and `cargo test --locked -p semaprax --all-features --lib wasm::aggregate::tests::node_executes_generic_option_result_and_preserves_full_failure_poison -- --exact` |
| Typed ordinary-`Result`/`Option` propagation | Canonical postfix parse/format/parse and precedence; stable `SPX-T218` rejection for cross-carrier, custom, nested/resource, and contract contexts plus preserved Result `SPX-T219` mismatch; exact compiler-owned carrier/case/field identities and source/outer concrete instances in HIR; hostile identity/type/ownership rejection. Result-only graphs remain exact v10 with CleanupPlan v2; authenticated Option propagation requires program-bound Graph v11 and per-function CleanupPlan v3 unless a generic record declaration raises the whole program to v12 or an explicit record pattern raises it to v13, including mixed legacy-root context, schema relabel rejection, complementary exact-Some branching, and exact payload-free Option-None source authentication. Both carriers require one evaluation, later-expression skip, shared ensures/commit, missing/duplicate/wrong-instance/status-confusion rejection, strict native C11 O0/O2 plus real Node/Wasm equality for both source/outer layout directions, semantic residual versus physical status, postcondition poison, invalid tags before payload access, and repeated re-entry. Keep general residual conversion, nested/resource/non-copy arguments, generic functions, `?` in contracts, public aggregate ABI, callable/component signatures, and conformance-trace aggregate values closed. Focused commands: `cargo test --locked -p semaprax --all-features --test option_try_semantics`, `cargo test --locked -p semaprax --all-features --test result_try_semantics`, `cargo test --locked -p semaprax --all-features --test graph_result_try`, `cargo test --locked -p semaprax --all-features --test executable_try_backends`, `cargo test --locked -p semaprax --all-features --lib cleanup_plan::replay::tests::try_replay_authenticates_complementary_result_cases_and_exact_staging -- --exact`, and `cargo test --locked -p semaprax --all-features --lib cleanup_plan::execute::tests::try_executor_stages_each_normal_result_path_then_fails_closed_at_materialization -- --exact` |
...
| Private WIT/component boundary v1/v2/v3/v4/v5/v6/v7 | Preserve every v1-v6 byte/KAT contract. V5 retains its hosted-green six-export direct-copy algebraic matrix; v6 retains its hosted-green exact nested scalar-record transform in [run 31365363898, job 93383304974](https://github.com/wavect/semaprax/actions/runs/31365363898/job/93383304974). Private Generic Record Component v7 is a separate default-off, capability-free fixed profile over ordered `Duo<i64, bool>`/`Duo<bool, i64>` and same-layout-distinct `Phantom<i64>`/`Phantom<bool>` instances. Require exact source/core/four-layout/Graph-v12/plan/profile/component authentication, stable-ID/core-index/distinct-WIT-type/export mapping, independent/upstream validation, every-byte/truncation/trailing/noncanonical/reindex/cross-version hostility, raw-core bool/status/poison checks, default-consumer hiding, source locks, strict gates, and independent security review. The isolated pinned-Rust-1.97.1/Wasmtime-47 typed runner is hosted green in [run 31373317800, job 93406924922](https://github.com/wavect/semaprax/actions/runs/31373317800/job/93406924922). General source selection/export, general/empty/nested/resource/non-Copy record mapping, imports/capabilities, callbacks/reentrancy/async, callable/FFI aggregate signatures, browser/multi-engine conformance, package negotiation, public API/ABI, and `SPX-B104`/`SPX-W111` remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --lib wit_component::tests::`, `cargo test --locked -p semaprax --all-features --lib wasm::generic_record_component_v7::tests::`, `cargo test --locked -p semaprax --all-features --lib wit_component::generic_record_v7::tests::`, and `cargo test --locked -p semaprax --test component_runtime_ci_contract` |

...
nonmutation at `-O0`/`-O2` and under ASan+UBSan. Documentation must still deny
canonical pre-execute unwind recovery, exhaustive fatal-allocator/process-crash
evidence, hosted Windows evidence until its configured job passes, Android/iOS,
quiescence, malicious-code containment, public admission, and any `SPX-B104`
change.
...
loaded sanitizer job. [Run 31312094861](https://github.com/wavect/semaprax/actions/runs/31312094861)
proved Linux, macOS, MSRV, dependency policy, generated-provider ASan+UBSan,
and Rust-host ASan. Windows stopped before runtime because MSVC deprecated the
fixture marker's `fopen`; the local fixture-only CRT fix is not hosted evidence
until the next run passes.

...
| Copy/generic-variant backend | Monomorphic unit/direct-scalar variants plus nominal templates explicitly instantiated with direct `i64`/`bool` arguments and ordinary compiler-owned `Option<T>`/`Result<T,E>` execute exhaustive copy-only matches with scalar arms through strict native C11 at O0/O2 and real Node/Wasm. Require owner/index-stable parameters, exact substitution, hostile arity/scope/nested/resource rejection, distinct concrete identity/cache/symbols, internal layout digest-v2 reconstruction/KATs, cleanup-free exact-scrutinee replay, authored constructor order before zero-initialization/payload writes/tag-last publication, one scrutinee, selected-arm-only execution, full caller-output poison, invalid-tag failure outside semantic status/publication, repeated Wasm shadow-stack restoration, and native/Wasm agreement. Generic functions/records, nested/resource arguments, resource- or record-bearing payloads, non-copy match modes, public aggregate ABI, callable/component signatures, and public resource gates remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --test generic_variants`, `cargo test --locked -p semaprax --all-features --test graph_generics`, `cargo test --locked -p semaprax --all-features --lib variant_layout::tests::`, `cargo test --locked -p semaprax --all-features --test executable_variant_backends`, and `cargo test --locked -p semaprax --all-features --lib wasm::aggregate::tests::node_executes_generic_option_result_and_preserves_full_failure_poison -- --exact` |
| Typed ordinary-`Result` propagation | Canonical postfix parse/format/parse and precedence; stable `SPX-T218`/`SPX-T219` rejection for non-Result context, Option, contract use, or mismatched `E`; exact compiler-owned Result/case/field identities and source/outer concrete instances in HIR; hostile identity/type/ownership rejection; Graph v10 KAT and v9/v10/rehashed semantic confusion; CleanupPlan v2 complementary same-Ok-case branching, exact body/residual staging, one evaluation, later-expression skip, shared ensures/commit, and missing/duplicate/wrong-instance/status-confusion rejection; strict native C11 O0/O2 plus real Node/Wasm equality for different source/outer layouts, semantic Err versus physical status, postcondition poison, invalid tags before payload access, and repeated re-entry. Keep Option/general residual conversion, nested/resource/non-copy arguments, generic functions/records, `?` in contracts, public aggregate ABI, callable/component signatures, and conformance-trace aggregate values closed. Focused commands: `cargo test --locked --test result_try_semantics`, `cargo test --locked --test graph_result_try`, `cargo test --locked --test executable_try_backends`, `cargo test --locked --lib cleanup_plan::replay::tests::try_replay_authenticates_complementary_result_cases_and_exact_staging -- --exact`, and `cargo test --locked --lib cleanup_plan::execute::tests::try_executor_stages_each_normal_result_path_then_fails_closed_at_materialization -- --exact` |
...
| Private WIT/component boundary v1/v2/v3/v4 | Default-off feature plus external default-consumer compile-fail; exact `SPXWIT01` framing and SHA-256 KAT; deterministic WIT, mapping JSON, JavaScript adapter, status bounds, hostile-object snapshots, standalone v1 scalar component, checked generated-core v2, private read-only digests, exact-profile parsers, canonical-LEB/every-byte/truncation/trailing closure, rehashed cross-type rejection, pinned upstream validation, immutable inputs, and Node v1/v2 execution. Portable Result Component v3 must bind exact core/profile/source/component KATs to `result<s64, status>`, preserve poison/sticky status at the generated-core boundary, execute the complete checked-operation status matrix, and execute typed Wasmtime success/add-overflow/division-by-zero/precondition/postcondition plus competing failures on repeated and fresh instances. The v3 runner is hosted green in [run 31347109201, job 93330959212](https://github.com/wavect/semaprax/actions/runs/31347109201/job/93330959212). Source-Result Component v4 must bind the exact effect-free `component.source`/`component.evaluate` closure, compiler prelude, `Result<i64,bool>`/`Result<bool,bool>` layout-v2 KATs, source revision, generated core, profile, and component DAG to WIT 0.2 `result<result<bool,bool>,status>`; independently parse exact core/component topology; pass upstream validation; reject every-byte/truncation/trailing/noncanonical/rehashed/cross-version/admission hostile; check status before poisoned result reads; reconstruct canonical memory without transmuting the internal variant; trap invalid tags/unknown statuses; and keep language `Err` distinct from physical status. The isolated exact Rust 1.97.1/Wasmtime 47.0.3 graph must remain outside the root workspace with zero imports, empty linker, no WASI/callbacks, locked offline format/lint/test/build/run, fail-closed dependency policy, typed inner `Ok`/`Err` and outer-status cases on same/fresh instances, and engine failures out of band. V4 compiler/component/source-lock gates and the typed runner are hosted green in [run 31356536123, job 93357169796](https://github.com/wavect/semaprax/actions/runs/31356536123/job/93357169796). General source `Result`/`Option`/`?` mapping, records/resources/imports, async, capabilities, callable/FFI aggregate signatures, multi-engine/browser conformance, public API/ABI, and `SPX-B104` remain closed. Focused commands: `cargo test --locked -p semaprax --all-features --lib wit_component::tests::`, `cargo test --locked -p semaprax --all-features --lib wasm::source_result_component_v4::tests::`, and `cargo test --locked -p semaprax --test component_runtime_ci_contract` |

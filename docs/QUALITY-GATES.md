...
| Private WIT boundary v1 | Default-off feature; exact `SPXWIT01` framing and SHA-256 KAT; deterministic WIT, canonical mapping JSON, and JavaScript adapter; every-byte/truncation/trailing/magic rejection; exact `semaprax.status.v1` u32 code and 1..=255 UTF-8 no-NUL domain bounds; valid-boundary and hostile-shape cases executed by Node; explicit no Component Model binary/parser/runtime, WIT import, resource, future/stream, capability, multi-language, public-interface, or `SPX-B104` claim |

...
The v0.2 source and verification subset supports typed `i64`/`bool` functions and expressions, lexical bindings, typed branches, resource declarations, ownership-qualified function boundaries, and control-flow move checking. Resource-free programs produce native host output and browser-loadable WebAssembly core output; resource-bearing modules fail closed until lifecycle declarations and exactly-once cleanup are implemented. This subset exists to validate the graph, transaction, ownership, and multi-backend architecture before expanding syntax.

...
The executable v0.2 subset supports typed `i64`/`bool` functions and expressions, lexical bindings, typed branches, resource declarations, ownership-qualified function boundaries, control-flow move checking, native host output, and browser-loadable WebAssembly core output. This subset exists to validate the graph, transaction, ownership, and multi-backend architecture before expanding syntax.

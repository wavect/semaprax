...
SEMAPRAX is a graph-native, contract-driven systems language for people and
autonomous software agents working on the same program.
...
The logical program is a versioned semantic graph. Nodes include declarations,
specifications, tests, packages, schemas, targets, and generated artifacts;
edges record calls, types, implementation, capabilities, proofs, targets, and
ownership.
...
This RFC defines the mature contract, not the current implementation subset.
The [completion matrix](COMPLETION-MATRIX.md) owns present status. Bounded
language additions such as [mutation](EXPLICIT-MUTATION-V1.md),
[matching](REFUTABLE-MATCH-V1.md), [text](USEFUL-TEXT-CONSUMER-V1.md),
[byte data](PORTABLE-INDEXED-BYTE-DATA-V1.md), and
[unsafe boundary mechanics](UNSAFE-BOUNDARIES-V1.md), and
[session protocols](SESSION-PROTOCOL-TYPES-V1.md#declared-session-protocols-issue-297)
each define their exact admission and non-claims without expanding this RFC's
status narrative.

A session protocol is a checked, erased declaration of legal message order:

```text
session_protocol := "session" "protocol" STRING "{"
                      "states" "{" [IDENT ("," IDENT)* [","]] "}"
                      "initial" IDENT ";"
                      ("terminal" IDENT "cleanup" "{" [IDENT ("," IDENT)* [","]] "}")*
                      transition*
                    "}"
transition       := "on" IDENT IDENT ":" KIND IDENT
                      ["requires" "capability" QUALIFIED_IDENT]
                      ["consumes" "resource"] ["via" STRING]
                      "->" (IDENT | "choice" "{" IDENT ":" IDENT ("," IDENT ":" IDENT)* [","] "}") ";"
KIND             := "send" | "receive" | "call" | "return" | "cancel" | "timeout" | "fail"
```

It has no runtime representation and grants no authority: a `requires
capability` must already be an effect its `via` function declares.

...
rides the same closed boundaries as floats. Text (`string`)
adds `u8`, an unsigned checked-arithmetic byte scalar with exact suffixed
...
closed boundaries as floats and chars. Text (`string`)
remains unimplemented until the allocation/ownership tranche lands.

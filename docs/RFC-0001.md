...
The v0.2 source and verification subset supports typed `i64`/`bool` functions
and expressions, including bounded explicitly instantiated effect-free Copy
generic functions, lexical bindings, typed branches, resource declarations,
ownership-qualified function boundaries, and control-flow move checking.
A locally evidenced tranche adds IEEE-754 `f32`/`f64` scalar value types with
total float arithmetic (no failure statuses), comparisons, and bit-exact
literals across the verifier, HIR, layouts, graph, and native/Wasm backends;
floats are not yet admitted as generic arguments or in the scalar-export,
native-host, and Rust-interop profiles. A further tranche adds `char`, one
Unicode scalar value per literal with exact canonical escapes and scalar-value
ordering; chars ride the same closed boundaries as floats. A further tranche
adds `i32`, a checked signed 32-bit integer with an explicit literal suffix;
arithmetic keeps the declared width and selects the same normalized failure
statuses as `i64` instead of wrapping, while `%` remains `i64`-only and i32
rides the same closed boundaries as floats. A further tranche adds `u8`, an
unsigned checked-arithmetic byte scalar with exact suffixed
literals, 0..=255 range-checked arithmetic that selects the same normalized
failure statuses as i64, and one-byte Native64 layout; u8 rides the same
closed boundaries as floats and chars. Owned UTF-8 `string` values and their
bounded allocation/ownership operations are implemented separately. The
additive [Useful Text Consumer v1](USEFUL-TEXT-CONSUMER-V1.md) profile adds a
real borrowed UTF-8 `str` input boundary: only `borrow str` parameters are
admitted, views cannot escape or enter aggregates/cleanup, and compiler-owned
byte-length/empty/prefix/contains operations lower across interpreter, a native
pointer-plus-length host contract, and a range-validated fixed-scratch
Wasm/JavaScript boundary. The profile has an exact 65,536-byte cumulative
borrowed-input limit; contains is linear-time byte-KMP, and Wasm uses fixed
three-page memory with only the first 64 KiB exposed as public scratch.
This remains a restrictive input profile, not `usize`, arrays, slices,
indexing, iteration, or a general text processor.
[Explicit Mutation v1](EXPLICIT-MUTATION-V1.md) adds immutable-by-default
local bindings with an opt-in `let mut` modifier and a statement-only
`<binding> = <expr>;` form over checked Copy scalars: the assigned value
evaluates fully (with initializer-equivalent failure statuses) before one
exact-type store into the binding's existing value identity; diagnostics
`SPX-U101`-`SPX-U106` reject immutable targets, exact type mismatches,
`mut` outside local lets, duplicate modifiers, non-scalar/non-Copy admission,
and contract-expression assignment, and field/aggregate, reference/borrow,
collection, and cross-task mutation stay closed. Resource-free admitted
programs produce native host output and browser-loadable
WebAssembly core output; broader resource-bearing modules fail closed outside
their separately documented private/narrow evidence. This subset exists to
validate the graph, transaction, ownership, and multi-backend architecture
before expanding syntax.

[Unsafe Boundary Mechanics v1](UNSAFE-BOUNDARIES-V1.md) adds a bounded
boundary-mechanics slice of the future restricted-`unsafe` profile, and no
raw-memory feature: an `unsafe { .. }` statement wraps ordinary safe checked
statements plus one required final value expression (no raw pointers or
memory operations exist in the language and none are added). Each block
requires a verbatim `@audit("...")` summary attribute following the `@id`
syntax pattern, and the enclosing module must declare the capability through
the existing `permit { .. }` mechanism with the reserved name `unsafe`
(`SPX-N101`-`SPX-N105` reject missing declarations, missing or malformed
audit summaries, non-scalar Copy body results, and contract-context
boundaries). Graph serialization adds one explicit `"kind":"unsafe"` node per
boundary without changing schema selection; programs without boundary syntax
stay byte-identical. Backends lower the body transparently on native C11
O0/O2 and Node/Wasm. No lint, platform conformance, or safety claim about
block contents is made; contents remain fully checked safe code.

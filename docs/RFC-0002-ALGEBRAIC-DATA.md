...
classes and variants, resources, public aggregate ABIs, and hosted promotion
remain closed.
...
4. Add generic variants, recursive-unsized rejection, and ownership-aware matching. **Partially implemented for nominal variant templates with explicit direct `i64`/`bool` arguments, exact substitution/instance identity, Graph v10, internal layout digest v2, and cleanup-free copy matching. A bounded authored extension separately admits direct `Bytes` plus any of the eight admitted Copy-scalar arguments in flat `Either<Bytes, i64>` / `Either<i64, Bytes>`-equivalent instances with exactly one owned case through explicit own/borrow matching, conditional cleanup replay, interpreter, native C11 O0/O2, and Core-Wasm locally. Two-owned-case instances including `Result<Bytes, Bytes>`, nested/resource arguments, public ABIs, and hosted promotion remain open.**

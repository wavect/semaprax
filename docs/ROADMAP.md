...
| ABI-09: public generic programme | Existing public profiles remain separately closed; internal generic implementation does not change their signatures | Execute the nine prerequisite gates of the [Public Generic Ownership milestone](PUBLIC-GENERIC-OWNERSHIP-MILESTONE-V1.md). Four are implemented with local evidence: the versioned target-neutral type grammar with template and ordered argument identities, the semantic compatibility rules, and the candidate delta. The next tranche is the one artifact the remaining five all wait on — a new versioned public generic descriptor and carrier — followed by calling consumers, descriptor-byte hostile replay, boundary settlement on every claimed engine, hosted cross-platform evidence, and only then the explicit support and publication decision. Completing internal generic semantics advances none of them. |
...
Both are now measured, named with exact constants, and pinned by regression fixtures ([Semantic Kernel

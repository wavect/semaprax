...
| ABI-09: public generic programme | Existing public profiles remain separately closed; internal generic implementation does not change their signatures | Execute the nine prerequisite gates of the [Public Generic Ownership milestone](PUBLIC-GENERIC-OWNERSHIP-MILESTONE-V1.md) — versioned target-neutral type grammar, template and ordered argument identities, semantic compatibility rules, candidate ABI-delta evidence, generated Rust/TypeScript-Wasm/C/C++ consumers, hostile metadata replay, owned allocation and failure settlement, cross-platform hosted evidence — before the explicit support and publication decision. Completing internal generic semantics advances none of them. |

...
| `std.http` | HTTP request/response types, client and server interfaces, streaming, and limits | Partial: allocation-free HTTP/1.x parsing helpers remain portable; the explicit Rust host additionally has a bounded typed HTTPS GET client with HTTP/1.1/2 negotiation, redirects, pooling, and body limits. A source-level typed API, server parser, HTTP/3, and generated target adapters are Missing |

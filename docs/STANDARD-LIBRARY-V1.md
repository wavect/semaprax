...
| `std.tls` | Vetted provider-backed TLS interface and certificate policy | Partial: hosted outbound TLS 1.2/1.3 uses Rustls, authenticated server names, WebPKI roots, and explicit custom client configuration; server TLS and native/Wasm lanes are Missing |
| `std.http` | HTTP request/response types, client and server interfaces, streaming, and limits | Partial: allocation-free HTTP/1.x status-line validation (including the required reason-phrase separator), header-boundary, `Content-Length`, method, and body-length helpers over borrowed response bytes, composed with `net_send`/`net_stream_stdout` for plain-TCP clients; typed request/response values, a server interface, chunked transfer, and TLS are Missing |

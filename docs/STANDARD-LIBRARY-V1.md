...
added after that baseline, are not covered by that release baseline; their
gates are hosted-green on the named `std-library-depth` job and the
`verify-tests` shards at 1dfe12a6.
...
| `std.env.policy` | Effect-free environment name and assignment policy | Partial: additive sibling package holding the policy a caller would otherwise reimplement - a portable name predicate (non-empty, no `=` or NUL, not digit-initial, POSIX portable character set), a value predicate (no NUL, otherwise arbitrary bytes, no UTF-8 claim), and a `NAME=VALUE` assignment cursor splitting at the first `=`. It is a sibling package rather than more of `std.env` because the completion gate requires every `std.env` function to declare `process.environment.read`, and these functions read no environment; declaring that effect would over-claim a capability they never exercise. Broader Everyday environment support remains Missing |
| `std.process` | Bounded process launch, pipes, exit, and settlement | Partial: private `process-io.v1` composition now executes the example, conformance, and bundled consumer commands on the interpreter, native C11 `-O0`/`-O2`, and Core Wasm. Additive effect-free policy classifies an existing settlement value as a normal exit or a signal termination - the only two kinds the termination encoding defines - and extracts an exit code only where the classification says the process exited normally. A deadline expiry is deliberately not a settlement class: it is a call-level failure that prevents an output from being constructed at all, so no settlement value ever carries it. Argument admissibility and argv counting complete the policy; only `std.process.run` still carries `process.execute`. The historical five-case physical Darwin witness passed, covering registered launch, pipe/deadline and settlement behavior; the admitted release regression corpus is HOSTED GREEN, while broader physical-provider, public and general process support remain open |
...
| `std.data.toml` | TOML parsing and encoding | Partial: allocation-free bare-key validation, blank/comment line recognition, and simple-quote/comment-aware assignment-delimiter location over borrowed bytes; Additive quoted-key and value cursors close the key half: `basic_quoted_key_end` admits the TOML basic-string escapes `\b \t \n \f \r \" \\ \uXXXX \UXXXXXXXX` and rejects an unterminated string, a raw control byte and any other escape; `literal_quoted_key_end` admits `'...'` with no escapes at all; `key_end` dispatches bare, basic-quoted and literal-quoted forms; and `value_start`, `value_content_end` and `value_end` bound a value quote-aware, so a `#` inside quotes does not open a comment and trailing spaces and tabs are excluded. Failure is the package's `byte_len + 1 + offset` sentinel, the same encoding the `std.data.json` family uses for arbitrary-start scanners. Bare-key admission (`bare_key_end`) scans bytes one at a time through `is_bare_byte` rather than testing a growing `byte_range` sub-slice inside its `while` loop, because `byte_range` is not one of the shapes `hir::validation::owned_buffer::require_admitted_while_operation` admits inside a `while` body (`SPX-H006`, independent of the Core Wasm decoder fixed by issue #100); it then takes one `byte_range` of the scanned span after the loop and reuses `is_bare_key` on it, so the descriptor carrier is genuinely exercised end to end on all three backends rather than avoided (issue #218). Values remain uninterpreted bytes: tables, decoding, typed values, and encoding are Missing |
| `std.data.csv` | Streaming CSV reading and writing | Partial: allocation-free single-record field counting with quoted-comma and escaped-quote handling, balanced-quote checks, and strict complete-record quote-placement validation; Additive quote-aware field cursors walk one record: `field_end` stops at the first comma outside quotes, `field_start` opens the next field, `field_is_quoted` reports the quoted form, and `content_start`/`content_end` bound the field's content excluding the surrounding quotes, with `""` inside a quoted field treated as one escaped quote that never ends the field and left in the content bytes. Empty records, empty quoted fields and consecutive commas all yield exact offsets, and a walk terminates because the last field's `field_start` is the record length. The cursors compose with the `std.io.lines` record content and the `std.bytes` trimming offsets; these cursors are hosted-green at 1dfe12a6 through the `verify-tests` corpus shard rather than the targeted depth job. Typed fields, decoded content, dialects, streaming reads, and writing are Missing |

...
- Status: versioned reference; 34 packages are present under `std/`: nine
  `core`, sixteen `portable`, three `alloc`, three `hosted`, one `agent`, and two `test`. Every package remains
...
| `std.test` | Assertions, fixtures, property tests, fuzz targets, and snapshots | Partial: scalar equality predicates and deterministic unit or caller-selected failure status helpers remain in `std.test`; the sibling `std.test.bytes` package adds exact `Slice<u8>` equality, Reader-suffix equality with cursor validation and position preservation, and failure-bit wrappers. Reusable named byte snapshots additionally return a checked Copy comparison with equality, lengths, and relative first difference while preserving both borrowed inputs; source-package and bundled-consumer checks pass locally across the interpreter, C11 O0/O2, and repeated Core Wasm. Snapshot file discovery/updates, richer property tests, fuzz targets, and broader diagnostics remain open |
| `std.agent` | Agent types, model roles, context, Proposal grammar, approval, effects, checkpoints, and evidence | Partial: source-authored Task, Context, Observation, Outcome, lifecycle helper, and outcome helper records/functions are present in the private agent package. Focused package execution and all six linked-role cases now pass locally through the injected typed runtime, including imported roles, standard-package roles, root/body drift, intent/observed-ack recovery, suspend replay, and migration refusal; the four standard-library testing cases pass across the interpreter, native C11 `-O0`/`-O2`, and Core Wasm. Linked migration, native/Wasm Agent-stage execution, live providers, hosted support, and broader proposal, approval, effect, checkpoint, evidence, and full `std.agent` scope remain open. [Direct Agent Runtime v2](AGENT-RUNTIME-V2.md) and [Project Linked Agent Lifecycle v1](PROJECT-LINKED-AGENT-LIFECYCLE-V1.md) own the additive runtime boundaries |

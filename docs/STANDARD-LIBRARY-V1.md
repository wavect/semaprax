...
- Status: versioned reference; 35 packages are present under `std/`: nine
  `core`, seventeen `portable`, three `alloc`, three `hosted`, one `agent`, and two `test`. Every package remains
...
baseline for their admitted profiles. `std.io.lines`, added after that
baseline, has local evidence only and is not covered by it. The inventory is
nine core, seventeen portable, three alloc, three hosted, one agent and two
test packages.

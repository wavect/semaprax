...
while IFS="$tab" read -r kind gate _rest; do
    ...
    case "$gate" in
        ...
        example-checks)
            for source in meaning ownership lifecycle control_flow records native_callable chars integers_i32 bytes_u8; do
                cargo run --locked -p semaprax -- check "examples/$source.spx"
            done
            ;;
        example-fmt)
            for source in meaning effects ownership lifecycle control_flow records native_callable chars integers_i32 bytes_u8; do

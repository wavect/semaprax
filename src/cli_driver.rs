...
fn run(args: Vec<String>, host: Option<&PrivateHost>) -> Result<(), u8> {
    ...
    match command_id {
        ...
        CommandId::RetentionMetadataPlan => {
            persistence_dispatch::retention_metadata_plan(command, &args)
        }
        CommandId::RetentionMetadataPersist | CommandId::RetentionMetadataLoad => {
            persistence_dispatch::retention_metadata_persist_or_load(command, &args)
        }
        CommandId::ProjectDraftPersist | CommandId::ProjectDraftLoad => {
            persistence_dispatch::project_draft_persist_or_load(command, &args)
        }
        CommandId::ProjectCandidatePersist | CommandId::ProjectCandidateLoad => {
            persistence_dispatch::project_candidate_persist_or_load(command, &args)
        }
        ...
        CommandId::Explain => {
            let options = cli::explain::parse(&args[1..])?;
            cli::explain::run(options, |errors| report(errors, false))
        }
        CommandId::Fix => {

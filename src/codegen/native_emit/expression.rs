...
impl<'a, O: COutput> CEmitter<'a, O> {
    ...
    fn emit_match_expr(&mut self, expr: &ResolvedExpr) -> Result<CValue, Diagnostic> {
        let value = match &expr.kind {
            ResolvedExprKind::Match {
                        ...
                        hir::ResolvedMatchPattern::Variant {
                            ...
                            if *mode == hir::ResolvedMatchMode::Own {
                                let transitions = self
                                    ...
                                    .apply_variant_match_destructure_at(&expr.id, case)?;
                                for line in transitions.lines() {
                                    self.line(line);
                                }
                            }
                ...
                self.finish_variant_join(expr, &result)?;
                CValue {
                    code: result,
                    ty: expr.ty.clone(),
                }
    ...
    pub(super) fn variant_layout(&self, ty: &ResolvedType) -> Result<VariantLayout, Diagnostic> {

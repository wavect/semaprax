//! Deterministic, read-only C++ Shim Projection v1.
//!
//! `semaprax cxx-shim <file>` derives a C++17-compatible header fragment
//! from verified program facts for explicitly selected monomorphic scalar
//! functions. The emitted `extern "C"` declaration lines are extracted
//! verbatim from the production native C11 projection (`codegen::emit_c`),
//! so every shim declaration matches the ABI the native backend actually
//! emits. Generated comments carry only typed program facts: stable
//! identities, canonical contract text, effect sets, and the fixed
//! status/out-parameter contract of the native lane. No free-form host
//! input reaches the bytes.
//!
//! Diagnostics use the previously unused `SPX-X1xx` family:
//! - `SPX-X101`: invalid options (bounds, duplicates, malformed values).
//! - `SPX-X102`: invalid selection (empty, over budget, unknown target).
//! - `SPX-X103`: output byte-budget exhaustion (fail-closed, no truncation).
//! - `SPX-X104`: derived-text hygiene violation (comment-hostile characters).
//! - `SPX-X105`: envelope or native-projection consistency failure.
//!
//! This tranche performs no header import or parsing, no raw-binding import,
//! no safe wrapper generation, no aggregate/resource/string mapping, claims
//! no C++ compilation or conformance, adds no exception or ownership policy
//! beyond the bounded slice, executes nothing, and changes no source.
...
pub const SCHEMA: &str = "semaprax.cxx-shim.v1";
...
const GUARD_DOMAIN: &[u8] = b"semaprax.cxx-shim.guard.v1\0";
const SOURCE_DIGEST_DOMAIN: &[u8] = b"semaprax.cxx-shim.source.v1\0";
const PAYLOAD_DIGEST_DOMAIN: &[u8] = b"semaprax.cxx-shim.payload.v1\0";
const FRAGMENT_DIGEST_DOMAIN: &[u8] = b"semaprax.cxx-shim.fragment.v1\0";
const DECLARATION_DIGEST_DOMAIN: &[u8] = b"semaprax.cxx-shim.declaration.v1\0";
...
\"no_string_buffer_aggregate_or_resource_mappings\",\
\"no_hosted_execution\",\
\"read_only\"";
    ...
    pub fn new(functions: Vec<String>, max_bytes: usize) -> Result<Self, Diagnostic> {
        if functions.is_empty() || functions.len() > MAX_FUNCTIONS {
            return Err(option_error(format!(
                "cxx-shim requires between 1 and {MAX_FUNCTIONS} --function selections"
            )));
        }
        ...
        for function in &functions {
            if function.is_empty() {
                return Err(option_error(
                    "cxx-shim --function selections must be nonempty".to_owned(),
                ));
            }
            if !seen.insert(function.as_str()) {
                return Err(option_error(format!(
                    "cxx-shim selected `{function}` more than once"
                )));
            }
        }
        if !(graph::MIN_AGENT_CONTEXT_BYTES..=graph::MAX_AGENT_CONTEXT_BYTES).contains(&max_bytes) {
            return Err(option_error(format!(
                "cxx-shim max_bytes must be between {} and {}",
...
impl Default for CxxShimOptions {
    fn default() -> Self {
        Self {
            functions: Vec::new(),
...
fn option_error(message: String) -> Diagnostic {
    Diagnostic::io("SPX-X101", message)
}

fn selection_error(message: String) -> Diagnostic {
    Diagnostic::io("SPX-X102", message)
}
...
/// Generate the canonical `semaprax.cxx-shim.v1` envelope JSON for one
...
pub fn generate(source_path: &Path, options: &CxxShimOptions) -> Result<String, Vec<Diagnostic>> {
    generate_internal(source_path, options).map(|generation| generation.envelope)
}

/// Generate only the bare deterministic header-fragment bytes under the same
/// admission and budget rules as [`generate`].
pub fn fragment_text(
    source_path: &Path,
    options: &CxxShimOptions,
) -> Result<String, Vec<Diagnostic>> {
    generate_internal(source_path, options).map(|generation| generation.fragment)
}
...
/// embedded fragment digest. Returns the embedded fragment text on success.
pub fn verify_envelope(envelope: &str) -> Result<String, Diagnostic> {
    ...
    let Some(fragment) = payload_value["fragment"].as_str() else {
        return Err(consistency_error(
            "payload fragment must be a string".to_owned(),
        ));
    };
    let Some(fragment_digest) = payload_value["fragment_sha256"].as_str() else {
        return Err(consistency_error(
            "payload fragment_sha256 must be a string".to_owned(),
        ));
    };
    if fragment_digest != domain_digest(FRAGMENT_DIGEST_DOMAIN, fragment.as_bytes()) {
        return Err(consistency_error(
            "embedded fragment digest does not match the fragment text".to_owned(),
        ));
    }
    ...
    for function in &selected {
        match admission(function) {
            ...
            None => emitted.push(EmittedFunction {
                ...
                symbol: cxx_function_symbol(&function.stable_id),
        ...
        let function = program
            ...
            .ok_or_else(|| {
                vec![selection_error(format!(
                    "cxx-shim selection `{token}` does not name a function in this program"
                ))]
            })?;
        if !claimed.insert(function.stable_id.as_str()) {
            return Err(vec![selection_error(format!(
                "cxx-shim selections `{token}` and an earlier token both resolve to `{}`",
                function.stable_id
            ))]);
        }
...
/// Closed AST-level admission gate mirroring C Header Emission v1's widened
/// scalar profile (full Copy-scalar surface), plus the explicit-identity
/// requirement.
...
/// smuggle control characters into the generated fragment.
...
fn cxx_function_symbol(stable_id: &str) -> String {
...
fn include_guard(ids: &[String]) -> String {
    ...
    let mut guard = String::from("SPX_CXX_SHIM_");
...
\"declaration_sha256\":{},\"matches_native\":true}}",
    ...
    let exclusion_entries = excluded
        ...
        .collect::<Vec<_>>();
    let fragment_sha256 = domain_digest(FRAGMENT_DIGEST_DOMAIN, fragment.as_bytes());
...
\"fragment_sha256\":{},\"fragment\":{},\"nonclaims\":[{}]}}",
        SCHEMA,
...
fn render_fragment(revision: &str, guard: &str, emitted: &[EmittedFunction]) -> String {
    ...
    output.push_str("/*\n");
    output.push_str(" * Generated by SEMAPRAX C++ Shim Projection v1 (semaprax.cxx-shim.v1).\n");
...
"#;
    ...
    fn double_options() -> CxxShimOptions {
        CxxShimOptions::new(vec!["probe.double".to_owned()], DEFAULT_MAX_BYTES)
    ...
    fn options_reject_out_of_bounds_values() {
        ...
        assert!(
            CxxShimOptions::new(vec!["a".to_owned(); MAX_FUNCTIONS + 1], DEFAULT_MAX_BYTES)
                .is_err()
        );
        assert!(CxxShimOptions::new(vec![String::new()], DEFAULT_MAX_BYTES).is_err());
        assert!(
            CxxShimOptions::new(vec!["x".to_owned(), "x".to_owned()], DEFAULT_MAX_BYTES).is_err()
        );
        assert!(CxxShimOptions::new(vec!["x".to_owned()], 512).is_err());
        assert!(
            CxxShimOptions::new(vec!["x".to_owned()], graph::MAX_AGENT_CONTEXT_BYTES + 1).is_err()
        );
    ...
    fn selection_errors_fail_closed() {
        let path = write_temp(VALID_SOURCE);
        let unknown =
            CxxShimOptions::new(vec!["probe.missing".to_owned()], DEFAULT_MAX_BYTES).unwrap();

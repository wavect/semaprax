...
pub(super) fn payload(
    ...
    if method.name == "protocol/instructions" {
        ...
        {
            instructions.push_str(" Use agent/task-comparison only with an exact canonical caller-supplied observation set and its SHA-256. Each lane wrapper embeds and authenticates one existing semaprax.agent-task-comparison-observation.v1 document and adds exact source/image/candidate revision selectors plus wall, protocol and source byte observations. Semantic and source-first lanes are mandatory and unique. The v1 manifest marks Zero external_unrun; because this route does not replay a plan availability document, any supplied Zero observation is rejected and the Zero comparison remains not_assessed_missing_observation. The compiler validates closed shapes, bindings, metric and evidence-reference inventories, but performs no filesystem artifact lookup and does not verify the external recorder. Descriptive deltas are emitted only when the embedded observations' exact raw outcome fields are equal; any mismatch, including failed versus aborted, is not_assessed with no deltas. No model, tool, validator, filesystem, network or runtime is invoked and superiority is always not_assessed.");
        }
        if methods
            .iter()
        ...
        {
            instructions.push_str(" When change/catalog offers ordered signature parameters, from and optional name retain the selected existing parameter's exact type and ownership while permitting reordering. Additional checked owning metadata may describe bare string (surface mode value, checked ownership own) or explicitly own named records and variants with sized, resource-free, non-Copy drop facts. Every such owning parameter must be retained exactly once; omission or duplication is rejected. Exact borrow str and borrow Slice<u8> views may also be retained, reordered and renamed, and must each remain present exactly once; no borrowed nominal/storage type, borrow Bytes, shared parameter, resource or class is admitted. A mapping with only name and borrow_from may add one fresh borrowed parameter whose exact view type comes from the named original borrowed parameter. The source view must also be retained exactly once. The bounded owner-view forms instead replace one through eight distinct exact own Bytes or bare owning string parameters with borrow Slice<u8> or borrow str after authenticating the matching sole unprojected bytes_as_slice or string_as_str provider-body use and excluding every contract root. Callers stage all old arguments left to right, derive mapped views afterward, retain owner cleanup, and rely on full replay for the changed loan lifetime. Existing caller arguments are evaluated once in their original left-to-right order before mapped arguments reach the ordinary call commit boundary; full loan, provenance, ownership, cleanup, contract and target validation still decides admission. String reads retain ordinary compiler cloning semantics; staging can change allocations and does not promise allocation equivalence. This grants no owning default, copying operation, external consumer migration or permission to execute, and the existing computed/new parameter forms remain limited by their separate Copy admission.");
            instructions.push_str(" Typed expressions may use builtin_call with an exact compiler byte or string operation target and arguments in left-to-right order. Constructor schemas derive eight borrowed byte/view operations and seven owning string operations with exact argument counts from their compiler owners. Read optional builtin_calls metadata in change/catalog or full hole/query for source-compatible names, checked parameter ownership, return types and type families. Existing compiler_byte_operations rows retain their fields and append core.string.as-str; compiler_string_operations rows retain the StringOp inventory. array_u8_any_length remains a family rather than an exact zero-length type. string_as_str creates one non-escaping borrow str from an exact unprojected owning String root without transfer; str_as_bytes remains the separate borrow-str to byte-slice view. String length, emptiness, prefix, substring and Unicode scalar-count reads have borrowed String parameters; concat consumes both owning String arguments. FromChar copies one checked char value and returns String; its argument may be an authenticated char place, ordinary call result or the separate exact char literal constructor, never an integer/string coercion. Resolved ownership metadata is not a request to spell borrow string in source. No operation here returns char. Ordinary call and accessible_calls still select existing local or imported functions only. Empty declared effects do not imply allocation-free or infallible behavior; String results and bytes_copy may allocate, and all views, loans, ownership, cleanup, capacity and target-profile checks remain mandatory. These catalogues describe potential constructors, never contract validity, successful fill, or permission to execute. Every fill or candidate change requires full compiler admission; no ambient capability or source authority is granted.");
        }
        if methods
            .iter()
            .any(|method| method.name == "candidate/apply-intent")
        {
            ...
            instructions.push_str(" When change/catalog advertises add_record_field for an explicit monomorphic checked sized resource-free record, supply only field id, name, type and default. Existing source-admitted String, Bytes, array and nested record/variant storage can qualify through bounded compiler TypeFacts for the scalar lane; the target need not be Copy or match the flat owned-byte profile. Generic targets, classes, variant targets, resources and borrowed storage remain excluded. Scalar fields use i64, bool, i32, u8 or usize with the matching exact literal and migrate authenticated constructors and exact patterns while preserving checked ownership flags. A separate owning lane admits string with at most 4096 Unicode scalars/16384 UTF-8 bytes or Bytes with at most 4093 exact byte values only when the original target is Copy, drop-free, sized and resource-free, has an authenticated constructor, and has no target record pattern. Each constructor appends one fresh owner after every old initializer; Bytes uses the retained compiler-owned bytes_copy and array_as_slice operations, while an inherited field in record update transfers with the consumed base. Full replay must prove the exact Copy-to-needs-drop transition and all cleanup, layout, loan and target checks. No arbitrary expression, owning-pattern rewrite, public ABI guarantee, execution or source authority is introduced.");
            instructions.push_str(" When change/catalog advertises add_variant_case, supply one globally fresh case id/name and exactly one globally fresh field id/name whose type is Bytes. The target must be an explicit monomorphic originally Copy, drop-free, sized, resource-free variant with at least one authenticated existing constructor and no exact target variant pattern. The operation appends only the declaration case: it preserves every old case and constructor byte-for-byte, invents no constructor or handler, and relies on complete Project, interpreter, native and Wasm admission to rebuild the Copy-to-needs-drop transition. String payloads remain explicitly unsupported because the admitted runtime lane is the existing flat owned-Bytes variant algebra. Generic or already-owning variants, scalar/nominal/multiple/empty payloads, target patterns, and source, execution, compatibility or publication authority remain excluded.");

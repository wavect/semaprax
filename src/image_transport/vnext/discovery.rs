...
pub(super) fn payload(
    ...
    if method.name == "protocol/instructions" {
        ...
        if methods
            ...
            .any(|method| method.name == "candidate/apply-intent")
        {
            instructions.push_str(" Typed expressions additionally accept string with only kind and value, and array_u8 with only kind and values. String value is decoded UTF-8 data, including empty strings and escaped controls, bounded to 16384 UTF-8 bytes per literal; it is never raw source. Byte-array values are zero through 255 integers in source order, including an empty array, with at most 4095 elements. Each element consumes the shared expression-node budget in addition to the literal root; enclosing constructors, aggregate JSON and wire limits still apply. No repeated-array form or arbitrary element expressions are introduced. Both forms compose with existing typed lets and ordinary compiler admission; String ownership, fixed-array type/length, loans, cleanup and selected targets remain checked. Signature defaults, new computed signature parameter types, record-field defaults and integer-repair retagging retain their separate scalar/Copy restrictions. Hole constructor discovery lists these forms, but fill suggestions retain their existing place/direct-call search and do not invent literal values.");
        }

...
pub(super) fn payload(
    ...
    if method.name == "protocol/instructions" {
        ...
        {
            instructions.push_str(" When change/catalog offers ordered signature parameters, from and optional name retain the selected existing parameter's exact type and ownership while permitting reordering. Additional checked owning metadata may describe bare string (surface mode value, checked ownership own) or explicitly own named records and variants with sized, resource-free, non-Copy drop facts. Every such owning parameter must be retained exactly once; omission or duplication is rejected. Exact borrow str and borrow Slice<u8> views may also be retained, reordered and renamed, and must each remain present exactly once; no borrowed nominal/storage type, borrow Bytes, shared parameter, resource or class is admitted. Existing caller arguments are evaluated once in their original left-to-right order before mapped arguments reach the ordinary call commit boundary; full loan, provenance, ownership, cleanup, contract and target validation still decides admission. String reads retain ordinary compiler cloning semantics; staging can change allocations and does not promise allocation equivalence. This adds no new view, root lifetime, owning default, copying operation or permission to execute, and the existing computed/new parameter forms remain limited by their separate Copy admission.");

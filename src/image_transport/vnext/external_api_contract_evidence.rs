//! Exact digest-only external API contract evidence over one retained candidate.
//! Declaration bytes carry no endpoint, provider, network, runtime, conformance,
//! ambient, or deployment authority.
...
const METHOD: Method = Method {
    name: "candidate/analysis-external-api-contract-evidence",
    operation: Operation::VNext(Action::CandidateExternalApiContractEvidence),
    parameters: &[
        REVISION,
        Parameter {
...
pub(super) fn prepare(
    ...
    if text(params, "image_revision") != image.image_digest() {
        return Err(failure(
            "SPX-G282",
            "candidate external API contract image revision is stale",
        ));
    }
    let candidate = registry.candidate(text(params, "candidate_revision"))?;
    let report = candidate.analysis_external_api_contract_evidence(
        candidate.candidate_digest(),
        text(params, "declaration").as_bytes(),
        text(params, "declaration_digest"),
    )?;
    if report.len() > MAX_PROJECT_CANDIDATE_EXTERNAL_API_CONTRACT_EVIDENCE_BYTES {
        return Err(failure(
            "SPX-G439",
            "candidate external API contract evidence exceeds its transport byte bound",
        ));
    }
        ...
        || !report.is_char_boundary(offset)
    {
        return Err(failure(
            "SPX-G438",
            "candidate external API contract evidence chunk is outside its bounded UTF-8 report",
        ));
    }
    ...
    if end == offset && offset < report.len() {
        return Err(failure(
            "SPX-G439",
            "candidate external API contract evidence chunk cannot make progress",
        ));
    }
    Ok(json!({
        "schema":CHUNK_SCHEMA,

...
fn build_report_with_complete_limits(
    ...
    let base_schema = graph::legacy_graph_schema(&before).map_err(|error| vec![error])?;
    let candidate_schema = graph::legacy_graph_schema(&candidate).map_err(|error| vec![error])?;

//! Source-authenticated Offline Semantic Package Lock v3.
...
pub const SCHEMA: &str = "semaprax.offline-semantic-package-lock.v3";
pub const SUBJECT_SCHEMA: &str = "semaprax.offline-semantic-package-subject.v3";
...
const SUBJECT_DOMAIN: &[u8] = b"semaprax.offline-semantic-package-subject.v3\0";
const LOCK_DOMAIN: &[u8] = b"semaprax.offline-semantic-package-lock.v3\0";
    ...
    pub fn new(max_bytes: usize) -> Result<Self, Diagnostic> {
        if !(MIN_OUTPUT_BYTES..=MAX_OUTPUT_BYTES).contains(&max_bytes) {
            return Err(wire::option_error(
                "semantic lock v3 max_bytes is outside the frozen range",
            ));
        }
        Ok(Self { max_bytes })
    }
...
pub(crate) struct ResolutionSubject {
    ...
    pub(crate) dependencies: Vec<DependencyRequirement>,
...
pub(crate) fn authenticate_subject_for_resolution(
    ...
    if overflowed {
        return Err(wire::limit_error("semantic subject render budget exceeded"));
    }
    result
}
...
pub fn generate(subjects: &[String], options: &LockOptions) -> Result<String, Vec<Diagnostic>> {
    let (result, overflowed) =
        crate::bounded_output::with_limit(MAX_RENDER_BYTES, || graph::build(subjects, options));
    if overflowed {
        return Err(vec![wire::limit_error(
            "semantic lock render budget exceeded",
        )]);
    }
...
pub fn verify(
    ...
    let (result, overflowed) = crate::bounded_output::with_limit(MAX_RENDER_BYTES, || {
        verify_inner(lock, subjects, options)
    });
    if overflowed {
        return Err(wire::limit_error("semantic lock render budget exceeded"));
    }
    result
}

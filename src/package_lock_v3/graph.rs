...
use super::subject::{parse_subject, render_coordinate, render_requirement};
...
pub(super) fn build(subjects: &[String], options: &LockOptions) -> Result<String, Diagnostic> {
    ...
    for (dependent, subject) in &map {
        for requirement in &subject.dependencies {
            let version = identities
                .get(requirement.package.as_str())
                .ok_or_else(|| confusion_error("missing semantic dependency"))?;
            let dependency = Coordinate {
                package: requirement.package.clone(),
                version: (*version).to_owned(),
            };
            if !crate::package_range::parse_range(&requirement.range, confusion_owned)?.contains(
                crate::package_range::parse_version(&dependency.version, confusion_owned)?,
            ) {
                return Err(confusion_error(
                    "selected dependency version does not satisfy authenticated range",
                ));
            }
            *indegree.get_mut(dependent).expect("present") += 1;
            ...
            edges.push((requirement.clone(), dependency.clone(), dependent.clone()));
    ...
    for rows in dependents.values_mut() {
        rows.sort_by(coordinate_order);
    }
    edges.sort_by(|a, b| {
        coordinate_order(&a.1, &b.1)
            .then(coordinate_order(&a.2, &b.2))
            .then(a.0.range.cmp(&b.0.range))
    });
    ...
    let mut depth = map
        ...
        .map(|c| (c, 1usize))
        .collect::<BTreeMap<_, _>>();
    while let Some(next) = ready.iter().min_by(|a, b| coordinate_order(a, b)).cloned() {
        ready.remove(&next);
    ...
    for coordinate in &order {
        ...
        let mut closure = subject
            ...
            .collect::<BTreeSet<_>>();
        for requirement in &subject.dependencies {
            let version = identities[requirement.package.as_str()];
            let dependency = Coordinate {
                package: requirement.package.clone(),
                version: version.to_owned(),
            };
    ...
    let (lock, overflowed) = bounded_output::with_limit(64 * 1024 * 1024, || {
        let packages = order
            .iter()
            .map(|coordinate| render_package(&map[coordinate], &closures[coordinate], &identities))
            .collect::<Vec<_>>()
            .budgeted_join(",");
        let edge_json = edges
            .iter()
            .map(|(requirement, dependency, dependent)| {
                bf!(
                    "{{\"requirement\":{},\"selected\":{},\"dependent\":{}}}",
...
fn render_package(
    ...
    bf!("{{\"package\":{},\"version\":{},\"subject_digest\":{},\"subject_bytes\":{},\"report_digest\":{},\"report_bytes\":{},\"revision\":{},\"targets\":[{}],\"dependencies\":[{}],\"capabilities\":[{}],\"capability_closure\":[{}]}}", quote_json(&subject.coordinate.package), quote_json(&subject.coordinate.version), quote_json(&subject.digest), subject.bytes, quote_json(&subject.report_digest), subject.report.len(), quote_json(&subject.revision), subject.targets.iter().map(|(t,s)| bf!("{{\"target\":{},\"status\":{}}}",quote_json(t),quote_json(s))).collect::<Vec<_>>().budgeted_join(","), subject.dependencies.iter().map(|d| bf!("{{\"package\":{},\"range\":{},\"selected_version\":{}}}", quote_json(&d.package), quote_json(&d.range), quote_json(identities[d.package.as_str()]))).collect::<Vec<_>>().budgeted_join(","), subject.capabilities.iter().map(|v| quote_json(v)).collect::<Vec<_>>().budgeted_join(","), closure.iter().map(|v| quote_json(v)).collect::<Vec<_>>().budgeted_join(","))
}
...
fn coordinate_order(a: &Coordinate, b: &Coordinate) -> std::cmp::Ordering {
    a.package.cmp(&b.package).then_with(|| {
        let a = crate::package_range::parse_version(&a.version, confusion_owned)
            .expect("authenticated coordinate");
        let b = crate::package_range::parse_version(&b.version, confusion_owned)
            .expect("authenticated coordinate");
        a.cmp(&b)
    })
}

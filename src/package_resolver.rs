//! Authority-free Offline Deterministic Package Resolver v1.
...
pub const SCHEMA: &str = "semaprax.offline-package-resolution-evidence.v1";
...
const DIGEST_DOMAIN: &[u8] = b"semaprax.offline-package-resolution-evidence.v1\0";
const CATALOG_DOMAIN: &[u8] = b"semaprax.offline-package-resolution-catalog.v1\0";
...
pub struct VerifiedResolution {
    pub packages: Vec<package_lock_v2::Coordinate>,
    pub lock: String,
}
...
fn build(input: &ResolutionInput, options: &ResolutionOptions) -> Result<String, Diagnostic> {
    ...
    let selected_subjects = solved
        ...
        .collect::<Vec<_>>();
    let lock_options = package_lock_v2::LockOptions::default();
    let lock = package_lock_v2::generate(&selected_subjects, &lock_options)
        .map_err(|errors| wire::map_lock_errors(&errors, "final Lock-v2 generation failed"))?;
    package_lock_v2::verify(&lock, &selected_subjects, &lock_options)
        .map_err(|error| wire::map_lock_error(&error, "final Lock-v2 replay failed"))?;

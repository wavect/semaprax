...
fn subject(
    ...
    version: &str,
    report: &str,
    dependencies: &[package_lock_v2::Coordinate],
    capabilities: &[&str],
) -> String {
    package_lock_v2::create_subject(
        &package_lock_v2::Coordinate {
...
fn first_feasible_backtracking_and_catalog_permutation_are_exact() {
    ...
    let missing = package_lock_v2::Coordinate {
    ...
    assert_eq!(
        verified.packages,
        vec![package_lock_v2::Coordinate {
            package: "examples.meaning".to_owned(),
...
fn catalog_confusion_and_outer_remints_fail_closed() {
    ...
    let unknown = payload.replacen(
        ...
        "\"schema\":\"semaprax.offline-package-resolution-evidence.v1\",\"unknown\":0",
        1,
    );
    assert_eq!(
        verify(
...
fn input_and_catalog_count_limits_precede_subject_replay() {
    ...
    assert_eq!(wire::charge(&mut work, 1).unwrap_err().code, "SPX-PR505");
...
fn unavailable_and_unproven_candidates_are_policy_rejections() {
    for status in ["unavailable", "unproven"] {
        ...
        let entry = catalog::Entry {
            subject: package_lock_v2::ResolutionSubject {
...
fn public_selected_package_limit_accepts_four_and_rejects_five() {
    ...
    let coordinates = fixtures
        .iter()
        .map(|(_, package)| package_lock_v2::Coordinate {
            package: (*package).to_owned(),
            version: "1.0.0".to_owned(),
        })
        .collect::<Vec<_>>();

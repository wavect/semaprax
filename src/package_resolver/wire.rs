...
pub(super) fn map_subject_error(error: &Diagnostic) -> Diagnostic {
    match error.code {
        "SPX-PL506" => limit_error("nested Subject-v2 bound failed"),
...
pub(super) fn map_lock_error(error: &Diagnostic, message: &str) -> Diagnostic {
    match error.code {
        "SPX-PL506" => limit_error("nested Lock-v2 bound failed"),
        "SPX-PL507" => replay_error(message),
        "SPX-PL503" => authentication_error("nested Lock-v2 authentication failed"),
...
pub(super) fn wire_error(message: impl Into<String>) -> Diagnostic {
    Diagnostic::io("SPX-PR506", message.into())
}
pub(super) fn replay_error(message: impl Into<String>) -> Diagnostic {
    Diagnostic::io("SPX-PR507", message.into())
}

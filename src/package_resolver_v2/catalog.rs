...
use crate::package_lock_v3::{self, ResolutionSubject};

use super::model;
use super::semver::{self, compare_coordinates, Range, Version};
use super::wire;
use super::{
...
pub(super) struct Entry<'a> {
    ...
    pub(super) dependency_ranges: Vec<Range>,
...
pub(super) fn authenticate<'a>(
    ...
    for bytes in &input.subjects {
        ...
        wire::charge(work, 1)?;
        let subject = package_lock_v3::authenticate_subject_for_resolution(bytes, work)
        ...
        let dependency_ranges = subject
            .dependencies
            .iter()
            .map(|dependency| {
                model::validate_identity(&dependency.package, "dependency package")?;
                semver::parse_range(&dependency.range)
            })

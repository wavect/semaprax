...
pub(super) fn render_evidence(
    ...
    let lock_value: Value = serde_json::from_str(lock)
        .map_err(|_| wire::authentication_error("verified Lock-v3 is not JSON"))?;
    let lock_digest = lock_value["digest"]
        .as_str()
        .ok_or_else(|| wire::authentication_error("verified Lock-v3 digest is missing"))?;
    let payload = bf!(
...
pub(super) fn recheck_lock_policy(lock: &str, input: &ResolutionInput) -> Result<(), Diagnostic> {
    ...
    for package in value["payload"]["packages"]
        .as_array()
        .ok_or_else(|| wire::authentication_error("Lock-v3 package rows missing"))?
    {
        ...
        if target != Some("available") {
            return Err(wire::policy_error("Lock-v3 target policy recheck failed"));
        }
        for capability in package["capability_closure"]
            .as_array()
            .ok_or_else(|| wire::authentication_error("Lock-v3 capability closure missing"))?
        {
            if capability
                .as_str()
                .is_none_or(|value| !allowed.contains(value))
            {
                return Err(wire::policy_error(
                    "Lock-v3 capability policy recheck failed",
                ));
            }
        }
    }
...
pub(super) fn exact_lock_bytes(evidence: &str) -> Result<&str, Diagnostic> {
    ...
    let start = evidence
        ...
        .ok_or_else(|| wire::wire_error("exact embedded Lock-v3 missing"))?;
    let end = structural_json_value_end(evidence.as_bytes(), start)?;
    let suffix_end = end
        .checked_add(",\"limits\":".len())
        .ok_or_else(|| wire::wire_error("embedded Lock-v3 boundary overflows"))?;
    if evidence.get(end..suffix_end) != Some(",\"limits\":") {
        return Err(wire::wire_error(
            "embedded Lock-v3 is not followed by outer limits",
        ));
    }
...
fn structural_json_value_end(bytes: &[u8], start: usize) -> Result<usize, Diagnostic> {
        ...
        match byte {
            ...
            b'}' | b']' => {
                if stack.pop() != Some(byte) {
                    return Err(wire::wire_error("embedded Lock-v3 JSON nesting is invalid"));
                }
                if stack.is_empty() {
                    return Ok(index + 1);
                }
    ...
    Err(wire::wire_error(
        "embedded Lock-v3 JSON value is unterminated",
    ))
}

...
impl Parser {
    pub fn parse(mut self) -> Result<Program, Diagnostic> {
        ...
        while !self.at(&TokenKind::Eof) {
            ...
            if self.at_keyword("agent") {
                ...
                if let Some(diagnostic) = self.foreign_declaration() {
        ...
        depth::validate_program(Program {
            path: self.path,
            module,
            module_uses,
            permits,
    ...
    fn prefix_atom(&mut self, allow_record_literals: bool) -> Result<Expr, Diagnostic> {
        ...
        let expression = match token.kind {
            ...
            TokenKind::LParen => {
                ...
                if let Some(diagnostic) = self.tuple_literal() {
                    return Err(diagnostic);
                }
                let end = self
    ...
    fn block_after_open(&mut self, start: Span) -> Result<Expr, Diagnostic> {
        ...
        if let Some(diagnostic) = self
            .foreign_statement()
            .or_else(|| self.compound_assignment())
        {
            return Err(diagnostic);
        }
    ...
    fn match_expression(&mut self, start: Span) -> Result<Expr, Diagnostic> {
        ...
        while !self.at(&TokenKind::RBrace) {
            ...
            if let Some(diagnostic) = self.call_pattern(&pattern) {
                return Err(diagnostic);
            }

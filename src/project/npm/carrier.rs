...
use super::{
    command, command_v2, command_v3, command_v4, data, flat_owned_record, https_command,
    nested_owned_record, network_command, owned_data, package_error, validate_replayed_package,
    UsefulTextNpmPackage, USEFUL_TEXT_PACKAGE_PATHS,
};
...
pub const PROJECT_NPM_BUILD_SCHEMA_V12: &str = "semaprax.project-npm-build.v12";
...
const PROJECT_NPM_BUILD_DIGEST_DOMAIN_V12: &[u8] = b"semaprax.project-npm-build.payload.v12\0";
...
impl ReplayedNpmArtifacts {
    pub(super) fn as_slice(&self) -> &[NpmArtifact] {
        match self {
            ...
            | Self::NetworkCommand(value)
            | Self::HttpsCommand(value) => value,
            Self::Command(value)
...
impl ProjectNpmBuild {
    ...
    fn replay_envelope(
        ...
        let expected_paths: &[&str] = match schema {
            ...
            PROJECT_NPM_BUILD_SCHEMA_V12 => &https_command::PACKAGE_PATHS,
        ...
        let artifacts = match schema {
            ...
            PROJECT_NPM_BUILD_SCHEMA_V12 => ReplayedNpmArtifacts::HttpsCommand(
                artifacts
                    .try_into()
                    .map_err(|_| package_error("npm build artifact inventory is not exact"))?,
            ),
        ...
        let payload_digest = match &artifacts {
            ...
            ReplayedNpmArtifacts::HttpsCommand(artifacts) => {
                https_command::validate_replayed(identity, artifacts)?;
                payload_digest_artifacts_v12(identity, artifacts)
            }
        };
...
pub(in crate::project) fn payload_digest_artifacts_v12(
    identity: NpmBuildIdentity<'_>,
    artifacts: &[NpmArtifact],
) -> String {
    payload_digest_artifacts_with_domain(PROJECT_NPM_BUILD_DIGEST_DOMAIN_V12, identity, artifacts)
}
...
pub(super) fn decode_carrier_artifacts(
    ...
    match schema {
        ...
        PROJECT_NPM_BUILD_SCHEMA_V11 => artifacts
            ...
            .map_err(|_| package_error("npm build artifact inventory is not exact")),
        PROJECT_NPM_BUILD_SCHEMA_V12 => artifacts
            .try_into()
            .map(ReplayedNpmArtifacts::HttpsCommand)
            .map_err(|_| package_error("npm build artifact inventory is not exact")),
        _ => unreachable!("carrier schema selected above"),
    }
}

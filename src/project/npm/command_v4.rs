...
"#;
        ...
        let wasm = crate::wasm::emit_resolved_line_command_io_v1(&program, "test.npm.command.run")
            .unwrap();
...
"#;
        ...
        let wasm =
            crate::wasm::emit_resolved_line_command_io_v1(&program, "test.npm.append_boundary.run")
                .unwrap();
        let package =
            super::render_package("spxgrep", "0.1.0", "test.npm.append_boundary.run", &wasm);
        let directory = std::env::temp_dir().join(format!(
...
"#;
        ...
        let wasm =
            crate::wasm::emit_resolved_line_command_io_v1(&program, "test.npm.range_binding.run")
                .unwrap();
        let package =
            super::render_package("spxgrep", "0.1.0", "test.npm.range_binding.run", &wasm);

//! Project-v13 fixture-driven HTTPS Client I/O v1 npm carrier.
...
use super::{
    artifact, package_error, payload_digest_artifacts_v12, render_carrier_artifacts,
    valid_package_name, valid_package_semver, valid_sha256_fact, NpmArtifact, NpmBuildIdentity,
    ProjectNpmBuild, PROJECT_NPM_BUILD_SCHEMA_V12,
};
...
pub(super) fn prepare(
    ...
    if !manifest.is_v13()
        || manifest.profile() != Some(PROJECT_PROFILE_HTTPS_COMMAND_IO_V1)
        ...
        || manifest.web_exports().len() != 1
    {
        return Err(package_error(
            "HTTPS npm package requires the exact Project v13 profile",
        ));
    }
    let version = manifest
        .package_version()
        .ok_or_else(|| package_error("HTTPS npm package requires a version"))?;
    ...
    let artifact_bytes = artifacts.iter().try_fold(0usize, |total, item| {
        total
            ...
            .ok_or_else(|| package_error("HTTPS npm artifacts exceed the trusted limit"))
    })?;
    ...
    let payload_digest = payload_digest_artifacts_v12(identity, &artifacts);
    let envelope = render_carrier_artifacts(
        PROJECT_NPM_BUILD_SCHEMA_V12,
        identity,
        &artifacts,
        artifact_bytes,
        &payload_digest,
...
fn render_package(name: &str, version: &str, command: &str, wasm: &[u8]) -> [NpmArtifact; 6] {
    ...
    [
        ...
        artifact("semaprax.https.json", metadata.as_bytes()),
...
pub(super) fn validate_replayed(
    ...
    if identity.project_schema != PROJECT_SCHEMA_V13
    ...
    {
        return Err(package_error("HTTPS npm replay identity is invalid"));
    }
    let metadata: serde_json::Value =
        serde_json::from_slice(artifact_bytes(artifacts, "semaprax.https.json")?)
            .map_err(|_| package_error("HTTPS npm metadata is invalid"))?;
    let command = metadata
        .get("command")
        .and_then(serde_json::Value::as_str)
        .ok_or_else(|| package_error("HTTPS npm command identity is absent"))?;
    let ast = crate::parse(
        identity.semantic_recipe,
        Path::new("semaprax-project-npm-https-command-recipe.spx"),
    )
    .map_err(|_| package_error("HTTPS npm semantic recipe does not parse"))?;

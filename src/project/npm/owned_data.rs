    ...
    wasmparser::Validator::new()
        ...
        .map_err(|error| {
            package_error(format!(
                "owned-data app.wasm is not structurally valid: {error}"
            ))
        })?;
    let expected_wasm =
...
fn render_runtime(
    ...
    let facade = if exports.iter().any(|export| {
        ...
        render_mixed_runtime_facade(exports, bounded)
    } else if exports
        .iter()
        .any(|export| export.result != PublicApiResultType::OwnedBytes)
    {
        render_variant_runtime_facade(exports, bounded)
    } else {
        render_runtime_facade(exports, bounded)
    };
...
fn render_mixed_runtime_facade(exports: &[OwnedExport], bounded: bool) -> String {
    ...
    let input_admission = if bounded {
        ...
        r#"const snapshots=values.map((value,index)=>snapshot(value,fact.params[index],`argument ${index}`));let used=0;for(const value of snapshots)if(value instanceof Uint8Array){if(value.byteLength>65536-used)throw new RangeError("SEMAPRAX borrowed input capacity exceeded");used+=value.byteLength}"#
    };
...
fn render_runtime_facade(exports: &[OwnedExport], bounded: bool) -> String {
    ...
    let input_admission = if bounded {
        ...
        r#"const snapshots=values.map((v,i)=>snapshot(v,fact.params[i],`argument ${i}`));let used=0;for(const v of snapshots)if(v instanceof Uint8Array){if(v.byteLength>65536-used)throw new RangeError("SEMAPRAX borrowed input capacity exceeded");used+=v.byteLength}"#
    };
    let facts = exports
        .iter()
        .map(|export| {
...
#[cfg(test)]
#[path = "owned_data/hostile_source_tests.rs"]

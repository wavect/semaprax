...
fn sample() -> WasmProviderBindingV1 {
    WasmProviderBindingV1::new(
        sample_carrier_binding(),
        "sha256:2222222222222222222222222222222222222222222222222222222222222222",
        "spx_pg_wasm_endpoint_reverse_bytes_v1",
        "semaprax-0.4.1",
    )
}
...
fn encode_decode_round_trips() {
    ...
    let decoded = decode_wasm_provider_binding(&bytes).unwrap();
...
fn schema_and_target_profile_are_fixed() {
    let binding = sample();
    assert_eq!(binding.target_profile(), TargetProfile::CoreWasm);
    assert_eq!(
...
#[test]
fn schema_is_unique_from_the_native_adapter_schema() {
    assert_ne!(
        WASM_ADAPTER_SCHEMA,
        crate::public_generic_abi::native::binding::NATIVE_ADAPTER_SCHEMA
    );
    assert_ne!(
        MALFORMED_WASM_BINDING,
        crate::public_generic_abi::native::binding::MALFORMED_NATIVE_BINDING
    );
    assert_ne!(
        WASM_BINDING_REPLAY_MISMATCH,
        crate::public_generic_abi::native::binding::NATIVE_BINDING_REPLAY_MISMATCH
    );
}
...
fn decode_rejects_wrong_schema() {
    ...
    frame(&mut bytes, b"not-the-wasm-adapter-schema");
    let error = decode_wasm_provider_binding(&bytes).unwrap_err();
...
fn decode_rejects_embedded_binding_naming_another_target_profile() {
    ...
    let mut bytes = Vec::new();
    frame(&mut bytes, WASM_ADAPTER_SCHEMA.as_bytes());
    frame(&mut bytes, &wrong_target.encode());
    frame(&mut bytes, WASM_ADAPTER_ABI_VERSION.as_bytes());
    frame(&mut bytes, b"digest");
    frame(&mut bytes, b"export");
...
fn decode_rejects_truncated_bytes() {
    ...
    for cut in [0, 1, 4, bytes.len() - 1] {
        let error = decode_wasm_provider_binding(&bytes[..cut]).unwrap_err();
        assert_eq!(error.code, MALFORMED_WASM_BINDING);
    }
}
...
fn replay_accepts_byte_identical_candidate() {
    ...
    let replayed = replay_wasm_provider_binding(&candidate, &trusted).unwrap();
...
fn replay_rejects_wrong_descriptor_digest() {
    let trusted = sample();
    let candidate = WasmProviderBindingV1::new(
        CarrierBindingV1::new(
    ...
    .encode();
    let error = replay_wasm_provider_binding(&candidate, &trusted).unwrap_err();
    assert_eq!(error.code, WASM_BINDING_REPLAY_MISMATCH);
}
...
#[test]
fn replay_rejects_wrong_export_name() {
    let trusted = sample();
    let candidate = WasmProviderBindingV1::new(
        sample_carrier_binding(),
        "sha256:2222222222222222222222222222222222222222222222222222222222222222",
        "spx_pg_wasm_endpoint_some_other_v1",
        "semaprax-0.4.1",
    )
    .encode();
    let error = replay_wasm_provider_binding(&candidate, &trusted).unwrap_err();
...
fn replay_rejects_wrong_target_profile() {
    ...
    // decode itself already rejects a non-CoreWasm embedded binding above;

...
fn semantic_change_apply_process_child() {
    ...
    let ready = PathBuf::from(std::env::var_os("SEMAPRAX_SEMANTIC_CHANGE_APPLY_READY").unwrap());
...
fn semantic_change_apply_killed_process_boundaries_preserve_exact_old_or_new() {
    for boundary in ["pre", "post"] {
        ...
        let source_bytes = source_paths.map(|path| std::fs::read(fixture.root.join(path)).unwrap());

        let (mut child, _) =
            spawn_semantic_change_apply_process(&fixture, &proposal_path, &evidence_path, boundary);
        child.kill().unwrap();
...
fn real_change_has_exact_typed_delta_impact_and_deterministic_replay() {
    ...
    assert!(
        first.impact().iter().any(|fact| {
            fact.kind == "declaration" && fact.minimum_depth > 0 && fact.root_provenance.len() > 1
        }),
        "impact facts: {:#?}",
        first.impact()
    );
    let base_consumer_module = first
        .impact()
        .iter()
        .find(|fact| fact.state == "base" && fact.kind == "module" && fact.id == "change.consumer")
        .unwrap();
...
fn proposal_limits_and_canonical_replay_are_fail_closed() {
    ...
    let files = (0..=MAX_CHANGED_FILES)
        ...
        .collect();
    let over = SemanticWorkspaceChangeSet::new(DIGEST.to_owned(), "change.entry".to_owned(), files)
        .err()
        .unwrap();
...
fn stale_unmanaged_noop_and_entry_removal_are_rejected() {
    ...
    noop.files[0].source = std::fs::read_to_string(fixture.root.join("a/provider.spx")).unwrap();
...
fn construct_delta_with_limit(
    ...
    let (result, overflowed, used) = crate::bounded_output::with_limit_usage(limit_bytes, || {
...
fn cumulative_builder_and_closure_limits_reject_during_construction() {
    ...
    let prepared = build_authenticated_change(&fixture.root, fixture.proposal()).unwrap();
    let exact = construct_delta_with_limit(&prepared, semantic_workspace::MAX_CHANGE_BUILDER_BYTES)
        .unwrap();
    assert!(exact > 3072);
    ...
    std::thread::scope(|scope| {
        ...
        let writer = scope.spawn(move || {
            apply_authenticated_with_hook(root, proposal_path, evidence_path, |point, _, _, _| {
                if point == SemanticApplyPoint::Workspace(WorkspacePoint::BeforeActiveReplace) {

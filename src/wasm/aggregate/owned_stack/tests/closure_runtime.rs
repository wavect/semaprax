...
use super::super::{derive, longest_paths, FunctionPlan};
use crate::project::{
...
#[test]
fn closure_body_frames_exclude_public_aliases_and_settle_helper_owners() {
...
fn scalar(value:i64)->i64 {
    let local=[7u8,0u8,255u8];
    let owner=bytes_copy(array_as_slice(local));
    if byte_len(bytes_as_slice(owner)) == 3usize { value } else { 0 }
}
@id("stack.bytes")
fn bytes(input: borrow Slice<u8>, zero: i64) -> Bytes {
    let staged = middle(input);
    let numerator=1;
    let callback=fn(divisor:i64)->i64 { scalar(numerator)/divisor };
    let ignored = callback(zero);
    staged
}
@id("stack.option")
...
"#;
    ...
    let old_calls = crate::call_index::PersistentCallIndex::build(&program).unwrap();
    let old_frames = program
        .functions
        .iter()
        .map(|function| {
            (
    ...
    let old_extents = longest_paths(&old_frames, old_calls.calls_by_owner()).unwrap();
    for root in ["stack.bytes", "stack.option"] {
        let id = crate::hir::DeclarationId::new(root);
        assert!(
            extents[&id] > old_extents[&id],
            "fixture must exceed the old omitted-body bound"
        );
    }
    let floors = [
...
for(const [id,size,floor] of fixtures){
  ...
  for(const pointer of [floor-size,65536]){
    ...
    if(copies!==copyBefore+2||drops!==dropBefore+2||owners.size)throw Error('failed helper did not settle exactly once');

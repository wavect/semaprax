...
fn raw_two_owned_generic_variant_rejects_invalid_tag_before_payload_authority() {
    ...
    let source = r#"
module test.wasm_two_owned_variant_hostile;
@id("wasm.hostile.either") variant Either<T, U> {
  @id("wasm.hostile.either.left") Left {
    @id("wasm.hostile.either.left.value") value: T,
  },
  @id("wasm.hostile.either.right") Right {
    @id("wasm.hostile.either.right.value") value: U,
  },
}
@id("wasm.hostile.consume")
fn consume(value: own Either<Bytes, Bytes>) -> i64 {
  match own value {
    Either::Left { value: left } =>
      if byte_len(bytes_as_slice(left)) == 2usize { 2 } else { 0 },
    Either::Right { value: right } =>
      if byte_len(bytes_as_slice(right)) == 3usize { 3 } else { 0 },
  }
}
@id("app.main") fn main() -> i64 { 42 }
"#;
    let resolved =
        hir::resolve(&parse(source, Path::new("wasm-two-owned-variant-hostile.spx")).unwrap())
            .unwrap();
    let bytes = emit_profile(&resolved, true, false).unwrap();
    assert_eq!(bytes, emit_profile(&resolved, true, false).unwrap());
    let id = NEXT_ID.fetch_add(1, Ordering::Relaxed);
    let stem = format!(
        "semaprax-two-owned-variant-hostile-wasm-{}-{id}",
        std::process::id()
    );
    ...
    let consume = format!(
        "__spx_test_{}",
        hex_identity(&DeclarationId::new("wasm.hostile.consume"))
    );
    let script = format!(
...
const unchanged=()=>{{for(const byte of new Uint8Array(memory.buffer,output,8))if(byte!==0xa5)throw Error("hostile tag published output")}};
for(const [tag,carrier,expected] of [[0,2n,2n],[1,3n,3n]]){{
  ...
  if(instance.exports["{consume}"](input,output)!==0||view.getBigUint64(output,true)!==expected)throw Error("valid selected case");
  if(stack.value!==top)throw Error("valid stack restore");
}}
if(drops!==2)throw Error("valid cases did not settle exactly one selected owner");
...
if(!trapped)throw Error("invalid tag did not trap");
unchanged();
if(drops!==2)throw Error("invalid tag granted payload cleanup authority");
"#
    );

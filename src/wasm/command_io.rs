...
"#;
        ...
        let legacy_program = crate::hir::resolve(
            &crate::parse(legacy, Path::new("command-legacy-wasm.spx")).unwrap(),
        )
        .unwrap();
        ...
        let line_program =
            crate::hir::resolve(&crate::parse(&line, Path::new("command-line-wasm.spx")).unwrap())
                .unwrap();
        let v6_error = super::super::emit_resolved_language_command_io_v1(
            &line_program,
...
"#;
        let program = crate::hir::resolve(
            &crate::parse(source, Path::new("command-range-wasm.spx")).unwrap(),
        )
        .unwrap();
        let wasm =
            super::super::emit_resolved_line_command_io_v1(&program, "test.command_range.run")
                .unwrap();
...
"#;
        ...
        .unwrap();
        let wasm = super::super::emit_resolved_line_command_io_v1(
            &program,
            "test.command_range_binding.run",
        )
        .unwrap();
        let directory = std::env::temp_dir().join(format!(
...
"#;
        let program = crate::hir::resolve(
            &crate::parse(source, Path::new("command-capacity-wasm.spx")).unwrap(),
        )
        .unwrap();
        let wasm =

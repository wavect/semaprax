...
const read = decoded => {{ if((decoded.root&0xc0000000)===0x40000000){{const pointer=(decoded.root&0xffff)*8,key=(decoded.root>>>16)&0x1fff,view=new DataView(instance.exports.memory.buffer);if(pointer+32>view.byteLength||view.getUint32(pointer,true)!==key||view.getUint32(pointer+4,true)!==pointer||Number(view.getBigUint64(pointer+24,true))!==decoded.length)throw Error("descriptor");const root=view.getBigInt64(pointer+8,true),offset=Number(view.getBigUint64(pointer+16,true)),all=read(decode(root));if(offset>all.length||decoded.length>all.length-offset)throw Error("range");return all.slice(offset,offset+decoded.length);}} if(decoded.tagged){{const value=entries.get(decoded.token);if(!(value instanceof Uint8Array)||value.length!==decoded.length)throw Error("stale");return value;}} const memory=new Uint8Array(instance.exports.memory.buffer);if(decoded.root>memory.length-decoded.length)throw Error("range");return memory.slice(decoded.root,decoded.root+decoded.length); }};

...
fn private_desktop_packages_are_feature_gated_native_and_source_locked() {
    ...
    assert_contains_all(
        ...
        &[
            ...
            "rust.llvm=22.1.6",
            "macos.clang.version=17.0.0",
            "macos.clang.build=1700.0.13.5",
            "macos.ld.project=ld-1167.5",
            "macos.ld.build-version=1167.5",
...
fn macos_source_lock_rejects_hostile_gate_removal() {
    ...
    for (index, hostile) in [
        ...
        source.replace("@(#)PROGRAM:ld PROJECT:ld-1167.5", "ambient-ld"),
        ...
        source.replace(
            "--remap-path-prefix=$target=/semaprax-private-desktop-target",
            "",
        ),
...
fn windows_source_lock_rejects_hostile_gate_removal() {
    ...
    for hostile in [
        ...
        source.replace("$Matches[1] -ne $visualStudioMajor", "$false"),
        ...
        source.replace(
            "\"/out:$providerFile\", $providerObject) + $providerLibraries",
            "\"/out:$providerFile\", $sourceFile) + $providerLibraries",
        ),
        source.replace(
            "& $lldLinkPath @providerLinkArguments",
            "lld-link @providerLinkArguments",
        ),
        source.replace("'/WX'", "'/WX:NO'"),
        source.replace("'/subsystem:console'", ""),
...
fn windows_contract(source: &str) -> Result<(), String> {
    ...
    require_all(
        ...
        &[
            ...
            "$lldLinkPath --version",
            "& $clangPath -std=c11",
            "-c $sourceFile -o $providerObject",
            "$providerLinkArguments = @('/dll', '/Brepro', '/nodefaultlib', '/noimplib', '/subsystem:console', '/machine:x64', '/WX'",
            "\"/out:$providerFile\", $providerObject) + $providerLibraries",
            "& $lldLinkPath @providerLinkArguments",
            "windows.vswhere.version",
            "windows.visual-studio.major",

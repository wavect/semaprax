//! Candidate-bound compact semantic-impact navigation: authored and intentionally unrun.
...
use semaprax::workspace_analysis::WorkspaceImpactOptions;
...
const IMPACT_DEPTH: usize = 16;
const IMPACT_MAX_BYTES: usize = 1_048_576;
const IMPACT_MAX_NODES: usize = 1024;
const FILES: [&str; 4] = [
    "semaprax.toml",
    "src/app.spx",
    "src/core.spx",
...
fn page_params(candidate: &ProjectCandidate, summary: &Value, view: &str) -> Value {
    json!({"candidate_revision":candidate.candidate_digest(),"target":TARGET,
        "depth":IMPACT_DEPTH,"impact_max_bytes":IMPACT_MAX_BYTES,"max_nodes":IMPACT_MAX_NODES,
        "view":view,"handle":handle(summary,view),"page_size":1,"max_bytes":65536})
}
...
fn candidate_summary_and_pages_equal_library_reports_without_retaining_results() {
    ...
    assert_eq!(
        summary["query"],
        json!({"direction":"reverse","depth":IMPACT_DEPTH,
        "max_bytes":IMPACT_MAX_BYTES,"max_nodes":IMPACT_MAX_NODES})
    );
    assert_eq!(summary["artifact_digest"].as_str().unwrap().len(), 71);
    assert!(summary["truncation"].is_object());
    assert!(summary["budget"].is_object());
    assert_eq!(summary["facets"].as_array().unwrap().len(), 3);
    for view in CandidateImpactView::ALL {
        ...
        let expected: Value = serde_json::from_str(
            &candidate
                .impact_page(
                    ...
                    &token,
                    None,
                    CandidateImpactPageOptions::new(1, 65536).unwrap(),
                )
                .unwrap(),
        )
        .unwrap();
        ...
        for item in actual["items"].as_array().unwrap() {
            assert_eq!(item["schema"], "semaprax.project-candidate-impact-item.v1");
...
fn candidate_grant_selects_closed_schemas_clients_and_mcp_tools_without_widening_old_profiles() {
    ...
    for method in ["candidate/impact-summary", "candidate/impact-page"] {
        assert_eq!(
    ...
    assert!(!readonly_bundle["documents"]
        ...
        .unwrap()
        .iter()
        .any(|row| row["$id"] == "urn:semaprax.project-candidate-impact-summary.v1"));
    for capability in [
    ...
    for (method_name, parameter_count, schema) in [
        (
            "candidate/impact-summary",
            6,
            "urn:semaprax.project-candidate-impact-summary.v1",
        ),
        (
            "candidate/impact-page",
            11,
            "urn:semaprax.project-candidate-impact-page.v1",
        ),
    ] {
        ...
        assert_eq!(params["properties"]["depth"]["minimum"], 0);
        assert_eq!(params["properties"]["depth"]["maximum"], 1024);
        assert_eq!(params["properties"]["impact_max_bytes"]["minimum"], 4096);
        assert_eq!(
            params["properties"]["impact_max_bytes"]["maximum"],
            16_777_216
        );
        assert_eq!(params["properties"]["max_nodes"]["minimum"], 1);
        assert_eq!(params["properties"]["max_nodes"]["maximum"], 8208);
        if method_name.ends_with("page") {
            ...
            assert_eq!(
                ...
                json!(["affected", "dependency_edges", "frontier"])
            );
    ...
    assert!(bundle["unbundled_payload_schemas"]
        .as_array()
        .unwrap()
        .contains(&json!("urn:semaprax.project-candidate-impact-item.v1")));
    for language in ["typescript", "python", "rust"] {
        ...
        for fragment in [
            "candidate/impact-summary",
            "candidate/impact-page",
            "CandidateImpactSummaryPayload",
            "CandidateImpactPagePayload",
            "semaprax.project-candidate-impact-item.v1",
        ] {
    ...
    let initialized = json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{
        "protocolVersion":"2025-11-25","capabilities":{},"clientInfo":{"name":"candidate-impact-evidence","version":"1"}}});
    ...
    assert!(names.contains(&"candidate__impact-summary".to_owned()));
    assert!(names.contains(&"candidate__impact-page".to_owned()));
    mcp.finish().unwrap();
    session.finish().unwrap();
...
fn sequential_and_parallel_candidate_navigation_are_byte_identical_without_registry_mutation() {
    ...
    let requests = [
        ...
        frame(
            ...
            3,
            "candidate/impact-page",
            page_params(&left, &left_summary, "affected"),
        ),
        frame(
            &parallel,
            7,
        ...
        frame(
            ...
            4,
            "candidate/impact-page",
            page_params(&right, &right_summary, "frontier"),
        ),
    ];
...
fn stale_candidates_foreign_references_and_changed_options_fail_without_poisoning_valid_reads() {
    ...
    let valid = page_params(&left, &left_summary, "affected");
    let first = payload(call(&mut session, "candidate/impact-page", valid.clone()));
    let cursor = first["next_cursor"]
        .as_str()
        .expect("calculator add has multiple affected nodes");
    let right_first = payload(call(
        &mut session,
        "candidate/impact-page",
        page_params(&right, &right_summary, "affected"),
    ));
    let foreign_cursor = right_first["next_cursor"]
        .as_str()
        .expect("sibling candidate has multiple affected nodes");
    for (field, value) in [
        ("target", json!("calculator.subtract")),
        ("view", json!("frontier")),
        ("handle", json!(handle(&right_summary, "affected"))),
    ] {
        let mut wrong = valid.clone();
        wrong[field] = value;
        assert_eq!(
            call(&mut session, "candidate/impact-page", wrong)["error"]["code"],
            -32000
        );
    }
    for (field, value) in [
        ("depth", json!(17)),
        ("impact_max_bytes", json!(1_048_575)),
        ("max_nodes", json!(1023)),
        ("page_size", json!(2)),
    ...
    assert_eq!(
        call(&mut session, "candidate/impact-page", foreign)["error"]["code"],
        -32000
    );
    ...
    let request = json!({"jsonrpc":"2.0","id":1,"method":"candidate/impact-page","params":stale});
    ...
    for (field, value) in [
        ...
        assert_eq!(
            call(&mut session, "candidate/impact-page", malformed)["error"]["code"],
            -32602
        );
    }
    assert_eq!(
        payload(call(&mut session, "candidate/impact-page", valid)),
        first
    );
    assert_eq!(fixture.bytes(), disk);
    session.finish().unwrap();

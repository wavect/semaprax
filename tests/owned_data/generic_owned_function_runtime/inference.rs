...
fn inferred_nested_owned_call_results_materialize_ordered_vectors_and_execute_once() {
    ...
    for allowed in [true, false] {
        let inferred_text = nested_source(true, allowed);
        let explicit_text = nested_source(false, allowed);
        ...
        let evaluate = |program: &hir::ResolvedProgram| {
            let call =
                interpreter::retained_call::prepare_retained_call(program, "app.main").unwrap();
            interpreter::retained_call::evaluate_retained_call(program, &call, &[], 100_000)
                .unwrap()
        };

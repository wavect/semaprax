//! Native C11 O0/O2 execution and settlement probe for the SPX-AI-019
//! owned-record collection profile (SPX-AI-020, issue #119).
//!
//! Every case compiles the emitted C with `-Wall -Wextra -Werror`, runs the
//! program four times in one process, and checks three things the interpreter
//! cannot: the published value or selected status, that no heap allocation the
//! generated code made is still live, and that no `vec` authority entry is
//! still live. That is the physical settlement evidence this issue asks for.
...
/// `refusal_mode` injects a failing allocator at exactly one site:
/// `carrier` refuses the element array, `payload` refuses every `Bytes` leaf.
pub(super) fn run_native(
    ...
    let parsed = semaprax::check(source, "owned-record-vec-native.spx").unwrap();
    ...
    assert!(
        ...
        "an owned record element must never be read out by value"
    );
    let tracked = generated
        ...
        .replace(
            ...
            if refusal_mode == "payload" {
                "(uint8_t *)spx_test_refused_calloc((size_t)count, sizeof(uint8_t))"
            } else {
        ...
        .replace(
            ...
            if refusal_mode == "carrier" {
                "(spx_vec_record_v1 *)spx_test_refused_calloc((size_t)capacity, sizeof(spx_vec_record_v1))"
            } else {
                "(spx_vec_record_v1 *)spx_test_calloc((size_t)capacity, sizeof(spx_vec_record_v1))"
            },
        )
        .replace(
...
static __attribute__((unused)) void spx_test_free(void*p){if(p){if(!spx_test_live_allocations)abort();--spx_test_live_allocations;free(p);}}"#;
    for opt in ["-O0", "-O2"] {
        let base = std::env::temp_dir().join(format!(
            "semaprax-owned-record-vec-native-{}-{}-{opt}",
            std::process::id(),

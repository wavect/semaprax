...
const SOURCE: &str = "module utf8.npm;\n@id(\"bytes.raw\") fn raw(value: borrow Slice<u8>) -> Bytes { bytes_copy(value) }\n@id(\"utf8.greeting\") fn greeting() -> string { \"hello\\u{0}世界\" }\n@id(\"utf8.inspect\") fn inspect(value: borrow str, prefix: borrow str) -> bool { !str_is_empty(value) && str_starts_with(value, prefix) && str_contains(value, prefix) }\n@id(\"utf8.length\") fn length(value: borrow str) -> i64 { str_len_bytes(value) }\n@id(\"app.main\") fn main() -> i64 { 0 }\n";
...
fn v10_npm_maps_only_string_results_to_fatal_decoded_js_strings() {
    ...
    fs::write(
        ...
        "import fs from 'node:fs';import instantiate from './semaprax.bindings.js';const wasm=new Uint8Array(fs.readFileSync(new URL('./app.wasm',import.meta.url)));const api=await instantiate(wasm);const text=api.functions['utf8.greeting']();if(typeof text!=='string'||text!=='hello\\0世界')throw Error('string mapping');const starts=api.functions['utf8.inspect']('hello','he'),middle=api.functions['utf8.inspect']('hello','ell');if(starts!==true||middle!==false)throw Error(`borrowed text operations: ${starts}/${middle}`);const input=new Uint8Array([0xff,0xc3,0x28]);const raw=api.functions['bytes.raw'](input);if(!(raw instanceof Uint8Array)||raw.length!==3||raw[0]!==0xff)throw Error('Bytes decoded');let rejected=false;try{api.functions['utf8.length']('x\\ud800y')}catch(error){rejected=error instanceof TypeError}if(!rejected)throw Error('unpaired UTF-16 admitted');console.log('owned-utf8-ok');\n",
    )
    .unwrap();

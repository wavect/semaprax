...
pub(super) fn run_conformance() {
    let directory = temporary("test-bytes-package");
    std::fs::create_dir_all(directory.join("src")).unwrap();
    for file in ["bytes.spx", "examples.spx", "tests.spx"] {
        let source = std::fs::read_to_string(root().join("std/test-bytes/src").join(file)).unwrap();
    ...
    let manifest_source =
        std::fs::read_to_string(root().join("std/test-bytes/semaprax.toml")).unwrap();
    for bundled in [false, true] {
        let manifest = if bundled {
            manifest_source
                .replace("std.test = \"=0.1.0\"\n", "")
                .replace("\"src/bytes.spx\", ", "")
                .replace("std.io = \"=0.1.0\"", "std.test.bytes = \"=0.1.0\"")
        } else {
    ...
    for (index, body) in [
        ...
        project::with_authenticated_project(&directory.join("semaprax.toml"), |snapshot| {
            for _ in 0..2 {
                ...
                let project::ProjectExecutionOutcome::LanguageFailure(status) = report.outcome() else {panic!("case {index}: {report:?}")};
            ...
            for optimization in ["-O0", "-O2"] {
                ...
                assert!(String::from_utf8_lossy(&output.stderr).contains("SEMAPRAX contract failure"), "{}", String::from_utf8_lossy(&output.stderr));
            }
